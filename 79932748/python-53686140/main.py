...
binfo = {     # 原图片,作为掉落物的图片,掉落物的bid,硬度
    0: [None,None,0],
    1001: [lb('草地.png',BLOCK_SIZE),lb('土.png',DROP_SIZE),1002,5],
    1002: [lb('土.png',BLOCK_SIZE),lb('土.png',DROP_SIZE),1002,10],
    1003: [lb('石块.png',BLOCK_SIZE),lb('石块.png',DROP_SIZE),1003,35],
    1004: [lb('树干.png',BLOCK_SIZE),lb('树干.png',DROP_SIZE),1004,20],
    1005: [lb('木板.png',BLOCK_SIZE),lb('木板.png',DROP_SIZE),1005,15],
    1006: [lb('工作台.png',BLOCK_SIZE),lb('工作台.png',DROP_SIZE),1006,30],
    1007: [lb('石工作台.png',BLOCK_SIZE),lb('石工作台.png',DROP_SIZE),1007,60],
    2001: [None,lb('木棍.png',DROP_SIZE),2001,0],
}
data = [
    *([0 for i in range(100)] for line in range(10)),
    *([1001 for i in range(100)] for line in range(1)),
    *([1002 for i in range(100)] for line in range(90)),
    *([1003 for i in range(100)] for line in range(99)),
]
    ...
    def draw(self):
        ...
        if not bag_open:
            ...
            if 0 <= ex < SC_WIDTH and 0 <= ey <= SC_HEIGHT and distance <= self.player.chooserange:
                ...
                self.highbid = self.data[highlight_row][highlight_col]
                if self.highbid:
                    border = 3
                    if mousestate[1]:
...
def create_craft(size):
    ...
    craftsize = size
    for c in craft:      # 回收资源。不知道后果的去掉试试。
        boxclick.remove(c)
...
inventory[2].catch(Card(1004,16))
