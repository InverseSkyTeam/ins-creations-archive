...
#y:        　　   组件右上角的y坐标；
#放置清除按钮和等号按钮
btnClear=tkinter.Button(root,text='C',bg = 'orange',command=lambda:buttonClik('C'))#在root主界面中放置按钮，按钮上显示C，红色，点击按钮后进入buttonClik回调函数做进一步的处理，注意传入了参数‘C’，这样就能分清是哪个按钮按下了
#下面的内容和上面的模式都是一样的
...
for index,operator in enumerate(operators):
    btnOperator=tkinter.Button(root,text=operator,bg = 'white',command=lambda x=operator:buttonClik(x))#创建的过程和上面类似
