try:
    ...
    def move_window(event):
        bug.geometry("+{0}+{1}".format(event.x_root, event.y_root))
    ...
    title_close = Button(title_frame, bg="grey", text="×", activebackground='#ff1e5e', activeforeground='white', padx=5,
                         fg="white", pady=3, command=bug.quit)
    title_close.grid(row=0, sticky=E)
    title_frame.pack()
    text = Text(bug,font=(None,15))
    text.insert(END,chars=b)
    text.pack()
    bug.mainloop()
    c_print('\033[1;36m代码运行结束(hhh)\033[6A\033[8m\033[?25l')
else:
