try:
    ...
    def sj_main():
        ssj = Tk()
        Label(ssj,text="现在时间："+strftime("%H:%M:%S")).pack(fill=X)
        ssj.mainloop()
    ...
    #
    #----------------------------------------------------------------------#
    a = buttonbox(msg="此代码不带图标拖动，要图标拖动吗？(图标拖动版已经很久没更新了(六月13日：已经废了))",title="提示",choices=("要","不要"))
    if a == "要":
        os.startfile(stf3)
        sys.exit()
    else:
                ...
                if button == mouse.RIGHT and xmm == True:
                    if bg.collidepoint(pos):
                        if computer.collidepoint(pos) and delcom == False:
                            ...
                            root15 = Tk()
                            Button(root15,text="打开",command=cd_main).pack(fill=X)
                            root15.mainloop()
