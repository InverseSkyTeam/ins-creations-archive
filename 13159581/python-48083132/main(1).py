try:
            ...
            if a == "登陆":
                ...
                if x.read() == "":
                    ...
                    if str(b) == d.read():
                        ...
                        msgbox("密码错误")
            if a == "退出":
                c_print('\033[1;36m代码运行结束(CWin最后一个版本)\033[6A\033[8m\033[?25l')
                sys.exit()
            if a == "注册":
                y = open("D:\\账号.txt","r")
                if y.read() != "":
    ...
    #
    #----------------------------------------------------------------------#
    a = buttonbox(msg="此代码不带图标拖动，要图标拖动吗？(图标拖动版已经很久没更新了,已经废弃)",title="提示",choices=("要","不要"))
    if a == "要":
        showinfo("你来这干啥呢？","图标拖动版的遗迹")
    else:
        from pgzrun import *
        #窗口大小
        WIDTH = 1000
                    ...
                    if bg.collidepoint(pos):
                        if computer.collidepoint(pos) and delcom == False:
                            ...
                            def com_main():
                                ...
                                def run_stf4():
                                    os.system(stf4)
                                threading.Thread(target=run_stf4).start()
                            def com_del():
                                global delcom
                                root1.destroy()
                                delcom = True
                            ...
                            def ll_main():
                                ...
                                def run_ll():
                                    os.system(stfllq)
                                threading.Thread(target=run_ll).start()
                            def ll_del():
                                global delll
                                root5.destroy()
                                delll = True
                            ...
                            def jsb_main():
                                ...
                                def run_jsb():
                                    os.system(stf9)
                                threading.Thread(target=run_jsb).start()
                            def jsb_del():
                                global deljsb
                                root10.destroy()
                                deljsb = True
                            ...
                            def ewm_main():
                                ...
                                def run_ewm():
                                    os.system(stf10)
                                threading.Thread(target=run_ewm).start()
                            def ewm_del():
                                global delewm
                                root11.destroy()
                                delewm = True
                            ...
                            def dm_main():
                                ...
                                def run_dm():
                                    os.system(stf11)
                                threading.Thread(target=run_dm).start()
                            def dm_del():
                                global deldm
                                root12.destroy()
                                deldm = True
                ...
                if button == mouse.LEFT and xmm == True:
                    ...
                    if cd.collidepoint(pos):
                        ...
                        if hshs == "a":
                            ...
                            def run_cd():
                                os.system(stf13)
                            threading.Thread(target=run_cd).start()
                        elif hshs == "c":
                            try:
                                f = open("D:\\news.txt")
                                fread = f.read()
                                ...
                                showinfo("彩蛋","在D盘中创建news.txt就可以定义一个彩蛋的内容了")
                            else:
                                showinfo("彩蛋",fread)
                    if gj.collidepoint(pos):
                        ...
                        def wgj():
                            ...
                            if gjj == True:
                                ...
                                f.write(str(gjcs))
                                f.close()
                                showinfo("提示","tips:有一个彩蛋需要把代码下载到本地运行才可以正常输出(你可能已经发现了)")
    ...
    def move_window(event):
        bug.geometry(f'+{event.x_root - xwin}+{event.y_root - ywin}')

    def iconic_window():
        bug.overrideredirect(False)
        bug.state('iconic')
    ...
    title_frame = Frame(bug)
    title_label = Label(title_frame, bg="grey", height=2, fg="lightgreen",
                        text="bug　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　")
    title_label.grid(row=0, sticky=W)
    title_label.bind("<B1-Motion>", move_window)
    title_label.bind('<Button-1>', get_pos)
    title_iconic = Button(title_frame, bg="grey", text="—", bd=0, font='bold', fg='white',
                          activebackground='lightskyblue', activeforeground='white', highlightthickness=0,
                          cursor='hand2', command=iconic_window)
    title_iconic.grid(row=0)
    title_close = Button(title_frame,bg="grey",text="×",bd=0,font='blod',activebackground='#ff1e5e',activeforeground='white',fg="white",pady=3,command=bug.destroy,cursor='hand2',highlightthickness=0)
    title_close.grid(row=0, sticky=E)
    title_frame.pack()
    text = Text(bug,font=(None,15))
    text.insert(END,chars=traceback.format_exc())
    text['state'] = 'disabled'
    text.pack()
    run()
    c_print('\033[1;36m代码运行结束(CWin最后一个版本)\033[6A\033[8m\033[?25l')
