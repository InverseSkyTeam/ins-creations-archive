...
class MainWindow:
    ...
    def __init__(self,root):
        ...
        self.frame.grid(row=2, column=2, columnspan=6, rowspan=15)
    #打开
    ...
    def save_button_event(self):
        filename = self.src_filename if self.src_filename else filedialog.askopenfilename(defaultextension=".txt",filetypes=[('txt格式', 'txt')])
        if filename:
            self.__sava_data(filename)
    #另存为
    def other_save_button_event(self):
        f = filedialog.asksaveasfile(defaultextension=".txt",filetypes=[('txt格式', 'txt')])
        if f:
            f.write(self.__get())
    #查找
