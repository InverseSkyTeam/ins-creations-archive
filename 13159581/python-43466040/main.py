            ...
            if button == mouse.LEFT and xmm == True:
                ...
                if gj.collidepoint(pos):
                    ...
                    if gjj == True:
                        ...
                        f.write(str(gjcs))
                        f.close()
                        print("tips:有一个彩蛋需要把代码下载到本地运行才可以发现(你可能已经发现了)")
                        sys.exit()
                    else:
                        pass
                if xm.collidepoint(pos):
                    xmm = False
                if jsb.collidepoint(pos):
                    os.startfile(stf9)
                if ewm.collidepoint(pos):
                    os.startfile(stf10)
                if dm.collidepoint(pos):
                    os.startfile(stf11)
            else:
