import pygame,sys,tkinter.filedialog,tkinter,tkinter.messagebox,os
...
# 体验新版
startlist1 = [pygame.image.load("game_start_up (1).png"),pygame.image.load("game_start_down (1).png")]
startlist1[1] = pygame.transform.scale(startlist1[1],(300,50))
startlist1[0] = pygame.transform.scale(startlist1[0],(300,50))
startrect1 = pygame.Rect(450,470,300,50)
# 上传图片
...
while True:
    for event in pygame.event.get():
        ...
        if startrect1.collidepoint(x1,y1):
            startimg1 = startlist1[1]
            screen.blit(startimg1,startrect1)
            pygame.display.update()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button==1:
                    import webbrowser
                    webbrowser.open("http://code.xueersi.com/home/project/detail?lang=code&pid=13086796&version=offline&form=python&langType=python")
                    sys.exit()
        if not startrect1.collidepoint(x1,y1):
                startimg1 = startlist1[0]
    # 显示图片
    screen.fill((255,255,100))
