...
def helper():
    from main import gz
    gz.create_screen(1400,720,"gameeasy")
    logo = gz.load("img/Mac工作室logo.jpg")
    logo = gz.resize(logo,(112,63))
    logo2 = gz.load("img/gameeasy_logo.png")
    logo2 = gz.resize(logo2,(112,63))
    ...
    step1 = gz.load("img/1.png")
    step1 = gz.resize(step1,(1120,700))
    step2 = gz.load("img/2.png")
    step2 = gz.resize(step2,(1120,700))
    step3 = gz.load("img/3.png")
    step3 = gz.resize(step3,(1120,700))
    step4 = gz.load("img/4.png")
    step4 = gz.resize(step4,(1120,700))
    step5 = gz.load("img/5.png")
    step5 = gz.resize(step5,(1120,700))
    step6 = gz.load("img/6.png")
    step6 = gz.resize(step6,(1120,700))
    r1 = pygame.Rect(0,60,105,25)
    r2 = pygame.Rect(107,60,165,25)
    r3 = pygame.Rect(274,60,105,25)
    y = 0
    y2 = 0
    y3 = 0
    page = 1
    while True:
        ...
        if gz.mouse_button_down():
            ...
            if r3.collidepoint(event.pos):page = 3
        # gz.print_mouse_pos()
        gz.fill((255,255,255))
        ...
        if page == 1:
            ...
            gz.text(size = 23,word = "(2) 创建窗口  gz.creat_screen(宽度,高度,名称,图标(选填参数，ico格式))",color = (255,0,0),pos = (565,152+y))
            ...
            gz.draw(logo,(1260,8))
            gz.draw(logo2,(1132,8))
            # screen.blit(pygame.font.SysFont(FONTNAME,15).render("copyright:Mac工作室",True,(255,255,255)),(720,690))
            gz.text(size = 15,word = "copyright:Mac工作室",color = (0,0,0),pos = (620,35))
            
            pygame.draw.rect(screen,(220,220,220),r1,0)
            pygame.draw.rect(screen,(175,175,175),r2,0)
            pygame.draw.rect(screen,(175,175,175),r3,0)
        if page == 2:
            ...
            pygame.draw.rect(screen,(175,175,175),r3,0)
        if page == 3:
            gz.text(size = 23,word = "调用gameeasy库需要做什么？下面以win10系统为例。",color = (0,0,0),pos = (0,90+y3))
            gz.draw(step1,(0,120+y3))
            gz.text(size = 23,word = "首先打开C站选择python游戏",color = (0,0,0),pos = (0,820+y3))
            gz.draw(step2,(0,850+y3))
            gz.text(size = 23,word = "然后找到此作品",color = (0,0,0),pos = (0,1550+y3))
            gz.draw(step3,(0,1580+y3))
            gz.text(size = 23,word = "点击改编",color = (0,0,0),pos = (0,2280+y3))
            gz.draw(step4,(0,2310+y3))
            gz.draw(step5,(0,3010+y3))
            gz.text(size = 23,word = "将main.py中的代码如图修改并运行",color = (0,0,0),pos = (0,3710+y3))
            gz.draw(step6,(0,3740+y3))
            gz.text(size = 23,word = "将img文件与gameeasy.py文件加入新建作品即可开始coding了~",color = (0,0,0),pos = (0,4440+y3))
            gz.text(size = 23,word = "快去创作作品吧！",color = (0,0,0),pos = (0,4480+y3))
