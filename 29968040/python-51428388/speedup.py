...
@jit(nopython=True, cache=True)
def generate_faces(triangles, width, height, screen, texture_array, intensities):
    ...
    for i in prange(length):
        ...
        for j in prange(minx, maxx + 2):
            for k in prange(miny - 1, maxy + 2):
                # 必须显式转换成 double 参与底下的运算，不然结果是错的
                x = float(j)
                y = float(k)

                m = cross_product(c[0] - a[0], b[0] - a[0], a[0] - x, c[1] - a[1], b[1] - a[1], a[1] - y)
                if abs(m[2]) > 0:
                    bcy = m[1] / m[2]
                    bcz = m[0] / m[2]
                ...
                if zbuffer.get(idx) is None or zbuffer[idx] < z:
                    ...
                    screen[j, k] = int(color[0] * intensity), int(color[1] * intensity), int(color[2] * intensity)
