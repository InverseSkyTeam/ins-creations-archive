...
def render_cube(model, height, width, screen, zbuffer, angle_x, angle_y, tx, ty, tz, fov, O2=True):
    ...
    speedup.generate_faces_flat(
        ...
        model.uv_vertices, clip_vert, model.texture_width, model.texture_height,
        pygame.surfarray.pixels2d(screen), model.texture_array, zbuffer, O2
    )  # 逐个绘制三角形

    # speedup.fog_calc(pygame.surfarray.pixels3d(screen), zbuffer)  # 处理雾效
