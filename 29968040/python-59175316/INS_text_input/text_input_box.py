...
class TextInput:
    def __init__(self, font, font_size, font_color, line_height, rect, init_text, fps) -> None:
        # 期望 fps
        self.fps = int(fps * 1.5)
        ...
        self.tool = Tool(self.rect, self.vScroll, self.hScroll,
                         self.font, self.cursor, self.selected_manager, self.cursor_blink)
        ...
        # 事件处理
        self.event = EventManager(self.tool, self.select_menu, self.keyboard, self.shortcut)
