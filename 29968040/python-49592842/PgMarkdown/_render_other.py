...
def return_line(self):
    ...
    for i, sf in enumerate(line_buffer):
        new_x, new_y = x, y + (line_height - sf['surface'].get_height()) / 2  # 绘制位置在该行正中间
        # self.render_queue_text_screen.append([sf['surface'], (new_x, new_y), sf['href'], sf['hover']])
        self.render_queue_text_screen.append({
            'name': 'surface',
            'surface': sf['surface'],
            'pos': (new_x, new_y),
    ...
    return offest_y + line_height  # 返回整体高度
