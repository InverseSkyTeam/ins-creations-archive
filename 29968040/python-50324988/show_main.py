...
class Blog:
    def __init__(self, w, h, bg_color):
        ...
        self.logo = pygame.transform.smoothscale(pygame.image.load('logo.png'), (101, 24))
    ...
    def render_yzx(self, screen, mouse_pos, w):
        ...
        self.render_text(screen, pygame.Rect(21, 135 + self.avatar.get_height(), w, 0),
                         'Alex袁梓轩', 21, (54,)*3, True)
        self.render_text(screen, pygame.Rect(21, 164 + self.avatar.get_height(), w, 0),
                         '一个热爱编程的初中生', 14, (74,) * 3, True)
        screen.blit(self.pos, calc_rect_center(self.pos, 21, 184 + self.avatar.get_height(), w, self.pos.get_height()))
    ...
    def render(self, screen, mouse_pos, mouse_pressed, pygame_events):
        ...
        for event in pygame_events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.follow_rect.collidepoint(mouse_pos):
                    wb.open('https://code.xueersi.com/space/59842528')
...
class Data:
    ...
    def lexer_code(code):
        ...
        cover = re.sub(r'cover:\s*', '', cover).replace('\n', '')
    ...
    def get_cover(self):
        ...
        for i, url in enumerate(self.data):
            if calculate_md5(url['cover']) + '.png' in files:
                self.data[i]['img'] = pygame.image.load('cover/' + calculate_md5(url['cover']) + '.png')
                continue
            file_data = requests.get(url['cover']).content
            cover = pygame.image.load_extended(BytesIO(file_data)).convert_alpha()
            new_width = (self.rect.width-21)//2
            self.data[i]['img'] = self.scale_img(cover, new_width, int(new_width / 208 * 89))
            pygame.image.save(self.data[i]['img'], 'cover/' + calculate_md5(url['cover']) + '.png')
