...
class Player(entity.Entity):
    ...
    def check_in_frustum(chunk_pos):
        ...
        for plane in planes:
            ...
            normal = plane[:3]
            w = plane[3]
            if np.dot(normal, center + chunk2) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk3) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk4) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk5) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk6) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk7) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk8) + w < 0:
                _out += 1
            else:
                _in += 1
            if np.dot(normal, center + chunk9) + w < 0:
                _out += 1
            else:
                _in += 1
    ...
    def update_matrices(self):
        ...
        self.p_matrix_np = gluPerspective(
            (options.FOV + 10 * (self.speed - WALKING_SPEED) / (SPRINTING_SPEED - WALKING_SPEED)),
