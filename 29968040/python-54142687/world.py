...
class World:
    def __init__(self, player, texture_manager, options):
        ...
        print("计算光照中...")
        for world_chunk in tqdm.tqdm(self.chunks.values()):
    ...
    def propagate_skylight_increase(self, light_update):
        ...
        while self.skylight_increase_queue:
            ...
            for direction in DIRECTIONS_TUPLE:
                ...
                if transparency and _chunk.get_sky_light(lx, ly, lz) < light_level:
                    ...
                    elif _chunk.get_sky_light(lx, ly, lz) + 2 <= light_level:
    ...
    def can_render_chunk(self, chunk_position, player_chunk):
        return self.player.check_in_frustum(np.array(chunk_position, dtype=np.int32)) and dist(player_chunk, chunk_position) <= self.options.RENDER_DISTANCE
    ...
    def prepare_rendering(self):
        ...
        self.visible_chunks = [self.chunks[chunk_position]
                               for chunk_position in self.chunks if self.can_render_chunk(chunk_position, player_chunk)]
        self.sort_chunks()
    
    def sort_chunks(self):
        player_chunk_pos = self.get_chunk_position(self.player.position)
        cmp = lambda a, b: dist(player_chunk_pos, a.chunk_position) - dist(player_chunk_pos, b.chunk_position)
        self.visible_chunks.sort(key=cmp_to_key(cmp))
        self.sorted_chunks = tuple(self.visible_chunks)
    
    def draw(self, screen, mod_line_x):
        ...
        light_buffer = np.ones((*screen.get_size(), 3), dtype=np.float64)  # 深度缓冲数组
        color_buffer = np.zeros((*screen.get_size(), 4), dtype=np.float64)  # 深度缓冲数组
        ...
        for render_chunk in self.visible_chunks:
            render_chunk.draw(color_buffer, z_buffer, final_matrix, daylight_multiplier, light_buffer)
        if mod_line_x is not None:
            _mod_line_x = int(screen.get_width() * mod_line_x)
            light_buffer[_mod_line_x:, :, 0] = color_buffer[_mod_line_x:, :, 2]
            light_buffer[_mod_line_x:, :, 1] = color_buffer[_mod_line_x:, :, 2]
            light_buffer[_mod_line_x:, :, 2] = color_buffer[_mod_line_x:, :, 2]
        color_mix(pygame.surfarray.pixels3d(screen), color_buffer, light_buffer, self.texture_manager.texture_array)
