...
def render_face(screen, zbuffer, ptsa, ptsb, ptsc,
                uva, uvb, uvc, width, height, texture_array,
                light_a, light_b, light_c, O2):
    ...
    miny, maxy = screen.shape[1] - miny - 1, screen.shape[1] - maxy - 1
    for j in prange(minx, maxx):
        ...
        if (m0 < 0 and addm2 <= 0) or (m1 < 0 and addm1 <= 0) or (m2 < 0 and addm0 <= 0):
            continue
        n = max(calc(m0, addm2), calc(m1, addm1), calc(m2, addm0))
        if n:
            m0 += n * addm2
            m1 += n * addm1
            m2 += n * addm0
            ...
            u += n * u_add
            v += n * v_add
            light_r += n * light_r_add
            light_g += n * light_g_add
            light_b1 += n * light_b_add
        for k in prange(miny-n, maxy, -1):
...
@jit(nopython=True, fastmath=True, looplift=True, parallel=False)
def generate_faces(screen, z_sorted, indices, uv_indices, pts, uv_triangle, uv_index, width, height,
                   v_light_r, v_light_g, v_light_b, texture_arrays, z_buffer, O2):
    # 使用 z-buffer 算法绘制三角形
    for _i in prange(indices.shape[0]):
        i = z_sorted[_i]
        ...
        if nums:  # 透视裁剪
            ...
            if out_vert_num == 3:
                ...
                light_b = out_light[1, 0], out_light[1, 1], out_light[1, 2]
                light_c = out_light[2, 0], out_light[2, 1], out_light[2, 2]
                render_face(screen, z_buffer, ptsa, ptsb, ptsc,
                            uva, uvb, uvc, width, height, texture_array,
                            light_a, light_b, light_c, O2)
                uva = out_uv[0, 0], out_uv[0, 1]
...
@jit(nopython=True, fastmath=True, looplift=True, parallel=True)
def generate_faces_parallel(screen, z_sorted, indices, uv_indices, pts, uv_triangle, uv_index, width, height,
                            v_light_r, v_light_g, v_light_b, texture_arrays, z_buffer, O2):
    # 使用 z-buffer 算法绘制三角形
    for _i in prange(indices.shape[0]):
        i = z_sorted[_i]
