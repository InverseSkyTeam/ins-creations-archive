...
class World:
    ...
    def can_render_chunk(self, chunk_position, player_chunk):
        return dist(player_chunk, chunk_position) <= self.options.RENDER_DISTANCE and self.player.check_in_frustum(chunk_position)

    def prepare_rendering(self):
        player_chunk = self.get_chunk_position(self.player.position)
        self.visible_chunks = [self.chunks[chunk_position] for chunk_position in self.chunks if self.can_render_chunk(chunk_position, player_chunk)]
        self.sort_chunks()
