...
class Cursor:  # 文本框中的指针
    ...
    def get_cursor_text(self):  # 获取光标右边的一个字符
        return self.text.text[self.row][self.column]

    def get_cursor_left_text(self):  # 获取光标左边的一个字符
        return self.text.text[self.row][self.column - 1]
    ...
    def set_cursor_pos(self, row, column):  # 设置光标在文本中坐标
        ...
        current_text = self.text.text[self.row]
        text_width = [self.text.font.rendered_text_width[char] for char in current_text]
        self.x = sum(text_width[:column])
        self.y = row * self.text.font.line_height

    def get_last_cursor_pos(self):  # 获取光标在文本末尾时的位置
        return len(self.text.text) - 1, len(self.text.text[-1])

    def set_last_cursor_pos(self):  # 光标移到文本末尾
        self.row, self.column = self.get_last_cursor_pos()
        self.x = self.text.line_width[-1]
        self.y = self.row * self.text.font.line_height
    ...
    def set_screen_pos(self, x, y):  # 设置光标在屏幕中坐标
        ...
        # 计算光标行的位置
        line_index = relative_y // self.text.font.line_height

        # 特殊情况
        if line_index >= len(self.text.text):
            self.row, self.column = len(self.text.text) - 1, len(self.text.text[-1])
            self.y = (len(self.text.text) - 1) * self.text.font.line_height
            self.x = self.text.line_width[self.row]
            return
        if line_index < 0:
        ...
        # 设置光标行位置
        self.row = line_index
        self.y = line_index * self.text.font.line_height
        ...
        if col >= len(text_width):
            ...
            self.x = self.text.line_width[line_index]
        elif col < 0:
    ...
    def is_first_line(self):  # 光标是否在第一行
        return self.text.is_first_line(self.row)

    def is_last_line(self):  # 光标是否在最后一行
        return self.text.is_last_line(self.row)

    def is_line_start(self):  # 光标是否在某一行的开始
        return self.text.is_line_start(self.column)

    def is_line_end(self):  # 光标是否在某一行的末尾
        return self.text.is_line_end(self.row, self.column)

    def cursor_last_line(self):  # 光标移到上一行末尾
        if not self.is_first_line():
            self.row -= 1  # 光标上移
            self.y -= self.text.font.line_height

            self.column = len(self.text.text[self.row])  # 光标移到这一行末尾
            self.x = self.text.line_width[self.row]
    ...
    def cursor_left(self):  # 光标左移
        if self.is_line_start():  # 光标在一行开头
            ...
            self.column -= 1  # 光标左移
            self.x -= self.text.font.rendered_text_width[self.get_cursor_text()]
    ...
    def delete_char(self):
        ...
        if not self.is_line_start():
            self.row, self.column, del_char = self.text.remove_char(self.row, self.column)
            self.x -= self.text.font.rendered_text_width[del_char]
            return

        self.column = len(self.text.text[self.row - 1])
        self.x = self.text.line_width[self.row - 1]

        self.y -= self.text.font.line_height
        self.row = self.text.remove_line_break_char(self.row)  # 删除换行符

    def add_string(self, string):
        self.x += self.text.add_string(self.row, self.column, string)
        self.column += len(string)

    def add_line_break_char(self):
        self.text.add_line_break_char(self.row, self.column)
        self.cursor_next_line()
    ...
    def add_text(self, text):
        ...
        if len(lines) == 1:
            ...
            return
        width = self.text.add_text(self.row, self.column, lines)
        self.column = len(lines[-1])
        self.x = width

        self.row += len(lines) - 1
        self.y += (len(lines) - 1) * self.text.font.line_height
