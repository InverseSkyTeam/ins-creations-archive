...
def show_text(screen,text,color=(0,0,0),pos=(0,0),size=30):
    screen.blit(pygame.font.Font('方正达利体.ttf', size).render((text),True,color),pos)
...
def calc_Block_number(x,y,surface):#计算格子的编号
    shi_wei = str( (x*(surface+2) + y*(surface)) % 10)#十位
    ge_wei = str( (x*(surface) + y*(surface+2)) % 10)#个位
    return shi_wei+ge_wei
class Block:#格子类（单个格子），这个功能很单一，可以简单理解为c++结构体的替代
    def __init__(self,x,y,sf,mode):#x,y:当前格子xy,sf:当前格子在哪个面上 mode:当前格子类型
        ...
        self.wh=200
        #当格子是传送门时，mode是 goto-方向 如goto-left
