...
try:
    ...
    import pypandoc_binary  # 如果没有pypandoc库，就要故意导入pypandoc_binary，让c站安装，因为windows和mac只有安装pypandoc_binary才有效果
...
class Load:
    ...
    def parse_markdown_file(self):
        ...
        for i in inline:  # 存入缓存
            self.in_line_LaTeX[i[0]] = pygame.image.load('images/'+calculate_md5(i[0])+'.png')
        for i in not_inline:
            self.LaTeX[i[0]] = pygame.image.load('images/'+calculate_md5(i[0])+'_.png')
