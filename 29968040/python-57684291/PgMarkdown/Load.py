import re
from .util.code_hlight import CodeHilite
...
with open(path + '/assest/error.png', 'rb') as f:
    error_img = f.read()
...
def get_video_t(v_add):
    ...
    sec = int(res.group()[2:]) if res else 0
...
class Load:
    ...
    def set_markdown_from_string(self, md_string):
        ...
        self.html = md_to_html(md_string)
        self.blocks = None
    ...
    def parse_markdown_file(self):
        ...
        # 解析 HTML 代码
        # with open('yzx.html', 'w', encoding='utf-8') as f:
        #     f.write(self.html)
        ...
        loading_img = pygame.font.Font('HarmonyOS_Regular.ttf', 16).render('LaTeX加载中...', True, (125, )*3)
        for span in span_tags:
            if span.name == 'span':
                ...
                if src.startswith('bilibili:'):
                    ...
                    continue
                self.img_url[src] = pygame.image.load(path + '/assest/error.png')
                self.now_load_LaTeX.append((src, 'img'))
