...
class ScrollStatus:
    no_dragging = 0  # 滚动条没有被拖动
    mouse_dragging = 1  # 被鼠标拖动
    finger_dragging = 2  # 滚动条所在内容区域被手指滑动（类似移动端）


class ScrollBar:
    def __init__(self, container_length, content_length):
        self.allow_finger = False  # 是否允许手指拖动
    ...
    def save(self):
        return (self.scroll_offset, self.content_offset, self.scroll_dragging)
    
    def load(self, data):
        self.update_scroll_offset(data[0])
        self.update_content_offset(data[1])
        self.scroll_dragging = data[2]
...
class HScrollBar(ScrollBar):  # 水平滚动条
    def __init__(self, rect):
        ...
        self.rect = rect
        self.scroll_dragging = ScrollStatus.no_dragging  # 是否在拖动滚动条
    ...
    def scroll_rect(self):
        return pygame.Rect(self.rect.x + self.scroll_offset, self.rect.bottom - 5, self.scroll_length, 5)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and self.need_scrollbar():
                    if self.scroll_rect.collidepoint(event.pos):
                        self.scroll_dragging = ScrollStatus.mouse_dragging
                        self.scroll_color = self.scroll_color_hover
                    elif self.allow_finger and self.rect.collidepoint(event.pos):
                        self.scroll_dragging = ScrollStatus.finger_dragging
            elif event.type == pygame.MOUSEMOTION:
                if self.scroll_dragging == ScrollStatus.mouse_dragging:
                    self.update_scroll_offset(self.scroll_offset + event.rel[0])
                elif self.allow_finger and self.scroll_dragging == ScrollStatus.finger_dragging:
                    self.update_content_offset(self.content_offset - event.rel[0])
            elif event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    self.scroll_dragging = ScrollStatus.no_dragging
                    self.scroll_color = self.scroll_color_normal
...
class VScrollBar(ScrollBar):  # 竖直滚动条
    ...
    @property
    def scroll_rect(self):
        return pygame.Rect(self.rect.right - 5, self.rect.y + self.scroll_offset, 5, self.scroll_length)

    def handle_event(self, events):
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 4 and self.need_scrollbar():  # 滚轮移动
                    self.update_content_offset(self.content_offset - 20)
                if event.button == 5 and self.need_scrollbar():
                    self.update_content_offset(self.content_offset + 20)
                if event.button == 1 and self.need_scrollbar():
                    if self.scroll_rect.collidepoint(event.pos):  # 拖动滚动条
                        self.scroll_dragging = ScrollStatus.mouse_dragging
                        self.scroll_color = self.scroll_color_hover
                    elif self.allow_finger and self.rect.collidepoint(event.pos):  # 类似于移动端的滑动
                        self.scroll_dragging = ScrollStatus.finger_dragging
            elif event.type == pygame.MOUSEMOTION:
                if self.scroll_dragging == ScrollStatus.mouse_dragging:
                    self.update_scroll_offset(self.scroll_offset + event.rel[1])
                elif self.allow_finger and self.scroll_dragging == ScrollStatus.finger_dragging:
                    self.update_content_offset(self.content_offset - event.rel[1])
            elif event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    self.scroll_dragging = ScrollStatus.no_dragging
                    self.scroll_color = self.scroll_color_normal
