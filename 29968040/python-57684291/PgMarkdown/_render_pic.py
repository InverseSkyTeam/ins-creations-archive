...
video_state = pygame.transform.smoothscale(pygame.image.load(path + '/assest/video_state.png'), (100, 100))
...
def render_inline_LaTeX(self, x, y, end_x, line_start_x, font_size, block):
    ...
    if font_size / self.font_normal_size != 1:  # 当前字体大小不是正常大小
        LaTeX_width, LaTeX_height = LaTeX_surface.get_size()
...
def render_LaTeX(self, x, y, end_x, line_start_x, font_size, block):
    ...
    if font_size / self.font_normal_size != 1:  # 当前字体大小不是正常大小
        ...
        LaTeX_surface = pygame.transform.smoothscale(LaTeX_surface,
                                                     (round(LaTeX_width * font_size / self.font_normal_size),
                                                      round(LaTeX_height * font_size / self.font_normal_size)))
    LaTeX_width, LaTeX_height = LaTeX_surface.get_size()
    if end_x - line_start_x > LaTeX_width:
        new_sf = pygame.Surface((end_x - line_start_x, LaTeX_height), pygame.SRCALPHA)
        new_sf.blit(LaTeX_surface, LaTeX_surface.get_rect(center=new_sf.get_rect().center))
        LaTeX_surface = new_sf
    last_line_height = self.return_line()  # 强制闭合上一行
    x = line_start_x  # 重设 x 坐标
    y += last_line_height  # 更新 y 坐标
    self.line_buffer = [
...
def blue_img(sf):
    sf_bytes = pygame.image.tobytes(sf.convert_alpha(), 'RGBA')
    pil_image = Image.frombytes('RGBA', sf.get_size(), sf_bytes)
...
def render_video_t(video_t):
    font = pygame.font.Font('HarmonyOS_Regular.ttf', 14)
...
def _render_title(title):
    font = pygame.font.Font('HarmonyOS_Regular.ttf', 16)
...
def render_img(self, x, y, end_x, line_start_x, font_size, block, href):
    """
    把图片或bilibili视频绘制到 screen(line-buffer) 里
    :param self: MarkdownRenderer类
    :param x: 当前行起始的 x 坐标
    :param y: 当前行起始的 y 坐标
    :param end_x: 结束 x 坐标
