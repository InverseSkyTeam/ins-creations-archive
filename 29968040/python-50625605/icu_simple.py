...
with open('data1.pickle', 'rb') as file2:
    data_u_charType = pickle.load(file2)


def u_charType(x):
    if isinstance(x, int):
        return data_u_charType[x]
    else:
        return data_u_charType[ord(x)]
...
def U16_GET_SUPPLEMENTARY(lead, trail):
    (ord(lead) << 10) + (ord(trail) - U16_SURROGATE_OFFSET)


# uprops
with open('data.pickle', 'rb') as file2:
    data_u_getIntPropertyValue = pickle.load(file2)


def u_getIntPropertyValue(x, b):
    if isinstance(x, int):
        return data_u_getIntPropertyValue[x]
    else:
        return data_u_getIntPropertyValue[ord(x)]
...
def GetIterator(string):
    boundary = BreakIterator.createLineInstance(Locale.getUS())
