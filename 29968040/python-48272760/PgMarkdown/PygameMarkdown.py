from . import _html_label, markdown2, mdx_math, strike
import markdown
import pygame
...
class MarkdownRenderer:
    ...
    def __init__(self):
        ...
        # 滚动条
        self.scroll = 0  # 内容滚动的距离
        self.scroll_line = 0  # 滚动条走的距离
        self.scroll_height = 0
        self.scroll_color = (153, 153, 153)
        self.scroll_width = 5
        self.last_y = None
        self.offest_y = None
        self.in_scroll = False  # 是否在拖动滚动条
    ...
    def set_markdown_from_string(self, md_string):
        ...
        extensions = ['sane_lists', 'tables', 'fenced_code',  # , 'codehilite'
                      mdx_math.MathExtension(), strike.DelExtension()]
    ...
    def set_area(self, surface, offset_x, offset_y, width=-1, height=-1, margin=10) -> None:
        ...
        self.scroll_height = self.w / self.render_height * self.w
    ...
    def render_to_screen(self):
        for i in self.render_queue_text_screen:
            x, y = i[1][0], i[1][1] - self.scroll
            if i[2] and self.mouse_pressed and \
                    pygame.Rect(x, y, i[0].get_width(), i[0].get_height()).collidepoint(
                        (self.mouse_x, self.mouse_y)):
                wb.open(i[2])
            self.text_screen.blit(i[0], (x, y))
        for i in self.render_queue_border_screen:
            i[0].draw_border(self.border_screen, i[1], i[2] - self.scroll, i[3], i[4], i[5])
    ...
    def display(self, pygame_events, mouse_x, mouse_y, mouse_pressed):
        ...
        if self.render_height > self.w:
            scroll_rect = pygame.Rect(self.x+self.w, self.y+self.scroll_line, self.scroll_width, self.scroll_height)
            pygame.draw.rect(self.screen, self.scroll_color, scroll_rect, border_radius=2)
            if mouse_pressed:
                if not self.in_scroll:
                    if scroll_rect.collidepoint(mouse_x, mouse_y):
                        self.in_scroll = 1
                        self.offest_y = mouse_y - self.y - self.scroll_line
                        # print(self.offest_y)
                else:
                    new_y = mouse_y - self.offest_y
                    self.scroll_line = max(self.y, min(self.y+self.h-self.scroll_height, new_y)) - self.y
                    # pygame.draw.line(self.screen, (0, 0, 0), (self.x+self.w, new_y),(self.x+self.w + 5, new_y))
                    self.scroll = (self.render_height - self.h) / (self.h - self.scroll_height) * self.scroll_line
            else:
                self.in_scroll = 0
                self.offest_y = 0
            for event in pygame_events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.button == 4:
                        self.scroll -= 50
                        self.scroll = max(0, min(self.scroll, self.render_height - self.h))
                        self.scroll_line = self.scroll * (self.h - self.scroll_height) / (self.render_height - self.h)
                    elif event.button == 5:
                        self.scroll += 50
                        self.scroll = max(0, min(self.scroll, self.render_height - self.h))
                        self.scroll_line = self.scroll * (self.h - self.scroll_height) / (self.render_height - self.h)
