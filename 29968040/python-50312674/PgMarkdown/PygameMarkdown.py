...
def get_label_type(block):
    ...
    is_inline_latex = block.name == 'span' and block['class'] == ['math', 'inline']

    if block.name in ['strong', 'em', 's', 'a', 'img', 'br', 'code'] or is_inline_latex:
        return 'inline'  # 内联标签
...
class MarkdownRenderer:
    ...
    from ._render_inline_LaTeX import render_inline_LaTeX, render_img
    ...
    def __init__(self):
        ...
        self.file_data = None
        self.tps_scroll_dragging = {}
        self.tps_contentScrollLength = {}
        self.tps_scrollbar = {}
    ...
    def update(self):
        ...
        _, end_y, mg = self.render(self.blocks, self.x, [self.y], self.x + self.w, self.x,
        ...
        if self.render_height > self.h:  # 渲染区域太大，需要滚动条
            ...
            x, w, h = self.x + self.w, 5, self.h / self.render_height * self.h
            y = self.y + self.contentScrollLength * (self.h - h) / (self.render_height - self.h)
            self.scrollbar: pygame.Rect = pygame.Rect(x, y, w, h)
        else:
            self.scrollbar = None  # 不需要滚动条
            self.contentScrollLength = 0  # 初始化内容滚动的距离
            self.scroll_dragging = False  # 取消拖动
        ...
        for tbody in block:
            ...
            for line in tbody:
                ...
                for tblock in line:
                    ...
                    y1 = self.render(tblock, 0, [0], 2147483647, 0, self_font_size, font_color,
                                     font_style + (['strong'] if is_header else []), 0)[1]
    ...
    def render(self, blocks, start_x, start_y, end_x, line_start_x,
        ...
        for i, block in enumerate(block_contents):  # 遍历内部的所有标签
            ...
            # 块级标签，行间公式，嵌套列表
                ...
                if data.padding_top + data.border_top_width > 0 or block.name in ['hr', 'pre', 'span', 'table']:
                    ...
                    if can_add:
                        ...
                        can_add = False
                    margin = 0
                if data.padding_bottom + data.border_bottom_width > 0 or block.name in ['hr', 'pre', 'span', 'table']:
                ...
                if block.name == 'li':  # 列表项
                    if list_start is None:  # 无序列表
                        ...
                        self.render_num = self.set_font(font_size, font_style).render(str(list_start) + '.', True,
                                                                                      self.font_normal_color)
                        list_start += 1  # 编号加一
                if block.name not in ['hr', 'pre', 'span', 'table']:  # 这不是分割线、多行代码块、表格、行间公式，直接绘制
                    ...
                    y_temp = yy[0]
                    _, y1, margin = self.render(block, content_sx, yy, content_ex, content_sx,
                                                self_font_size, font_color, font_style + add_style, margin, start_list)  # 递归绘制标签的子节点
                ...
                if block.name == 'pre':  # 多行代码块
                    pre_data = PreCode(block.contents[0].contents, self_font_size, data,
                                       content_ex - content_sx + data.padding_left + data.padding_right)
                    content_height = pre_data.height
                    self.render_queue_text_screen.append({
                        'name': 'pre',
                        'data': pre_data,
                        'pos': (content_sx - data.padding_left, y + data.border_top_width + data.padding_top)
                    })
    ...
    def render_to_screen(self, pygame_events, mouse_x, mouse_y, mouse_pressed):
        ...
        for i in self.render_queue_text_screen_temp:
            ...
            if i['name'] == 'pre':
                tps_num += 1
                if self.tps_scroll_dragging.get(tps_num) is not None:
                    i['data'].scroll_dragging = self.tps_scroll_dragging[tps_num]
                    i['data'].contentScrollLength = self.tps_contentScrollLength[tps_num]
                    i['data'].scrollbarLength = self.tps_scrollbar[tps_num]
                i['data'].render(self.text_screen, x, y, pygame_events, mouse_x, mouse_y, self.y, self.h)
                self.tps_scroll_dragging[tps_num] = i['data'].scroll_dragging
                self.tps_contentScrollLength[tps_num] = i['data'].contentScrollLength
                self.tps_scrollbar[tps_num] = i['data'].scrollbarLength
                continue
            if i['name'] == 'table':
                tps_num += 1
                if self.tps_scroll_dragging.get(tps_num) is not None:
        ...
        for num, i in enumerate(self.render_queue_border_screen_temp):
            is_choose = i[0].draw_border(self.border_screen, i[1], i[2] - self.contentScrollLength,
                                         i[3], i[4], mouse_x, mouse_y)
            if is_choose and has_choose and self.debug:
                has_choose = 0
                i[0].draw_debug_border(self.text_screen, i[1], i[2] - self.contentScrollLength, i[3], i[4])
        ...
        all_rect = pygame.Rect(self.x, self.y, self.w, self.h)
        if has_choose and all_rect.collidepoint(mouse_x, mouse_y) and self.debug:
            _html_label.Label.draw_alpha_rect(self.text_screen, (111, 168, 220, int(0.66*255)), all_rect)
        if debug_sf is not None:
            self.text_screen.blit(debug_sf, (self.x + (self.w - debug_sf.get_width())//2, self.y))
