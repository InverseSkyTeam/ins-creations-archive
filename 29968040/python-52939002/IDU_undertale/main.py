...
if opsystem_info['system_name'] == 'Windows':
    exec(
        "def show_text(text,color=(0,0,0),pos=(0,0),size=30):screen.blit(pygame.font.SysFont('kaiti', size).render((text),True,color),pos)")
else:
    print('非windows系统不一定支持哦')
    exec(
        "def show_text(text,color=(0,0,0),pos=(0,0),size=30):screen.blit(pygame.font.SysFont('kaittf', size).render((text),True,color),pos)")
...
BGM = pygame.mixer.Sound('./music/三重审判.wav')
sound_die = pygame.mixer.Sound('./music/逝世.wav')
sound_hurt = pygame.mixer.Sound('./music/伤害.wav')
BGM.play()
...
class Laser(object):
    def __init__(self, x, y, to, size=100, wait=(0.6, 0.4)):
        self.size = size
...
class Windmill(object):
    def __init__(self, center, r=200, size=20, circles=3, speed=0.1, wait=0.001, count=0):
        self.center = center
        self.size = size
        self.circles = 3
        self.total = 36 * circles
        self.count = count
        self.r = r
        self.speed = speed
        self.wait = wait
    ...
    def setrect(self):
        self.rect_list = []
        for i in range(1, 36 * self.circles + 1):
            angle = math.radians(10 * i)
            x = math.cos(angle) * self.r
            y = math.sin(angle) * self.r
            self.rect_list.append([pygame.Rect(x + self.center[0], y + self.center[1], self.size, self.size), 1.0])
        self.rect_list *= self.circles

    def draw(self):
        for i in self.rect_list:
            pygame.draw.rect(screen, (0, 255, 255), i[0], 0)

    def auto(self, player):
        for i in range(len(self.rect_list)):
            if i <= self.count:
                if self.rect_list[i][1] > 0: self.rect_list[i][1] -= self.wait
                self.rect_list[i][0].x = (self.rect_list[i][0].x - self.center[0]) * self.rect_list[i][1] + self.center[
                    0]
                self.rect_list[i][0].y = (self.rect_list[i][0].y - self.center[1]) * self.rect_list[i][1] + self.center[
                    1]
            if self.rect_list[i][0].colliderect(player.rect):
                player.hurt(2)
        self.count += self.speed
        if self.count > self.total + 10:
            self.used = 1
        self.draw()
...
fight_list = [
                 ...
                 Laser(
                     random.randint(play_rect.left, play_rect.right - 200),
                     play_rect.top - 260,
                     'bottom',
                     200,
                     (0.7, 0.6)
                 ...
                 Windmill(player.rect.center, 1500, speed=0.08, count=-5),
                 Windmill(player.rect.center, 1500, speed=0.2),
                 Windmill(player.rect.center, 300, 10, 2, 0.25, 0.002, -5),
             ] + [
                 Laser(
                     play_rect.left - 140,
                     random.randint(play_rect.top, play_rect.bottom - 120),
                     'right',
                     size=120,
                 ) for i in range(10)
             ]

def next(pygame_event):
    ...
    if fight_list:
        if len(fight_list) in range(15, 31):
            fight_list[0].lock(player)
        fight_list[0].auto(player)
        if fight_list[0].used:
            del fight_list[0]
    else:
        gamepart = 'win'
        return
