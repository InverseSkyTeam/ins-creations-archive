from . import _html_label, strike
...
class MarkdownRenderer:
    ...
    def set_markdown_from_string(self, md_string):
        ...
        extensions = ['sane_lists', 'tables', 'pymdownx.superfences', 'pymdownx.betterem',
                      'pymdownx.arithmatex', strike.DelExtension()]
        config = {
            'pymdownx.arithmatex': {
                'preview': False,
                'block_tag': 'Latex_block',
                'inline_tag': 'Latex_inline'
            }
        }
        self.html = markdown.markdown(md_string, extensions=extensions, extension_configs=config)
    ...
    def parse_markdown_file(self):
        ...
        div_tags = self.blocks.find_all('div')
        ...
        for div in div_tags:
            if div.get('class')[0] == 'highlight':
                div.find('pre').replace_with(div.find('code'))
                div.name = 'pre'
        # print(self.blocks)
    ...
    def render(self, blocks, start_x, start_y, end_x, line_start_x,
        ...
        for i, block in enumerate(block_contents):  # 遍历内部的所有标签
            if isinstance(block, bs4.element.NavigableString):  # 这个是普通文字
            ...
            elif block.name in ['strong', 'em', 's', 'a', 'img', 'br', 'code', 'latex_inline']:  # 当绘制的是内联块时
                ...
                elif block.name == 'latex_inline':  # 内联latex公式
                    x, y = self.render_inline_LaTeX(x, y, end_x, line_start_x, font_size, block.contents[0])
                elif block.name == 'img':  # 图片
                    pass
                else:
                    pass
            elif block.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ol', 'ul',
                                'ol_nest', 'ul_nest', 'li', 'blockquote', 'hr', 'pre', 'latex_block']:
                ...
                if block.name == 'li':
                    if list_start is None:
                        ...
                        list_start += 1
                if block.name not in ['hr', 'pre', 'latex_block']:  # 这不是分割线和多行代码块，直接绘制
                    start_list = None
                ...
                if block.name == 'latex_block':
                    LaTeX_surface = self.LaTeX[str(block.contents[0].contents[0])]
