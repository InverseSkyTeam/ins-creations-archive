...
class main:  # 主程序
    ...
    def draw_to_sf(self):
        if self.mode == 'home':
            ...
            self.Level.draw_board(self.main_sf)
...
class Home:
    def __init__(self,main_rect):
        ...
        self.image_bg = Calc.transform(pygame.image.load('image/bg/bg.png'), (main_rect.width, main_rect.height))
        self.image_bg1 = pygame.image.load('image/bg/bg1.png')
        self.image_bg1 = Calc.transform(self.image_bg1,
                                        (Calc.pos(self.image_bg1.get_width(),main_rect.height,self.image_bg1.get_height()),
                                         main_rect.height))
        self.image_button = Calc.transform(pygame.image.load('image/button/button.png'),
    ...
    def modify_rect(self,main_rect):
        ...
        self.image_bg1 = pygame.image.load('image/bg/bg1.png')
        self.image_bg1 = Calc.transform(self.image_bg1,
                                        (Calc.pos(self.image_bg1.get_width(), main_rect.height,
                                                  self.image_bg1.get_height()),
                                         main_rect.height))
        self.image_button = Calc.transform(pygame.image.load('image/button/button.png'),
                                           (self.main_rect.width * 250 / 1110, self.main_rect.height * 100 / 619))

    def show_text(self, screen, text, color=(0, 0), pos=(0, 0), size=30):
        screen.blit(pygame.font.Font('方正达利体.ttf', int(size)).render((text), True, color), pos)
...
class Choose_Level:
    ...
    def __init__(self,main_rect):
        self.main_rect = main_rect
        self.image_level_button = Calc.transform(pygame.image.load('image/button/level_button.png'),
                                            (self.main_rect.width * 150 / 1110, self.main_rect.height * 150 / 619))
        self.image_cancel = Calc.transform(pygame.image.load('image/button/返回.png'),
    ...
    def draw(self,screen):
        ...
        for i in range(1, 6):
            ...
            self.show_text(screen,str(i), (127, 127, 127), (
                x + (self.main_rect.width * 150 / 1110 / 2 - self.main_rect.height * 80 / 619 / 4),
                y + (self.main_rect.height * 150 / 619 / 2 - self.main_rect.height * 80 / 619 / 2)),
...
class Level:
    def __init__(self,level,main_rect):
        self.level=level
        self.main_rect=main_rect
        self.image_home = Calc.transform(pygame.image.load('image/button/home.png'),
                                    (Calc.pos(self.main_rect.width,75,1200), Calc.pos(self.main_rect.height,75,674)))
        self.image_next = Calc.transform(pygame.image.load('image/button/next.png'),
                                    (Calc.pos(self.main_rect.width,75,1200), Calc.pos(self.main_rect.height,75,674)))
        self.image_again = Calc.transform(pygame.image.load('image/button/again.png'),
                                     (Calc.pos(self.main_rect.width,75,1200), Calc.pos(self.main_rect.height,75,674)))
        self.brush=self.brush = pygame.image.load('image/button/刷子.png')
    ...
    def show_text_center(self, screen, text, color=(0, 0), ct=(0, 0), size=30):
        word=pygame.font.Font('方正达利体.ttf', int(size)).render((text), True, color)
    ...
    def draw_ans_board(self,screen):
        self.show_text(screen, "目标图像:", (255, 255, 255), (Calc.pos(self.main_rect.width,710,1200),
                       ...
                       Calc.pos(self.main_rect.height,40,619))
        target_board = Board(len(self.target), len(self.target[0]), self.paint_lie, self.paint_hang)
        target_board.board = np.array(self.target)
        target_board.draw()
        ...
        screen.blit(Calc.transform1(pygame.surfarray.make_surface(target_board.sf),
    ...
    def draw_board(self,screen):
        ...
        screen.blit(
            Calc.transform1(pygame.surfarray.make_surface(self.board.sf), (board_rect.width, board_rect.height)),
        ...
        if '竖着的刷子':
            ...
            for i in range(1, len(self.target) + 1):
                if '给刷子上色':
                    bh_np = pygame.surfarray.array3d(brush)
                    bh_np[np.all(bh_np == [0x99, 0x66, 0xff], axis=-1)] = self.paint_hang[i - 1][0]
                    brush1 = pygame.surfarray.make_surface(bh_np)
                    brush1.set_colorkey((0, 0, 0))
                br_y = (self.board_bg_rect.y + (self.board_bg_rect.height / 2 - board_rect.height / 2) +
                        (one_gezi.height / 2 - brush_rect.height / 2)) + \
                       (i - 1) * one_gezi.height
                screen.blit(brush1, (br_x, br_y))
                if pygame.mouse.get_pressed(3)[0] and \
                        pygame.Rect(br_x + self.main_rect.x, br_y + self.main_rect.y, brush_rect.width, brush_rect.height) \
        ...
        if "横着的刷子":
            ...
            for i in range(len(self.target) + 1, len(self.target) + len(self.target[0]) + 1):
                if "给刷子上色":
                    ...
                    brush1 = pygame.surfarray.make_surface(bh_np)
                    brush1.set_colorkey((0, 0, 0))
                br_x = self.board_bg_rect.x + (self.board_bg_rect.width / 2 - board_rect.width / 2) + \
                       (one_gezi.width / 2 - brush_rect.width / 2) + \
                       (i - 1 - len(self.target)) * one_gezi.width
                screen.blit(brush1, (br_x, br_y))
                if pygame.mouse.get_pressed(3)[0] and \
                        pygame.Rect(br_x + self.main_rect.x, br_y + self.main_rect.y, brush.get_width(), brush.get_height()) \
                                .collidepoint(pygame.mouse.get_pos()):
                    self.board.draw_color(i)
...
class AC_Animation:#AC动画
    def __init__(self,main_rect):
        self.main_rect=main_rect
        self.image_AC = pygame.image.load('image/other/congratulation.png')
...
class FadeIn_Animation:#渐显动画
    ...
    def draw(self,screen,t):
        bg_alsf = pygame.Surface((self.main_rect.width, self.main_rect.height), pygame.SRCALPHA)
        bg_alsf.fill((29, 83, 0xbb))
        bg_alsf.set_alpha(t)
        screen.blit(bg_alsf, (0, 0))

class Board:
    def __init__(self, width, height, paint_lie, paint_hang, bg=np.array([0x3b, 0xc4, 0xb1])):
        self.width = width
        self.height = height
        self.sf = np.zeros((351, 351, 3))
        self.sf[:, :] = bg
        interval_x = int(350 / self.width)  # 计算x轴间隔
        interval_y = int(350 / self.height)  # 计算y轴间隔
        self.sf[::interval_x, :] = [0x52, 0x71, 0xff]  # 设置x轴上的条纹
        self.sf[:, ::interval_y] = [0x52, 0x71, 0xff]  # 设置y轴上的条纹
        ...
        for i in range(self.height):
            for j in range(self.width):
                if self.board[i][j][0] != -1:
                    pos=Calc.create_pos(i*int(350 / self.width),(i+1)*int(350 / self.width),j*int(350 / self.height),(j+1)*int(350 / self.height))
                    self.sf[pos[:,1],pos[:,0]]=self.board[i][j]
