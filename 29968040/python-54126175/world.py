...
class World:
    ...
    def can_render_chunk(self, chunk_position, player_chunk):
        return self.player.check_in_frustum(np.array(chunk_position, dtype=np.int32)) and math.dist(player_chunk, chunk_position) <= self.options.RENDER_DISTANCE
    ...
    def draw(self, screen):
        ...
        for render_chunk in self.visible_chunks:
            render_chunk.draw(screen, z_buffer, final_matrix, daylight_multiplier, light_buffer)
        light_mix.light_mix(pygame.surfarray.pixels3d(screen), light_buffer)
