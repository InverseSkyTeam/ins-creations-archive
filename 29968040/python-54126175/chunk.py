...
class Chunk:
    ...
    def update_mesh(self):
        ...
        self.model.lights = np.array(subchunk_lights, dtype=np.float64)
        self.model.lights_1_5 = self.model.lights * 1.5
        self.model.lights_1_25 = self.model.lights * 1.25
        self.model.skylights = np.array(subchunk_skylights, dtype=np.float64)
        self.model.shading = np.array(subchunk_shading, dtype=np.float64)

    def draw(self, screen, zbuffer, matrix, daylight_multiplier, light_buffer):
        if self.model.indices is None or not len(self.model.indices):
            return
        render_cube(self.model, screen, zbuffer, matrix, daylight_multiplier, light_buffer)
