...
class Chunk:
    def __init__(self, world, chunk_position, init_blocks=True):
        ...
        x_w = int(CHUNK_WIDTH / subchunk.SUBCHUNK_WIDTH)
        y_h = int(CHUNK_HEIGHT / subchunk.SUBCHUNK_HEIGHT)
        z_l = int(CHUNK_LENGTH / subchunk.SUBCHUNK_LENGTH)
        for x in range(x_w):
            for y in range(y_h):
                for z in range(z_l):
    ...
    def update_mesh(self):
        ...
        vertices_num = 0
        for _subchunk in self.subchunks.values():
            # print(len(_subchunk.indices), _subchunk.subchunk_position)
        ...
        if len(subchunk_indices) == 0:
            self.model.vertices = np.array([], dtype=np.float64)
            self.model.indices = np.array([], dtype=np.uint32)
            self.model.uv_index = np.array([], dtype=np.uint32)
            self.model.shading = np.array([], dtype=np.float64)
            self.model.lights = np.array([], dtype=np.float64)
            self.model.lights_1_5 = np.array([], dtype=np.float64)
            self.model.lights_1_25 = np.array([], dtype=np.float64)
            self.model.skylights = np.array([], dtype=np.float64)
            return
        self.model.vertices = np.concatenate(subchunk_vertices, axis=0).astype(np.float64)
        self.model.indices = np.concatenate(subchunk_indices, axis=0).astype(np.uint32)
