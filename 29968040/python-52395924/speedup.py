...
def cross_product(a0, a1, b2, bc_c, a2b1, a2b0):
    x = a1 * b2 - a2b1
    y = a2b0 - a0 * b2
    return float(x)/bc_c, float(y)/bc_c
...
def render_clip_face(screen, zbuffer, ptsa, ptsb, ptsc, norms, i, uva, uvb, uvc,
                     clip_a, clip_b, clip_c, width, height, texture_array, O2, norma, normb, normc):
    ...
    for j in prange(minx, maxx):
        ...
        for k in prange(miny, maxy):
            ...
            bc = (bc[0] / ptsa, bc[1] / ptsb, bc[2] / ptsc)
            bc_clip_sum = (bc[0] + bc[1] + bc[2])
            bc_clip = (bc[0] / bc_clip_sum,
                       bc[1] / bc_clip_sum,
                       bc[2] / bc_clip_sum)
            ...
            screen[j, k] = min(int(color[0] * intensity), 255), min(int(color[1] * intensity), 255), min(int(color[2] * intensity), 255)


@jit(nopython=True, cache=True, fastmath=True)
def generate_faces_new(indices, uv_indices, norm_indices, pts, uv_triangle,
