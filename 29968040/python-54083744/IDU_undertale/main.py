...
from .basic import *
from .sight import generate_surface
...
screen = pygame.Surface((SC_WIDTH,SC_HEIGHT))
...
data = [
    ...
    *([{1:1003,2:1003,3:1003,4:1003,5:1003,6:1009,7:1009,8:1009,9:1008,10:1008,}[random.randint(1,10)] for i in range(100)] for line in range(179)),
]
            ...
            if 0 <= ex < SC_WIDTH and 0 <= ey <= SC_HEIGHT and distance <= self.player.chooserange + ichooserange:
                ...
                if self.highbid:
                    ...
                    pygame.draw.line(screen, (0,0,0), (x_,y_),
                                     (x_ + BLOCK_SIZE_3in5, y_), 1)
                    pygame.draw.line(screen, (88,255,188), (x_,y_),
                                     (x_ + BLOCK_SIZE_3in5 * (1-self.dig_progress.part), y_), 1)
                    if mousestate[1]:
...
sightbg = generate_surface(SC_WIDTH,SC_HEIGHT,150,50)
sightbg.set_alpha(0)
...
def next(pygame_event, mouse_pos):
    global invent_choose, bag_open, furnace_open, furnace_is_burning, single_burn_time, furnace_burn_time, mousecard
    for event in pygame_event:
        if event.type == pygame.MOUSEMOTION:
            mousestate['pos'] = mouse_pos
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if ek == ord('b'):  # TODO: 为兼容考虑，这里由 e 改成 b
                record.new('背包！')
                if bag_open:
                    bag_open = False
                    pushout_craft()
