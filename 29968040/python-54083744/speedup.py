...
def render_clip_face(screen, zbuffer, ptsa, ptsb, ptsc, uva, uvb, uvc, width, height, texture_array, O2):
...
def generate_faces_flat(indices, uv_indices, pts, uv_triangle, uv_index, width, height,
                        screen, texture_arrays, z_buffer, O2):
    ...
    for i in prange(length):
        ...
        uva = uv_triangle[uv_indices[i % 12, 0], 0], 0  # uv_triangle[uv_indices[i % 12, 0], 1]
        uvb = uv_triangle[uv_indices[i % 12, 1], 0], uv_triangle[uv_indices[i % 12, 1], 1]
        uvc = uv_triangle[uv_indices[i % 12, 2], 0], uv_triangle[uv_indices[i % 12, 2], 1]
        ...
        if nums:  # 透视裁剪
            ...
            for j in range(3):
                ...
                if is_cur_inside != is_pre_inside:
                    ...
                    out_uv[out_vert_num] = lerp_vec(uv_triangle[uv_indices[i % 12, prev_index]],
                                                    uv_triangle[uv_indices[i % 12, curv_index]], ratio)
                    out_vert_num += 1
                if is_cur_inside:
                    out_pts[out_vert_num] = curv
                    out_uv[out_vert_num] = uv_triangle[uv_indices[i % 12, curv_index]]
                    out_vert_num += 1
            if out_vert_num == 3:
                ...
                uvc = out_uv[2, 0], out_uv[2, 1]
                ptsa, ptsb, ptsc = out_pts[0], out_pts[1], out_pts[2]
                render_clip_face(screen, z_buffer, ptsa, ptsb, ptsc, uva, uvb, uvc, width, height, texture_array, O2)
                uva = out_uv[0, 0], out_uv[0, 1]
