...
class Level:
    ...
    def draw_board(self,screen,can_mouse_down):
        ...
        if '竖着的刷子':
            ...
            br_yyy = np.int_(np.arange(0, board_rect.height + 1, board_rect.height / len(self.target)))
        ...
        if "横着的刷子":
            ...
            br_xxx=np.int_(np.arange(0,board_rect.width+1,board_rect.width/len(self.target[0])))
...
class Board:
    def __init__(self, width, height, paint_lie, paint_hang, bg=np.array([0x3b, 0xc4, 0xb1])):
        ...
        self.sf[:, :] = bg
        self.sf[np.int_(np.arange(0,351,350 / self.width)), :] = [0x52, 0x71, 0xff]  # 设置x轴上的条纹
        self.sf[:, np.int_(np.arange(0,351,350 / self.height))] = [0x52, 0x71, 0xff]  # 设置y轴上的条纹
    ...
    def draw(self):
        xx=np.int_(np.arange(0,351,350 / self.width))
        yy=np.int_(np.arange(0,351,350 / self.height))
        for i in range(self.height):
            for j in range(self.width):
                if self.board[i][j][0] != -1:
                    pos=Calc.create_pos(xx[i],xx[i+1],yy[j],yy[j+1])
