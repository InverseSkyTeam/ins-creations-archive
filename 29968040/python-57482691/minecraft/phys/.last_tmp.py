...
class AABB:
    ...
    def __init__(x0: float, y0: float, z0: float, 
     ...
     * @return 返回一个扩大的AABB盒
    '''
    def expand(xa: float, ya: float, za: float):
        _x0: float = self.x0
        _y0: float = self.y0
        _z0: float = self.z0
        _x1: float = self.x1
     ...
     * @return
    '''
    def grow(xa: float, ya: float, za: float):
     ...
     * @return 
    '''
    def clipXCollide(c, xa: float) -> float:
     ...
     * @return
    '''
    def clipYCollide(c, ya: float) -> float:
     ...
     * @return
    '''
    def clipZCollide(c, za: float) -> float:
        if c.x1 <= self.x0 or c.x0 >= self.x1:
            return za
        if c.y1 <= self.y0 or c.y0 >= self.y1:
            return za
     ...
     * @param za
    '''
    def move(xa: float, ya: float, za: float):
        self.x0 += xa
        self.y0 += ya
        self.z0 += za
        self.x1 += xa
