...
public class WorldRenderer implements WorldListener {
	...
	public void updateDirtyChunks(Player player) {
		List dirty = getAllDirtyChunks();
		if (dirty == null)
			return;

		Collections.sort(dirty,
				new DirtyChunkSorter(player, Frustum.getFrustum()));
	...
	public void pick(Player player, Frustum frustum) {
		...
		for (int x = x0; x < x1; x++) {
			...
			for (int y = y0; y < y1; y++) {
				...
				GL11.glPushName(0);
				for (int z = z0; z < z1; z++) {
					Block tile = Block.BLOCKS[this.world.getBlock(x, y, z)];
					if ((tile != null)
							&& (frustum.isVisible(tile.getBlockAABB(x, y, z)))) {
						GL11.glLoadName(z);
						GL11.glPushName(0);
						for (int i = 0; i < 6; i++) {
							GL11.glLoadName(i);
	...
	public void renderHit(HitResult h) {
		...
		GL11.glColor4f(
				...
				1.0F,
				1.0F,
				((float) Math.sin(System.currentTimeMillis() / 100.0D) * 0.2F + 0.4F) * 0.5F);
		t.init();
	...
	public void setDirty(int x0, int y0, int z0, int x1, int y1, int z1) {
		...
		for (int x = x0; x <= x1; x++)
			for (int y = y0; y <= y1; y++)
				for (int z = z0; z <= z1; z++) {
					this.chunks[((x + y * this.xChunks) * this.zChunks + z)]
							.setDirty();
				}
	}
 ...
 * \rd-161348.jar Qualified Name: com.mojang.minecraft.world.LevelRenderer
