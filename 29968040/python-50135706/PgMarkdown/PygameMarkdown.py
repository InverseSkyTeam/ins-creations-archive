...
class MarkdownRenderer:
    ...
    def update(self):
        ...
        if self.render_height > self.h:  # 渲染区域太大，需要滚动条
            x, y, w, h = self.x + self.w, self.y, 5, self.w / self.render_height * self.w
            if self.scrollbar is not None:
                y = max(self.y, min(self.y + self.h - h, self.scrollbar.y))
    ...
    def render(self, blocks, start_x, start_y, end_x, line_start_x,
        ...
        for i, block in enumerate(block_contents):  # 遍历内部的所有标签
            ...
            # 块级标签，行间公式，嵌套列表
                ...
                if i == len(block_contents)-1:  # 最后一块，需要把 margin_bottom 加上
    ...
    def render_to_screen(self, pygame_events, mouse_x, mouse_y, mouse_pressed):
        ...
        for i in self.render_queue_border_screen_temp:
    ...
    def display(self, pygame_events, mouse_x, mouse_y, mouse_pressed):
        ...
        if self.end_update and self.file_data.can_update:
            ...
            tt = thr.Thread(target=self.update, args=())
            tt.start()
