...
def get_latex_url(args):
    ...
    try:
        if is_inline == 'img':
            return BytesIO(requests.get(txt).content), is_inline
...
class Load:
    def __init__(self, lazy_load_latex=True, lazy_load_img=True, use_cache=False):
        self.html = ""
        self.blocks = None
        self.img_url = {}
        ...
        self.lazy_load_latex = lazy_load_latex  # 是否懒加载公式·
        self.lazy_load_img = lazy_load_img  # 是否懒加载图片
    ...
    def set_markdown_from_string(self, md_string):
        ...
        '''
        with open('markdown文档缓存.py', 'r', encoding='utf-8') as f:
            output = f.read()
    ...
    def load_latex(self):
        while len(self.now_load_LaTeX):
            ...
            sf = pygame.image.load_extended(get_latex_url((gs, inline))[0])
            if inline == 'img':
                self.img_url[gs] = sf
            else:
                if inline:
    ...
    def parse_markdown_file(self):
        ...
        images_img = os.listdir('images_img') if self.use_cache else None
            ...
            if span.name == 'span':
                if span['class'] == ['math', 'inline']:
                    ...
                    if (gs, True) not in self.now_load_LaTeX and gs not in self.in_line_LaTeX:
                        ...
                        self.in_line_LaTeX[gs] = pygame.font.SysFont('SimHei', 16).render('LaTeX加载中...', True, (125, )*3)
                else:
                    gs = str(span.contents[0])
                    ...
                    if (gs, False) not in self.now_load_LaTeX and gs not in self.LaTeX:
                        ...
                        self.LaTeX[gs] = pygame.font.SysFont('SimHei', 16).render('LaTeX加载中...', True, (125, )*3)
            else:
                src = span['src']
