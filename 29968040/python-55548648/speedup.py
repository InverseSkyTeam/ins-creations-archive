...
def render_face(screen, zbuffer, ptsa, ptsb, ptsc, uva, uvb, uvc, width, height, texture_array):
...
def generate_faces(screen, indices, uv_indices, pts, uv_triangle, texture_array, z_buffer):
    # 使用 z-buffer 算法绘制三角形
    ...
    for i in prange(indices.shape[0]):
        ...
        if nums:  # 透视裁剪
            ...
            if out_vert_num == 3:
                ...
                uvc = out_uv[2, 0], out_uv[2, 1]
                ptsa, ptsb, ptsc = out_pts[0], out_pts[1], out_pts[2]
                render_face(screen, z_buffer, ptsa, ptsb, ptsc, uva, uvb, uvc, width, height, texture_array)
                uva = out_uv[0, 0], out_uv[0, 1]
