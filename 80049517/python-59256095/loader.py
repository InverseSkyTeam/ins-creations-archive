"""
    ...
    使用方法：
    看示例作品得了（
    作品链接：https://code.xueersi.com/ide/code/59256095
"""
...
runtime_url = "https://livefile.xesimg.com/programme/python_assets/0f53697bdcecfb97b99ac8aa9d9a9e13.zip"
runtime_ver = "3.12.5"
packages_name = None
chunk_size = 2048
...
def _load_runtime():
    ...
    _download_file(
        runtime_url, f"../UniversalRuntime/runtimes/downloads/{runtime_ver}.zip"
    )
    print(f"Python {runtime_ver} 下载完毕！")

    with zipfile.ZipFile(
        f"../UniversalRuntime/runtimes/downloads/{runtime_ver}.zip"
    ) as zip:
...
def _load_packages(packages_url: str):
    ...
    print("正在下载依赖库...")
    _download_file(
        packages_url, f"../UniversalRuntime/packages/downloads/{packages_name}.zip"
    )
    print("依赖库下载完毕！")

    with zipfile.ZipFile(
        f"../UniversalRuntime/packages/downloads/{packages_name}.zip"
    ) as zip:
...
def _setup_runtime():
    ...
    with open(
        f"../UniversalRuntime/runtimes/{runtime_ver}/python{short_ver_num}._pth", "w"
    ) as file:
        ...
        if packages_name:
            data = data + "\n".join(
                [
                    ...
                    os.path.abspath(
                        f"../UniversalRuntime/packages/{packages_name}/site-packages"
                    ),
                ]
            )
        file.write(data)
...
def run(windowed=False):
    ...
    if windowed:
        ...
        os.system(" ".join(["start", program, "./main.py"]))
    else:
        program = os.path.normpath(
            f"../../UniversalRuntime/runtimes/{runtime_ver}/python.ex" + "e"
        )
        os.system(" ".join([program, "./main.py"]))
