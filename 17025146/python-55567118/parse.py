            ...
            if item.pos < len(item.product.body):
                for char in terminal:
                    if (state.status, char) in goto_table:
                        ...
                        if (state.status, char) not in action_table or action_table[(state.status, char)][0] == ("SHIFT", goto_state):
                            action_table[(state.status, char)] = [("SHIFT", goto_state)]
                        else:
                            action_table[(state.status, char)].append(("SHIFT", goto_state))
                ...
                if (state.status, item.follow) not in action_table:
                    ...
                    action_table[(state.status, item.follow)].append(("REDUCE", item.product))
            elif item.follow == "$":
...
class Parser:
    ...
    def compile(self):
        ...
        self.action_table = make_action_table(self.terminals, self.goto_table, state_list)
    ...
    def parse(self):
        ...
        branches = [([0], [], token, token.type, self.lexer)]
        accepts = []
        goto_table = self.goto_table
        action_table = self.action_table
        while branches:
            new_branches = []
            for (state_stack, value_stack, token, tp, lexer) in branches:
                state = state_stack[-1]
                if (state, tp) in action_table:
                    actions = action_table[(state, tp)]
                elif (state, tp) in goto_table:
                    state_stack.append(goto_table[(state, tp)])
                    tp = token.type
                    new_branches.append((state_stack, value_stack, token, tp, lexer))
                    continue
                elif (state, "ε") in action_table:
                    actions = action_table[(state, "ε")]
                else:
                    self.err()
                    break
                for action, arg in actions:
                    new_state_stack, new_value_stack, new_lexer = copy.deepcopy(state_stack), copy.deepcopy(value_stack), copy.deepcopy(lexer)
                    if action == "SHIFT":
                        new_state_stack.append(arg)
                        new_value_stack.append(token)
                        token = new_lexer.lex()
                        tp = token.type
                        new_branches.append((new_state_stack, new_value_stack, token, tp, new_lexer))
                    elif action == "REDUCE":
                        num = len(arg.body)
                        for _ in range(num):
                            new_state_stack.pop()
                        if arg.function:
                            args = [new_value_stack.pop() for _ in range(num)][::-1]
                            res = arg.function(args)
                            new_value_stack.append(res)
                        tp = arg.name
                        new_branches.append((new_state_stack, new_value_stack, token, tp, new_lexer))
                    elif action == "ACCEPT":
                        accepts.append(new_value_stack.pop())
