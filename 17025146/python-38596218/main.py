...
class ScopeSymbolTable():
    ...
    def init(self):
        ...
        self.define(VarSymbol("TIME",self.query("real")))
...
expr = """
println('
这是Dragonfly 1.8的Pre版本，之所以发一个Pre版本，是因为目前比正式版还少很多功能，但又比上一个版本多很多功能，所以发一个过渡的版本，也可以先看看有哪些新功能。
目前已实现的功能有：
1、switch-case-default结构
这个也是呼声比较高的一个功能，现在实现了，不过这个的case后面不仅可以跟常量表达式，也可以跟变量表达式，这是与传统不同的地方
示例如下：
let a = 404
switch a{
    case 200:
...
2、使用func来声明函数字面量
注意这里是func不是fun，这是为了区别语句和表达式，这个相当于JS的function，和匿名函数差不多。
示例：
let a = func(value:int){
    println(value + 1)
}
a(1) // 2
在目前为止，let a = func(...){...}与fun a(...){...}等价

3、闭包和函数一等公民
这是一个比较高级的特性了，可以带来很多便利，闭包不是很好解释，用一个比较经典的例子：计数器来演示
fun main(v:int){
...
4、loop无条件循环
这是和Rust学的，这个很简单，相当于while 1和while true的功能，只是一个语法糖
...
println(a) // 10
当然，这也是和Rust学的，因为我觉得优雅，所以就“借”过来了

6、函数的缺省参数
这一点上必须吐槽Rust，Rust的变量个数只能是固定的，要可变的就得写复杂而难看的宏，非常不爽
Dragonfly的缺省参数和Python差不多
...
add(6) // 7
如果
fun add(a:int = 1,b:int)的形式就会报错

7、列表的单个元素修改
这个好说，几乎所有语言都大同小异的，而且支持多维列表
let a = [[1,2,3],[2,3,4]]
a[0][1] = 3
println(a) // [[1,3,3],[2,3,4]]



虽然功能看着一大堆，但是没有达到1.8的水平，1.8还会加：
1、变量的解构
let a,b,c = 1,2,3 // 可以自动推导
let a:int,b:str,c:real = 1,"1",1.0 // 可以显式声明
let a,b,c = [1,2,3] // 可以解包数组

2、函数重载
不详细说明了，应该都知道

3、更多内置函数
预计有toreal,tobool,type等

4、模块化的支持
使用import xxx as xxx的形式来导入，使用xxx.xxx的形式来使用模块中的函数和变量

5、debug的支持
使用debugger关键字来停止程序，可以查看CallStack和符号表等等

以下功能为没想好，是否加入1.8待定的功能：
1、函数的pack和unpack
这个是跟Python学的语法糖，没想好语法该怎么样，实现难度倒是不大
2、AST绘图器
感觉在2.0之前意义不是很大
3、Map类型
就相当于Python的dict和JS的Object
')
"""
# 词法分析
lexer = Lexer(expr)
