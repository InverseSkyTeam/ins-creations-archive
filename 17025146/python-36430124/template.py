...
if fhash in dlist:
    ...
    cdn = CloudCDN()
    cdn.saveFile4Hash("zip.zip",fhash,'zip',1)
    unzip_file('./zip.zip',f"../{fhash}")

s = platform.platform(aliased=True,terse=True)
if "Windows" in s:
    command = f'"{sys.executable}" ../{fhash}/main.py'
    os.system(command)
else:
    pythonpath = "/".join(sys.executable.split("/")[:-1])
    pkgpath = '"' + str(pythonpath + "/Lib/site-packages") + '"'
    tem = """import sys
sys.path.insert(0,$$$pkgpath$$$)"""
    tem = tem.replace("$$$pkgpath$$$",pkgpath)
    with open(f"../{fhash}/main.py","r") as f:
        maintext = f.read()
    with open(f"../{fhash}/main.py","w") as f:
        f.write(tem + "\n" + maintext)
    command = f'"{sys.executable}" ../{fhash}/main.py'
    os.system(command)
