...
class ReserveWord(Enum):
    ...
    # 结构体
    structR = Token(TokenType.STRUCT,"struct") # 结构体
    enumR = Token(TokenType.ENUM,"enum") # 枚举类
    implenR = Token(TokenType.IMPLEN,"implen") # 实现方法
    extendR = Token(TokenType.EXTEND,"extend") # 继承
    # 表达式运算
    andR = Token(TokenType.AND,"and") # 与运算
    orR = Token(TokenType.OR,"or") # 或运算
    notR = Token(TokenType.NOT,"not") # 非运算
    # 一元运算
    newR = Token(TokenType.NEW,"new") # 创建新结构体
    # 内置常量
    trueR = Token(TokenType.BOOL,"true") # bool-真值
    falseR = Token(TokenType.BOOL,"false") # bool-假值
...
class UnaryOperate(AST):
    def __init__(self,operate,expr):
        ...
        self.expr = expr
# new运算符
class NewOperate(AST):
    def __init__(self,name,attr):
        self.name = name
        self.attr = attr
# 引用运算符
class RefOperate(AST):
    def __init__(self,name):
        self.name = name
# 解引用运算符
class UnrefOperate(AST):
    def __init__(self,expr):
        self.expr = expr
# 点运算符
...
class SwitchStatement(AST):
    def __init__(self,var,case,default):
        ...
        self.default = default
# struct语句
class StructStatement(AST):
    def __init__(self,name,attr,extend):
        self.name = name
        self.attr = attr
        self.extend = extend
# implen语句
class ImplenStatement(AST):
...
class Parser():
    ...
    # 获取变量类型
    def vartype(self):
        if self.token._type == SymbolToken.REF:
            self.judg()
            name = self.variable().name
            return VartypeTerm("ref",name)
        node = self.variable().name
        return VartypeTerm("value",node)
    # 变量调用
    def variable(self):
        token = self.token
    ...
    def change(self):
        left = self.method()
        if isinstance(left,(VarTerm,IndexOperate,DotOperate)):
            token = self.token
    ...
    def let_statement(self):
        ...
        if self.token._type == SymbolToken.COLON:
            ...
            vartype = VarTerm(Token(TokenType.VARIABLE,"auto"))
        token = self.token
        self.judg(SymbolToken.ASSIGN)
        right = self.expr()
    ...
    def const_statement(self):
        ...
        if self.token._type == SymbolToken.COLON:
            ...
            constype = VarTerm(Token(TokenType.VARIABLE,"auto"))
        token = self.token
        self.judg(SymbolToken.ASSIGN)
        right = self.expr()
    ...
    def struct_statement(self):
        self.judg(TokenType.STRUCT)
        name = self.variable()
        extends = []
        if self.token._type == TokenType.EXTEND:
            self.judg()
            extend = self.variable()
            extends = [extend]
            while self.token._type == SymbolToken.COMMA:
                self.judg()
                extend = self.variable()
        ...
        if self.token._type == SymbolToken.RPARAN:
            self.judg()
            attr = {}
            return StructStatement(name,attr,extends)
        atname1 = self.variable().name
        self.judg(SymbolToken.COLON)
        tp1 = self.vartype()
        attr[atname1] = tp1
        while self.token._type == SymbolToken.COMMA:
        ...
        return StructStatement(name,attr,extends)
    # 绑定方法
    def implen_statement(self):
        self.judg(TokenType.IMPLEN)
        name = self.variable()
        self.judg(SymbolToken.LPARAN)
        method = []
        while self.token._type == SymbolToken.ENDL:
        ...
        if self.token._type == SymbolToken.RPARAN:
            method = []
            return ImplenStatement(name,method)
        while self.token._type == TokenType.FUN:
            method.append(self.fun_statement())
            while self.token._type == SymbolToken.ENDL:
                self.judg()
        self.judg(SymbolToken.RPARAN)
        return ImplenStatement(name, method)
    ...
    def new(self):
        ...
        while self.token._type == SymbolToken.ENDL:
            self.judg()
        self.judg(SymbolToken.RPARAN)
        return NewOperate(name,attr)

    def ref(self):
        self.judg(SymbolToken.REF)
        name = self.variable()
        return RefOperate(name)

    def unref(self):
        self.judg(SymbolToken.MUL)
        expr = self.expr()
        return UnrefOperate(expr)
    # 表达式计算
    # 获取一个值
    def factor(self):
        token = self.token
        if token._type in (SymbolToken.ADD,SymbolToken.SUB,TokenType.NOT):
            ...
            return node
        elif token._type == TokenType.NEW:
            node = self.new()
            return node
        elif token._type == SymbolToken.REF:
            node = self.ref()
            return node
        elif token._type == SymbolToken.MUL:
            node = self.unref()
            return node
...
class TypeSymbol(Symbol): # struct、enum
    def __init__(self,name,attr,stype):
        self.name = name
        self.attr = attr
        self.stype = stype
        self.method = {}
...
class ScopeSymbolTable():
    def __init__(self,scopename = None,scopelv = None,enclosing = None):
        ...
        self.builtintypes = [
            "int","real","bool","str","nulltype","array","function","module","map","struct"
        ]
...
class SemanticAnalyzer(Visitor):
    ...
    def visit_AssignStatement(self,node):
        ...
        if tpname == "auto":
            ...
            if valuetp == "function":
                ...
                self.table.define(RefSymbol(valuetp,varname))
            else:
                v = self.table.query(valuetp)
                if v.name == "struct":
                    self.table.define(TypeSymbol(varname,self.table.query(node.right.name).attr,"struct"))
                else:
                    varsymbol = VarSymbol(varname, v)
            ...
            symbol = self.table.query(tpname) if node.vartype.tp != "ref" else RefSymbol("&" + node.vartype.name,None)
            if not symbol:self.error("不存在的类型")
            if isinstance(symbol,BuiltinTypeSymbol):
                ...
                tp = self.visit(node.right)
                if tp != "struct":self.error("类型错误")
                tpsymbol = TypeSymbol(varname,symbol.attr,"struct")
                self.table.define(tpsymbol)
            elif isinstance(symbol,RefSymbol):
                varsymbol = RefSymbol(symbol.stype,varname)
                self.table.define(varsymbol)
                tp = self.visit(node.right)
                if symbol.stype != tp:
                    self.error("类型错误")
    ...
    def visit_ChangeStatement(self, node):
        ...
        if name == "this":
            symbol = VarSymbol("this",self.table.query("map"))
        if symbol:
            if isinstance(symbol,ConstSymbol):
                ...
                if symbol.stype.name != tp:
                    if not isinstance(node.left,(IndexOperate,DotOperate)):
    ...
    def visit_NewOperate(self,node):
        attr = node.attr
        newname = node.name.name
        struct = self.table.query(newname)
        if not struct:self.error("不存在的结构体")
        newattr = struct.attr
        if len(attr.keys()) != len(newattr.keys()):self.error("结构体初始化值不一致")
        for name in newattr:
            tp = newattr[name].name
            attp = self.visit(attr[name])
            if tp != attp:
                self.error("结构体类型错误")
        return newname
    ...
    def visit_RefOperate(self,node):
        name = node.name.name
        symbol = self.table.query(name)
        if not symbol:self.error("空引用")
        return "&" + symbol.stype.name

    def visit_UnrefOperate(self,node):
        tp = self.visit(node.expr)
        if tp[0] != "&":self.error("不能解引用非引用类型")
        return tp[1:]
    ...
    def visit_StructStatement(self,node):
        if node.extend is not None:
            for extend in node.extend:
                tp = self.visit(extend)
                if tp != "struct":self.error("struct不能继承非struct方法")
        if node.extend is not None:
            attr = {}
            for extend in node.extend[::-1]:# 从左到右依次查找,也就是从右到左merge回来
                newattr = self.table.query(extend.name).attr
                attr.update(newattr)
        attr.update(node.attr)
        symbol = TypeSymbol(node.name.name,attr,"struct")
        self.table.define(symbol)

    def visit_ImplenStatement(self,node):
        name = node.name.name
        symbol = self.table.query(name)
        if not symbol or not isinstance(symbol,TypeSymbol):
            self.error("不能绑定方法")
        methods = {}
        for method in node.method:
            self.visit(method)
            methods[method.name] = FunSymbol(method.name,method.params)
    ...
    def visit_FunctionStatement(self,node):
        ...
        self.table.define(VarSymbol("this",self.table.query("map")))
...
class TypeSym(Sym):
    def __init__(self,name,attr,extend):
        ...
        self.attr = attr
        self.method = {}
        self.proto = extend # 原型
class StructSym(Sym):
    def __init__(self,attr,proto):
        self.value = attr
        self.proto = proto # 事实上,每个对象的原型都指向new出它的类型
class RefSym(Sym):
    def __init__(self,name):
        self.name = name
def VarSym(value):
...
class Interpreter(Visitor):
    ...
    def visit_DotOperate(self, node):
        ...
        while True:
            if isinstance(left,MapSym):
                self.this = left
                return left.value[right]
            elif isinstance(left,StructSym):
                if right in left.value:
                    self.this = left
                    return left.value[right]
                
                # 这玩意儿迭代不好写，于是乎用递归
                def get_method(proto,name):
                    if not proto:return None
                    for i in proto:
                        res = get_method(i.proto,name)
                        if res:return res
                        if name in i.method:
                ...
                ar = self.stack.peek()
                # 深度优先
                # 例:Xiaoming(Student(People),Boy(Person))
                # 查找次序：Xiaoming -> Student -> People -> Boy -> Person -> __proto__ -> null
                method = get_method(left.proto,right)
                self.this = left
                return method
                
            elif isinstance(left,RefSym):
                name = left.name
                ar = self.stack.peek()
    ...
    def visit_NewOperate(self,node):
        ...
        attr = {}
        for n in node.attr:
            attr[n] = self.visit(node.attr[n])
        return StructSym(attr,[ar[name]])
    ...
    def visit_UnrefOperate(self,node):
        name = self.visit(node.expr).name
        ar = self.stack.peek()
        if name in ar.members:
            return ar[name]
    ...
    def visit_StructStatement(self,node):
        ar = self.stack.peek()
        attr = {}
        extend = [TypeSym("__proto__",{},None)]
        if node.extend:
            for ext in node.extend[::-1]: # 同理，从右至左merge属性，并添加到原型列表
                attr.update(self.visit(ext).attr)
                extend.append(self.visit(ext))
        extend = extend[::-1] # 原型从左至右查找
        attr.update(node.attr)
        ar[node.name.name] = TypeSym(node.name.name,attr,extend)

    def visit_ImplenStatement(self,node):
        ar = self.stack.peek()
        struct = ar[node.name.name]
        methods = {}
        for method in node.method:
            symbol = FunctionSym(method.block,ar.members)
            for param in method.params[1:]:
                pname = param.name.name
                dvalue = param.default
                symbol.params[pname] = dvalue
            methods[method.name] = symbol
    ...
    def visit_ChangeStatement(self,node):
        if isinstance(node.left,VarTerm):
            ...
            if varname in ar.members:
                ...
                self.error("未定义的变量")
        elif isinstance(node.left,(IndexOperate,DotOperate)):
            v = self.visit(node.left)
            v.value = self.visit(node.right).value
            v.str = self.visit(node.right).str
    ...
    def visit_IfStatement(self,node):
        if self.visit(node.condition).value:
            prear = self.stack.peek()
            ar = ActivationRecord("if","if",prear.level + 1)
            ar.closure = {**ar.members,**ar.closure}
            self.stack.push(ar)
...
implen Student{
    fun do(this:Student,do:str){
        println(this.age,"岁的",this.name,"在",do)
    }
}
struct Boy{
    age:int
...
implen Boy{
    fun do(this:Boy,do:str){
        println(this.age,"的一个男孩在",do)
    }
}
struct Xiaoming extend Student,Boy{}
...
builder.visit(tree)
# 解释器
interpreter = Interpreter(tree,1)
