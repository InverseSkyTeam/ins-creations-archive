...
def make_goto_table(terminal, nonterminal):
    goto_table = {}
    I0 = State(state_closure([Item(Product("S'", ["S"]), 0, "$")], terminal, nonterminal))
        ...
        for item in state.items:
            if item.pos < len(item.product.body):
                ...
                if (state.status, item.follow) in action_table:
                    ...
                    if action[0] == "SHIFT":
                        shift_reduce.append((state.status, item.follow, shifts[(state.status, item.follow), item, action, ("REDUCE", (item.product.name, len(item.product.body), item.product.function))]))
                        continue
                action_table[(state.status, item.follow)] = ("REDUCE", (item.product.name, len(item.product.body), item.product.function))
...
"""
    ...
    def generate_parser(self, action):
        code = """
class Parser:
"""
            ...
            for b in pg.bodies:
                ...
                for m in range(len(b)):
                    if b[m].isdigit():
                        ...
                        rets.append(f"args[{m}]")
                fn = f"    def anomynous_{id_}(self, args):\n        return [{','.join(rets)}]\n"
                code += fn
                patterns[id_].append(Product(id_, b, f"anomynous_{id_}"))
        ...
        self.goto_table, state_list = make_goto_table(lex_rules, patterns)
    ...
    def __init__(self, lexer):
        ...
        self.goto_table = {self.goto_table}
