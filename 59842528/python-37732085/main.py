...
RESERVED_KEYWORDS = {  # 保留字字典
    ...
    'declare' : Token(FUNC,'declare'),
    'return' : Token(RETURN,'declare'),
}
...
class FuncCall(AST): #函数调用节点
    def __init__(self, proc_name, actual_params, token):
        self.func_name = proc_name  #过程名称
...
class Parser:  #语法分析器
    ...
    def func_statement():
        ...
        if self.current_token.type == TokenType.LPAREN:  # 如果遇到左括号
            ...
            params = self.parameter_list()  # 获取参数列表
...
class Interpreter(NodeVisitor):
    ...
    def visit_Change(self,node):
        ...
        if str(type(value)) == tp or vartype=='changeable':
    ...
    def visit_Srin(self,node):
        ...
        for node in node.params:
            val =vallist[i]
            name = node.name
            if self.VAR_RALATION_TYPE[name] == 'int':
                valnode = Num(Token(INT,int(val)))
            elif self.VAR_RALATION_TYPE[name] == 'float':
                valnode = Float(Token(FLOAT,float(val)))
            elif self.VAR_RALATION_TYPE[name] == 'bool':
                valnode = Bool(Token(BOOL,bool(val)))
            elif self.VAR_RALATION_TYPE[name] == 'str':
