...
LMidParen,RMidParen = 'LMidParen','RMidParen' #左右中括号
ARRPOINT = 'ARRPOINT' #'->'
#变、常量类
...
#语句类
IFNODE,WHILENODE,FORNODE='IFNODE','WHILENODE','FORNODE'
...
#传统列表 origin_list:... orlist a:int = 1,2,3;
ORLIST,OPLIST='ORLIST','OPLIST'
#操作性列表 operatabled_list 
# a[0]用于操作所有元素，从a[1]开始，a[1:2]截取  oplist b:int = 1,2,3;
# a[-1]用于操作所有奇数位元素
# a[-2]用于操作所有偶数位元素
...
class ErrorCode():
    def __init__(self):
        self.Undefined = "Error : The var you change is undefined. "
        self.ConstError = "Error : Consted var can't be changed. "
        self.TypeError = "Error : The type of the var is changed. "      
        self.UnexpectedToken = "Error : Unexpected Token. "
        self.SrinError = "Error : The Srin statement has the wrong params. "

def serror(messege):
    print(messege)
    raise messege#sys.exit()
...
class Lexer():      
    ...
    def error(self):  # 定义提示错误的方法
        msg = f"Lexer error on '{self.current_char}' line: {self.line} column: {self.column}"
...
class For(AST):  
    def __init__(self, varname,tp,params,then):
...
class Parser:  #语法分析器
    ...
    def eat(self,token_type):
        if (self.current_token.type==token_type):
            ...
            self.error(ErrorCode.UnexpectedToken,self.current_token)
    ...
    def for_statement(self): 
        ...
        self.eat(RBigParen) 
        node = For(varname,vartype,p,then_root)
        return node  
    ...
    def statement(self):
        ...
        if self.current_token.type == VAR:
            ...
            node = self.tostr()
        elif self.current_token.type == TOORD:
            node = self.toord()
        elif self.current_token.type == TOCHR:
            node = self.tochr()
        ...
        elif self.current_token.type == FORNODE:
...
class SemanticAnalyzer(NodeVisitor):
    ...
    def visit_Change(self, node):
        ...
        if symbol:
            ...
            msg = ErrorCode.Undefined + 'The var: \''+str(name)+'\' was undefined in the scope'
            serror(msg)
            '''self.error(error_code=ErrorCode.TypeError,token=node.var_node.token,)'''
    ...
    def visit_Var(self,node):
        ...
        if not symbol:
            msg = ErrorCode.Undefined + 'The var: \''+str(name)+'\' was undefined in the scope'
            serror(msg)  #raise NameError(f'引用了不存在的标识符：{repr(name)}')  # 抛出语义错误
...
class Interpreter(NodeVisitor):
    ...
    def visit_Assign(self, node):  # 访问赋值语句节点
        ...
        if str(type(value)) == tp or var_type == 'changeable':
            ...
            msg=ErrorCode.TypeError+"The Var:\'"+str(var_name)+"\' has the vaule:"+str(value) +" but the type: "+str(var_type)
            serror(msg)
    ...
    def visit_Const(self, node):  
        ...
        if str(type(value)) == tp:
            ...
            msg=ErrorCode.TypeError+"The Consted Var: \'"+str(var_name)+"\' has the vaule:"+str(value) +" but the type: "+str(var_type)
            serror(msg)
    ...
    def visit_Change(self,node):
        ...
        if varname in ar.members:
            ...
            if ar.isconst[varname] == True:
                msg= ErrorCode.ConstError+'The Consted var:\''+str(varname) + '\' can\'t be changed.'
                serror(msg)
            elif str(type(value)) == tp or vartype=='changeable':
                ar[varname] = value
            else:
                msg = ErrorCode.TypeError+"The Var:\'"+str(varname)+"\' has changed the vaule:"+str(value) +" but the type: "+str(vartype)
                serror(msg) #raise ValueError("类型发生改变")
                
        else:
            msg = ErrorCode.Undefined + 'The var: \''+str(varname)+'\' was undefined in the scope'
            serror(msg) #raise ValueError("变量不存在")
        
    def visit_Variable(self, node):  # 变量节点
        var_name = node.name  # 变量名称
        ar = self.stack.peek()  #获取调用栈的栈顶
        var_value = ar.get(var_name)
  
        if var_value is None:  # 变量不存在
            msg = ErrorCode.Undefined + 'The var: \''+str(var_name)+'\' was undefined in the scope'
            serror(msg) 
        else:  
    ...
    def visit_For(self,node):
        name = node.varname.value
        pl = []
        for i in node.p:
            pl.append(self.visit(i))
        ar = self.stack.peek()  #获取调用栈的栈顶
        ar.types[name] = 'int'
        ar.isconst[name] = False
        if(len(pl)<0 or len(pl)>3):
            print("错误的for语句")
        elif(len(pl)==1):
            ar[name] = 0
            for num in range(pl[0]):
                ret = self.visit(node.then)
                ar[name] = ar[name]+1
                if ret == Break:
                    break
            ...
            for num in range(pl[0],pl[1]+1):
                ret = self.visit(node.then)
                if (ar[name]+pl[2]<=pl[1]):
                    ar[name] = ar[name]+pl[2]
                    if ret == Break:
                        break
                    elif ret == Continue:
                        continue
    ...
    def visit_Srin(self,node):
        ...
        i=0
        if len(vallist)!=len(node.params):
            msg = ErrorCode.SrinError + 'The Srin statement has '+str(len(node.params))+' var ,but '+ str(len(vallist))+' was Input'
            serror(msg) 
        for node in node.params:
            val =vallist[i]
            name = node.name
    ...
    def visit_FunCall(self,node):
        ...
        for param_symbol,argument in zip(formal,actual):
            ...
            ar.types[param_symbol.name] = param_symbol.stype.name
            ar.isconst[param_symbol.name] = False
        self.stack.push(ar)
...
import DrawASTtree
drawastimg = DrawASTtree.DrawASTtreeImg(tree)
drawastimg.drawAST()
...
t2=time.time()
print("\nThis File cost "+str(t2-t1)+" second to run.")