...
import c,cpp,py,rain,txt,newlang,js,cfgl
#各种文件的高亮和运行支持
def clear():
    os.system("cls")
def gotoxy(x,y):
...
def edit(lang,save):
    ...
    def print_info():
        ...
        filename=" "*fsize
        print(theme["others"]+theme["selected"]+f" %6s  %s   ln: %4d col: %4d\033[1;0m"%(mode,filename,x,y))
    def pyins(ch):
    ...
    def do(cmd):
        ...
        if head=="help":
            clear()
            print(open(__file__[:__file__.rfind('\\')]+"\\help.txt",'r',encoding="utf-8").read())
            scrolled=True
            getch()
        elif head=="size":
            try:
...
''')
    ...
    def update_line(i,_i):
        gotoxy(_i-begin+1,1)
        print(theme["background"]+" "*os.get_terminal_size().columns)
        gotoxy(_i-begin+1,1)
        if nu:
            print(theme["background"]+theme["others"]+"%4d | "%(_i+1),flush=True,end="")
        ...
        if scrolled or mode=="SELECT":
            ...
            if mode=="SELECT":
                ...
                for i in range(begin,min(begin+size,len(rr))):
                    ...
                    gotoxy(i-begin+1,1)
                    if nu:
                        print(theme["background"]+theme["others"]+"%4d | "%(i+1),flush=True,end="")
                    for j in range(beginy,min(beginy+sizey,len(rr[i]))):
                        ...
                        if i in gar and gar[i][0]<=j<gar[i][1]:
                            if rr[i][j]:
                                print(theme["background"]+rr[i][j][:-1]+theme["selected"]+rr[i][j][-1],end="")
                        else:
                            '''if rr[i][j]:
                                print("\033[1;0m"+rr[i][j][:-1]+"\033[1;43m"+rr[i][j][-1],end="\033[1;0m")'''
                            print(theme["background"]+rr[i][j],end="\033[1;0m")
                        #time.sleep(0.1)
                    print("\033[1;0m")
                #gotoxy(size+3,1)
                ...
                for i in rr[begin:begin+size]:
                    ...
                    if nu:
                        print(theme["background"]+theme["others"]+"%4d | "%(_i+1),flush=True,end="")
                    print(theme["background"]+"".join(i[beginy:beginy+sizey]))
                    _i+=1
        else:
