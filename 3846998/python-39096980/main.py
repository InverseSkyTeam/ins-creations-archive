                ...
                if self.tk.tp!=RSB:
                    ...
                    while self.tk.tp==COMMA:
                        ...
                        if name=="flush":
                            flush=value
                self.eat(RSB)
            return Print(self.expr(),sep,end,flush)
        if self.match_id("if"):
            self.eat()
            self.eat(LPAREN)
            cond=self.expr()
...
class Interpreter:
    ...
    def visit_VarDecl(self,node:VarDecl):
        values=[new(i) for i in node.types]
        self.scope.buildin(dict(zip(node.names,values)))
    def visit_Block(self,node:Block):
        for i in node.children:
            v=self.visit(i)
            if type(v) in (Break,Continue,Return):
                return v
    def visit_Print(self,node:Print):
        print(self.visit(node.thing),sep=self.visit(node.sep),end=self.visit(node.end),flush=self.visit(node.flush))
    def visit_Var(self,node:Var):
        return self.scope.find(node.name)
    def visit_If(self,node:If):
