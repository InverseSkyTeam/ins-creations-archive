...
class IMemberFunction:
    def __init__(self,obj,func):
        ...
        self.func=func
class IType:
    def __init__(self,parents,cons,attr,attr_visit):
        self.parents=parents
        self.cons=cons
        self.attr=attr
        self.attr_visit=attr_visit
    def append_attr(self,name,value):
        self.attr[name]=value
    def get_attr(self,name):
        return self.attr[name]
class IObject:
    def __init__(self,tp,attr,attr_visit):
        self.tp=tp
        self.attr=attr
        self.attr_visit=attr_visit
    def append_attr(self,name,value):
...
class Parser:
    ...
    def stmt(self):
        ...
        if self.match_id("switch"):
            ...
            while self.match_id("case"):
                self.eat()
                cases.append(self.parse_tuple(self.expr))
        ...
        if self.match_id("throw"):
            ...
            self.tk=tk
            if isassign:
                base=self.parse_tuple(self.parse_lvalue)
                self.eat(ASSIGN)
                target=self.parse_tuple(self.expr)
                if type(base)!=Tuple:
                    base=[base]
                else:
                    base=base.l
                ...
                return self.parse_tuple(self.expr)
    def parse_lvalue(self):
        if self.tk.tp==BAND:
            self.eat()
            return IReference(self.parse_lvalue())
        if self.tk.tp==MUL:
            self.eat()
            return GetValueOp(self.parse_lvalue())
        base=self.eat(ID).v
        indextypes=[]
        indexs=[]
    ...
    def parse_class(self):
        ...
        attr_visit=[]
        while self.tk.tp!=END:
            if self.tk.tp==ID and self.tk.v in ("public","private","protected"):
                attr_visit.append(self.eat().v)
            else:
                attr_visit.append("public")
            attr_def.append(self.stmt())
            if self.need_semicolon:
                self.eat(SEMICOLON)
            self.need_semicolon=True
        self.eat(END)
        return NewType(parents,cons,attr_def,attr_visit)
    def fh(self,v):
        if self.tk.tp==LPAREN:
            self.eat()
            args=[]
...
class Interpreter:
    ...
    def visit_DOTOp(self,node):
        ...
        if type(l)!=IObject:
            ...
            if type(l)==list and node.r=="append":
                return l.append
        if l.attr_visit[node.r]=="public" or (type(node.l)==Var and node.l.name=="this"):
            res=l.attr[node.r]
            if type(res)==IFunction:
                return IMemberFunction(l,res)
            return res
        else:
            throw(InterpreterError("Visited a private or protected attribute"))
    ...
    def visit_NewType(self,node):
        ...
        attr={}
        _i=0
        for i in node.attr_def:
            if type(i)!=VarDecl:
                throw(InterpreterError());
            for j in range(len(i.name)):
                attr[i.name[j]]=self.visit(i.value[j])
                attr_visit[i.name[j]]=node.attr_visit[_i]
            _i+=1
        return IType(parents,cons,attr,attr_visit)
    ...
    def get_all_attrs(self,tp):
        ...
        for k,v in tp.attr.items():
            res[k]=v
        for k,v in tp.attr_visit.items():
            attr_visit[k]=v
        return res,attr_visit
    ...
    def assign(self,node):
        base_value=None
        lvalue=None
        if type(node.base)==GetValueOp:
            base=node.base
            gvocnt=0
            while type(base)==GetValueOp:
                base=base.lvalue
                gvocnt+=1
            lvalue=self.scope.find(base.base)
            for i in range(gvocnt):
    ...
    def visit_NewObject(self,node):
        ...
        return IObject(IType([],None,dict(zip(names,[None for i in range(len(names))])),dict(zip(names,["public" for i in range(len(names))]))),dict(zip(names,values)),dict(zip(names,["public" for i in range(len(names))])))
    ...
    def visit_Foreach(self,node):
        ...
        if type(base) in (list,tuple,str):
            for i in base:
                j=i
                if type(j)==tuple:
                    j=list(j)
    ...
    def visit_LValue(self,node):
        v=self.scope.find(node.base)
        for i in range(len(node.indexs)):
            if node.indextypes[i] in (IndexType.TYPE_ATTR,IndexType.OBJ_ATTR):
                v=v.attr[node.indexs[i]]
            else:
                v=v[self.visit(node.indexs[i])]
        return v
    ...
    def complete_args(self,para,paravalue,args):
        ...
        return res
    def visit_GetValueOp(self,node):
        refobj=self.visit(node.lvalue)
        return self.visit(refobj.lvalue)
    ...
    def visit_Tuple(self,node):
        return tuple(self.visit(i) for i in node.l)
    visit_int=visit_float=visit_str=visit_list=visit_bool=visit_NoneType=visit_IType=visit_IModule=visit_IObject=visit_IReference=visit_NoInitValue=visit_IMemberFunction=lambda self,a:a
        ...
        "Range":NewModule(Parser(Lexer('''
{
    ...
    }
    {
        var begin,end,skip;
        func __iter__(){
            return RangeIter(this.begin,this.end,this.skip);
        }
    }
}
''')).block()
...
4.5、异常处理
    try-except-default
        ...
        except <error type>{//error name应为字符串
            ...
        }
        ...
        default{
...
Interpreter(Parser(Lexer('''
{
    ...
    foreach(i:Range(10,11,2)) println(i);
    foreach(i,j:[(1,2),(3,4)]) println(i,j);
}
...

{
    ...
    cons(){
        this.error_name="IterationError";
        this.error_message="";
    }
    {
        var error_name,error_message;
    }
    class FibIter
    cons(end){
        this.now=0;
    ...
    }
    {
        ...
        func __next__(){
            if(this.now==this.end) return IterationError();
    ...
    }
    {
        var end;
        func __iter__(){
            var ret=FibIter(this.end);
            return ret;
        }
    }
    foreach(i:Fib(100)) println(i);
}
'''