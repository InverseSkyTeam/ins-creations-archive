...
def run(prog: list[tuple], reserve_cnt: int):
    ...
    while pc < len(prog) and prog[pc] != (0,):
        ...
        match prog[pc]:
            ...
            case 9, :  # ret
                if not scope_stack:
                    raise RainError('Return outside function')
                # scope.pop()
            ...
            case 6, attr:  # attr
                base = stack.pop()
                res = base[attr]
                if isinstance(res, dict) and isinstance(res, Func) or callable(res):
            ...
            case 18, p, name, argcnt, reserve_cnt:  # func
                func = Func(argcnt, name, p, scope, reserve_cnt)
                stack.append(func)
            case 19, ks:  # module
                x = stack[-len(ks):]
                del stack[-len(ks):]
