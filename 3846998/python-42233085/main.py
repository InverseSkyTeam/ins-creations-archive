...
def run_code(tree:tuple,scope:Scope=Scope()):
    ...
    def run(tree:tuple):
        ...
        if tp==Const:
            ...
            fn=run(v[0])
            if callable(fn):
                return fn(*[run(i) for i in v[1]])
            if type(fn)==Fn:
                a=run_code(fn.body,Scope(fn.closure,dict(zip(fn.para,[run(i) for i in v[1]]))))
                if a in (None,()):
                    return None
                return a[1]
            elif type(fn)==Type:
                ...
                sc=Scope(cons.closure,dict(zip(cons.para,[run(i) for i in v[1]])))
                sc.define("this",obj)
                a=run_code(cons.body,sc)
                return obj
            elif type(fn)==Method:
                obj,fn=fn.obj,fn.fn
                sc=Scope(fn.closure,dict(zip(fn.para,[run(i) for i in v[1]])))
                sc.define("this",obj)
...
{
    println("Hello,world!
这是新的NewLang，主要是语法的改变（更像integer3了）和实现的简化
