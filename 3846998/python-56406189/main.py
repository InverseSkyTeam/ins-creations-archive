...
def check(prog: list, scope: Scope):
        ...
        match node:
            ...
            case 'attr', base, attr:
                ...
                if isinstance(base, dict) and attr in base:
                    return base[attr]
                if isinstance(base, tuple) and base[0] == 'list' and attr == 'append':
                    return 'func', 'void', base[1]
                raise BTypeError(
            ...
            case 'func', params, ret_tp, body:
                ...
                if context is None:
                    ...
                    old_expect = expected_ret
                    expected_ret = ret
                    stmt(body)
                    expected_ret = old_expect
                    scope = scope.parent
                else:
    ...
    def stmt(node):
        ...
        match node:
            ...
            case 'type', tname, tval:
                scope.define(tname, type_eval(tval, tname))
    ...
    def type_eval(node, basename: str = ""):
        nonlocal scope, expected_ret, context
        match node:
            case 'basic', name:
            ...
            case 'template', name, targs:
                ...
                stred_targs = str(targs)
                tparams, tval, tdata = scope.find_typegen(name)
                if len(targs) != len(tparams):
                    raise BTypeError(f"Invalid number of type arguments: {name}, {targs}")
                if stred_targs in tdata:
                    return tdata[stred_targs]
                scope = Scope(scope)
                scope.types = dict(zip(tparams, targs))
                res = type_eval(tval)
                scope = scope.parent
                tdata[stred_targs] = res
                return res
            case 'complex', ntvpairs:
                ...
                res = {}
                if basename:
                    scope.define_type(basename, res)
...
def run(prog: list, scope: Scope):
    ...
    def expr(node):
        ...
        match node:
            ...
            case 'attr', base, attr:
                b = expr(base)
                if isinstance(b, list) and attr == 'append':
                    return b.append
                v = b[attr]
...
with open("expr.btf", encoding="utf-8") as f:
    code = f.read()
