...
def edit(lang,save):
    ...
    def _ins(ch):
        call(lang_cfg[lang]["ins"],[ch])
    def _del(ch):
        call(lang_cfg[lang]["del"],[ch])
    def _run(ch):
        call(lang_cfg[lang]["run"],[ch])
    def _render(ch):
        call(lang_cfg[lang]["render"],[ch])
    def getall():
        res=[]
...
''')
    ...
    def __map(key:str,_key:list[dict],fn):
        if len(key)==1:
            _key[0][key]=fn
        else:
            if type(_key[0])!=dict:
    ...
    run(parse(open(dire+"config.rain",'r',encoding='utf-8').read()),deepcopy(fns))
    rr=call(lang_cfg[lang]["render"],["\n".join(map("".join,code))],)
    while 1:
        change=False
        if x<begin:
            begin=x
