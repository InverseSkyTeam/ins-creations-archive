...
class Parser:
    ...
    def factor(self):
        ...
        if self.match_id("anonymous_class"):
            ...
            block=self.block()
            return self.fh(NewType(parents,cons,block))
...
class Interpreter:
    ...
    def visit_CallFunc(self,node):
        ...
        if type(func)==type(lambda:0):
            return func(*args)
        if type(func)==IFunction:
            para=func.para
            self.scope=Scope(self.scope,dict(zip(para,args)))
        ...
        if type(func)==IMemberFunction:
            ...
            para=func.para
            self.scope=Scope(self.scope)
            self.scope.var=dict(zip(para,args))
            ret=self.visit_Block(func.code,0)
            self.scope=self.scope.parent
            this_stack.pop()
            if ret!=None:
    ...
    def visit_NoOp(self,node):
        return None
    visit_int=visit_float=visit_str=visit_list=visit_bool=visit_NoneType=visit_IFunction=visit_IType=visit_IModule=visit_IObject=lambda self,a:a
