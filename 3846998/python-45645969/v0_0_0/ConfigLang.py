...
def parse(tokens: list[str]) -> list[tuple]:
    ...
    def parse_expression() -> tuple:
        ...
        while token in priority or token == ')':
            while token == ')':
                if '(' not in stack:
                    break
                eat()
...
def run(ast: tuple | list, scope: Scope) -> Any:
    if isinstance(ast, list):
        ...
        match ast:
            ...
            case 16, groups, body:  # FOR
                names = []
                objs = []
                for name, obj in groups:
                    names.append(name)
                    objs.append(run(obj, scope))
                for i in range(min([len(i) for i in objs])):
                    for n, name in enumerate(names):
                        scope.define(name, objs[n][i])
                    v = run(body, scope)
                    if isinstance(v, tuple) and v[0] == RETURN:
                        return v
                    if isinstance(v, tuple) and v[0] == BREAK:
