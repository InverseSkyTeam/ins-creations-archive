...
def edit(lang,save):
    ...
    def crun():
        ...
        clear()
        if save:
            os.system(f"gcc "+save+" -o "+save[:save.rfind(".")])
            print("\033[1;33m开始运行\033[1;0m")
            os.system(save[:save.rfind(".")])
            print("\033[1;33m运行结束\033[1;0m")
        else:
    ...
    def do_cmd(ch):
        ...
        if ord(ch)==224:
            ...
            if ch in ikey:
                ...
                INSERT_do(ch)
    def do(cmd):
        nonlocal mode,x,y,code,begin,beginy,history,size,sizey,selectx,selecty,nu,save,tab,change,delay,theme,ybefore
        head=cmd.split(" ")[0]
        cmd=cmd[len(head)+1:]
        if head=="help":
            clear()
...
def txtrender(code:str,theme:dict):
    res=list(code)
    rr=[]
    tmp=[]
    for i in res:
        if "\n" in i:
            ...
            tmp.append(i)
    return rr+[tmp]
def crender(code:str,theme:dict):
    op=[
        '+','-','*','/','%',
        '==','!=','>','<','>=','<=',
        '<<','>>','&&','||','&','|','^','~',
