...
{
    func better_print(s,scope){
        for(var i=0;i<len(s);++i){
            if(s[i]=="$"){
                if(i==len(s)-1) throw object{
                ...
                if(s[i+1]=="$"){
                    ...
                    ++i;
                    var varname="";
                    while(s[i]!="$") varname=varname+s[i++];
                    print(scope.find(varname));
                }
            }
            else{
                print(s[i]);
...
{
    var my_print=builtin_modules::Printer::better_print;
    var name="integer3.2.3 user";
    my_print("Hello,$name$!\\n",__scope__);
    println(1*1/1);
}
