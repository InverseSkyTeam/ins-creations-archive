try:
    ...
    def throw(error):
        if type(error)==IObject:
            raise Exception(f"{error.attr['error_name']}:{error.attr['error_message']}")
        raise Exception(f"{type(error).__name__}:{error.msg}.")
    #}
    #词法分析器{
    class Lexer:
    ...
    class Interpreter:
        ...
        visit_tuple=visit_int=visit_float=visit_str=visit_list=visit_bool=visit_NoneType=visit_IType=visit_IModule=visit_IObject=visit_NoInitValue=visit_IMemberFunction=lambda self,a:a
    ...
    '''class Printer:
        ...
        def __call__(self,*text,sep=" ",flush=True,end="\n"):
            ...
            for i in t:
                ...
                self.wordcnt+=1
                if self.wordcnt>=800 and time()-self.starttime<=0.1:
                    sleep(0.2-(time()-self.starttime))
                    self.wordcnt=0
                    self.starttime=time()
                elif self.wordcnt>=800:
                    self.wordcnt=0
                    self.starttime=time()
            print(end,end="")'''
    words=list(keywords)
    printer=lambda *args,sep=" ",flush=True,end="\n":print(*args,sep=sep,flush=flush,end=end)
    def render(code):
        words=list(keywords)
        res=[]
        while code:
            if code[0].isdigit():
                ...
                res.append("\033[1;47;30m"+code[0])
                code=code[1:]
        _res=[]
        tmp=[]
        for i in res:
    ...
    def IDE():
        global Main
        save_path=""
        LINE_SIZE,ROW_SIZE=15,20
        code=[[] for i in range(15)]
        line,row=0,0
        begin_line,begin_row=0,0
        def run_cmd(cmd):
            nonlocal LINE_SIZE,ROW_SIZE
            cmd_list=[
                "insert",
                "size",
            ]
            ...
            if cmd_head=="size":
                _,x,y=cmd.split(" ")
                LINE_SIZE,ROW_SIZE=int(x),int(y)
                print(LINE_SIZE,ROW_SIZE)
        ...
        def addc(ch):
            ...
            if ord(ch)==13:
                ...
                if ch=='}' and code[line][0:4]==[' ',' ',' ',' '] and code[line][-1:]==[' ']:
        ...
        while 1:
            ...
            if row>=max(len(code[line]),1):
                row=len(code[line])
            if line>=begin_line+LINE_SIZE:
                begin_line=line-LINE_SIZE+1
            if line<begin_line:
                begin_line=line
            if row>=begin_row+ROW_SIZE:
                begin_row=row-ROW_SIZE+1
            if row<begin_row:
                begin_row=row
            #print_info(f"{begin_line} {begin_row}",18)
            ...
            for i in range(LINE_SIZE):
                printer(" "*(ROW_SIZE+5))
            gotoxy(1,1)
            printer("\033[1;30m",end="")
            rend_res=render("\n".join(["".join(i) for i in code]))[begin_line:begin_line+LINE_SIZE]
            for i in range(len(rend_res)):
                printer("%3d"%(i+begin_line+1),end=" |\n")
            gotoxy(1,1)
            printer("\033[1;47m",end="")
            for i in range(len(rend_res)):
                gotoxy(i+1,6)
                printer("".join(rend_res[i][begin_row:begin_row+ROW_SIZE-1]))
            gotoxy(line-begin_line+1,row-begin_row+6)
            ...
            '''if ord(cmd)==27:
                cmd=getch()
                    ...
                    if cmd==50:
                        ...
                        system("clear")
                        try:
                            Interpreter(Parser(Lexer("\n".join(map("".join,code)))).block(),Main).interprete()
                            print("按任意键退出运行")
                        except Exception as e:
                            print("您的代码有错误或抛出了一个异常")
                            print(e)
                            print("按任意键退出运行")
                        getch()
                    if cmd==53:
            ...
            if ord(cmd)==0:
                cmd=ord(getch())
                if cmd==59 and line>=0:
                    line-=1
                if cmd==60 and line<len(code):
                    line+=1
                if cmd==61 and row>=0:
                    row-=1
                if cmd==61 and row==-1 and line>=0:
                    line-=1
                    row=len(code[line])
                if cmd==62 and row<=len(code[line]):
                    row+=1
                if cmd==62 and row>len(code[line]) and line<len(code):
                    line+=1
                    row=0
            elif ord(cmd)==19:
                if save_path=="":
                    gotoxy(LINE_SIZE+2,1)
                ...
                if cmd==79:
                    row=len(code[line])
                if cmd==72 and line>=0:
                    line-=1
                if cmd==80 and line<len(code):
                    line+=1
                if cmd==75 and row>=0:
                    row-=1
                if cmd==75 and row==-1 and line>=0:
                    line-=1
                    row=len(code[line])
                if cmd==77 and row<=len(code[line]):
                    row+=1
                if cmd==77 and row>len(code[line]) and line<len(code):
                    line+=1
                    row=0
            elif ord(cmd)==8:
                #print_info(f"delc {line} {row}")
            ...
            #sleep(0.2)
            system("cls")
            #print_info(";".join(map("".join,code)),19)
            if code[-1]!=[]:
                code.append([])
    try:
        IDE()
