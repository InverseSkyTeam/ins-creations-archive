...
6、内置模块
    ...
    Range模块
        定义了Range类
            这是一个用integer3实现的类
            初始化：Range(begin,end[,step])
            tolist将Range对象转为列表
            例如Range(0,3).tolist()得到[0,1,2]
            have(value)判断Range对象的范围中有没有value
            例如Range(0,3).have(3)得到false
差不多就这么多
...
Interpreter(Parser(Lexer('''
{
    ...
    import builtin_modules::Range;
    foreach(i:Range(0,10).tolist()) println(i);
    println(Range(0,10,2).have(7));
}
