...
public:
    ...
    void show(){
        system("clear");
        for(int i=begin_line;i<begin_line+10;i++){
            for(int j=begin_row;j<begin_row+10;j++)
...
public:
    ...
    void move(int direction){
        ...
        if(body[0].x>=screen->begin_line+10) screen->begin_line++;
        if(body[0].x<screen->begin_line) screen->begin_line--;
        if(body[0].y>=screen->begin_row+10) screen->begin_row++;
