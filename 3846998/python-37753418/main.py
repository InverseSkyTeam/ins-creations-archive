...
class Parser:
    ...
    def program(self):
        l=[]
        while self.currend_token.value_type!=EOF:
            l.append(self.statement())
            if type(l[-1]) not in (FuncStmt,ModuleStmt,VarStmtList):
                raise Exception("Wrong input!")
        return Program(l)
    def block(self):
        if self.currend_token.value_type==BEGIN:
...
MainScope.buildin({
    ...
    "sqrt":lambda a:a**0.5,
    "pow":lambda a,b:a**b,
    "input":lambda:GET.getline(),
    "getchar":lambda:ord(GET.getchar()),
    "putchar":lambda ch:print(chr(ch)),
    "throw":throw,
...
Interpreter(Parser(Lexer(t)).program(),MainScope).interprete()
