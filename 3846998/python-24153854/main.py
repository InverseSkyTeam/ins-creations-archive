...
notname="1234567890"
type_str=["整形","浮点","字符串","布尔"]
type_all={"整形":int,"浮点":float,"字符串":str,"布尔":bool}
def js():
    print('''前言：
    这个语言以模块为单位，分模块解析。模块头告诉程序模块的功能，有var、eval、in、out、add、sub、time、div、inv九种，每个模块的后面都要加模块完毕，程序末尾要加运行。还有，代码最下面有示范程序。
语法：
    一、变量
        var：
        变量名 变量类型=>定义一个该类型的变量（类型不能改）
        eval：
        变量名 值 要赋的值=>给变量赋值（值必须是本类型的）
    二、输入输出
        输出：
        ...
        要输入的变量 提示语=>输入
    三、计算
        加、减、乘、除、乘方分别是加减乘除和乘方，在它们下面写两个变量，程序会将计算施加于第一个变量，第二个变为0
        字符串加=>两个字符串相加，施加于第一个，第二个变为""
        字符串截取：
