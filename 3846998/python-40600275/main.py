...
AND,OR,NOT,\
IF,ELIF,ELSE,WHILE,FN,RETURN,BREAK,CONTINUE,END,LET,FOR,TO,IN,SWITCH,CASE,DEFAULT,WHERE,\
CONST,ID,\
=[i for i in range(51)]
op_dict=[('+', 1), ('-', 2), ('*', 3), ('/', 4), ('%', 5), ('==', 6), ('!=', 7), ('>=', 10), ('<=', 11), ('<<', 12), ('>>', 13), ('>', 8), ('<', 9), ('&', 14), ('|', 15), ('^', 17), ('!', 27), ('(', 20), (')', 21), (':', 18), (',', 19), ('=', 24), ('[',22), (']', 23),('\'',25),('{',26),('}',27),('.',28)]
Const,Id,Binary,Unary,Assign,While,If,Break,Continue,Return,List,Nth,Call,Fn,Let,LetFn,ForTo,ForIn,Tuple,Dict,Dot,Switch,Where=[i for i in range(23)]
def parse(code):
    def lex():
        nonlocal code
        if code=="":
        ...
        if code[0].isalpha() or code[0]=='_':
            ...
            while code!="" and (code[0].isalnum() or code[0]=='_'):
                s+=code[0]
                code=code[1:]
            if s in "and,or,not,if,else,elif,while,fn,return,break,continue,end,let,for,to,in,switch,case,default,where".split(","):
                return ({
                    "and":AND,
                    "or":OR,
                    "not":NOT,
