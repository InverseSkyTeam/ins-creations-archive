...
class Object:
    ...
    def setattr(self,name,value):
        self.attr[name]=value
    def __call__(self,*args):
        return self.getattr("__call__")(*args)
    def __getitem__(self,*args):
        return self.getattr("__getitem__")(*args)
    def __setitem__(self,*args):
        return self.getattr("__setitem__")(*args)
    __str__=__repr__=lambda self,*args:self.getattr("__str__")(*args)
class Method:
        ...
        for k,v in mr:
            ...
            def w(base,idx,to):
                if idx==[]:
                    return to
                if type(idx[0])==Var:
                    base[idx[0][0]]=w(base[idx[0][0]],idx[1:],to)
                else:
...
tks=lex('''
{
    ...
    class Func{
        let fn;
        let __init__(self,fn){
            self.fn=fn;
        }
        let __call__(self,arg)=self.fn(arg)
    }
    let fn=Func(!!(self,a)=a+1);
    println(fn(10));
    //测试2
    class List{
        let v;
        ...
        let __getitem__(self,n)=self.v[n];
        let __setitem__(self,n,v){
            self.v[n]=v;
        }
        let __str__(self)=to_string(self.v);
    }
    let lst=List([1,2,2]);
    println(lst,"[2]=",lst[2]);
    lst[2]=3;
