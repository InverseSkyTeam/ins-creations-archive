...
class Struct:
    def __init__(self,name:str,attrname:list,attrtype:list):
        self.attrname,self.attrtype=attrname,attrtype
class Object:
    ...
    def block(self):
        if self.tk.tp!=BEGIN:
            ...
            while self.tk.tp!=END:
                ...
                if type(stmts[-1]) not in (If,While,FuncDecl,StructDecl,MethodDecl):
                    self.eat(SEMICOLON)
            self.eat(END)
            return Block(stmts)
    def stmt(self):
...
code='''
{
    var a:int[]=array:int{1,2,3},b:int[]=array:int{4,5,6};
    var c:int[]=array_add!(a,b);
    print c;
}
'''
