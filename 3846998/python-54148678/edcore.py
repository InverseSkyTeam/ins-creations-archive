...
class Editor:
    ...
    def cmd_insert(self, ch: str):
        self.cmd_input = self.cmd_input[:self.cmd_x] + ch + self.cmd_input[self.cmd_x:]
        self.cmd_x += 1

    def cmd_paste(self, text: str):
        if '\n' in text or '\r' in text:
            return
        self.cmd_input += self.cmd_input[:self.cmd_x] + text + self.cmd_input[self.cmd_x:]
        self.cmd_x += len(text)
...
editor = Editor(os.get_terminal_size().lines, os.get_terminal_size().columns)
editor.mainloop()
