...
EOF="EOF"
GETCHAR,READLN,LEN,CHR,ORD,ARRAY_ADD="GETCHAR,READLN,LEN,CHR,ORD,ARRAY_ADD".split(",")
#}
#运算符及其对应的常量{
op_dict={
    '++':INC,
...
class Lexer:
    ...
    def get(self)->Token:
        ...
        if self.ch.isalnum() or self.ch=='_':
            ...
            if res in ("getchar","readln","len","ord","chr","array_add"):
                if self.ch!='!':
                    throw(LexerError())
                self.nc()
...
class BasicType:
    ...
    __str__=__repr__=lambda self:self.typename
class ArrayType:
    def __init__(self,base):
        self.base=base
    __str__=__repr__=lambda self:str(self.base)+"[]"
class TemplateType:
    def __init__(self,tname:str,targs:list):
        self.tname,self.targs=tname,targs
    __str__=__repr__=lambda self:self.tname+"<"+",".join([str(i) for i in self.targs])+">"
...
def typeof(obj):
    if type(obj)==LiteralArray:
        ...
        return BasicType(obj.typename)
    elif type(obj)==InterfaceInstance:
        return BasicType(obj.iname)
    elif type(obj)==Function:
        return TemplateType("function",obj.paratype)
    else:
        if type(obj)==int:
...
def new(tp):
    ...
    if type(tp)==TemplateType and tp.tname=="function":
        return None
    if type(tp)==BasicType and tp.typename not in "int float string null_t bool":
        return None
    return {
        "int":0,
        "float":0.0,
...
class Struct:
    def __init__(self,name:str,attrname:list,attrtype:list,impls:list=[]):
        self.attrname,self.attrtype,self.impls=attrname,attrtype,impls
class Object:
    def __init__(self,typename:str,attr:dict):
        self.typename,self.attr=typename,attr
class Interface:
    def __init__(self,name:str,methods:dict):
        self.name,self.methods=name,methods
class InterfaceInstance:
    def __init__(self,iname:str,obj:Object):
        self.iname,self.obj=iname,obj
#}
#AST定义{
class NoOp:
    pass
...
class Continue:
    csym=0
class Return:
    def __init__(self,retvalue,rettype=BasicType("Shit")):
        self.retvalue,self.rettype=retvalue,rettype
class Lambda:
    def __init__(self,para,paratype,body,rettype):
        self.para,self.paratype,self.body,self.rettype=para,paratype,body,rettype
class StructDecl:
    def __init__(self,name,attrname,attrtype):
        self.name,self.attrname,self.attrtype=name,attrname,attrtype
class DotOp:
    def __init__(self,l,r):
        self.l,self.r=l,r
class New:
    def __init__(self,tp):
        self.tp=tp
class MethodDecl:
    def __init__(self,typename,mname,para,paratype,rettype,body):
        self.typename,self.mname,self.para,self.paratype,self.rettype,self.body=typename,mname,para,paratype,rettype,body
class CallMethod:
    def __init__(self,obj,mname,args):
        self.obj,self.mname,self.args=obj,mname,args
class InterfaceDecl:
    def __init__(self,name:str,methods:dict):
        self.name,self.methods=name,methods
class Impl:
    def __init__(self,iname,typename,methods):
        self.iname,self.typename,self.methods=iname,typename,methods
#}
#语法分析器{
class Parser:
    ...
    def block(self):
        if self.tk.tp!=BEGIN:
            ...
            while self.tk.tp!=END:
                ...
                if type(stmts[-1]) not in (If,While,FuncDecl,StructDecl,MethodDecl,InterfaceDecl,Impl):
                    self.eat(SEMICOLON)
            self.eat(END)
            return Block(stmts)
    def stmt(self):
        ...
        if self.match_id("struct"):
            self.eat()
            name=self.eat(ID).v
            return StructDecl(name,*self.parse_structbody())
        if self.match_id("method"):
            self.eat()
            typename=self.eat(ID).v
            mname=self.eat(ID).v
            self.eat(LPAREN)
            if self.tk.tp!=RPAREN:
                para=[self.eat(ID).v]
            ...
            rettype=self.parse_type()
            body=self.block()
            return MethodDecl(typename,mname,para,paratype,rettype,body)
        if self.match_id("interface"):
            self.eat()
            name=self.eat(ID).v
            self.eat(BEGIN)
            method={}
            while self.tk.tp!=END:
                ...
                rettype=self.parse_type()
                self.eat(SEMICOLON)
                method[mname]=[rettype]+paratype
            self.eat(END)
            return InterfaceDecl(name,method)
        if self.match_id("impl"):
            self.eat()
            iname=self.eat(ID).v
            typename=self.eat(ID).v
            self.eat(BEGIN)
            ...
            while self.tk.tp!=END:
                ...
                self.eat(COLON)
                rettype=self.parse_type()
                method[mname]=[[rettype]+paratype,self.block(),para]
                self.eat(SEMICOLON)
            self.eat(END)
            return Impl(iname,typename,method)
        else:
            return self.xexpr()
    def parse_structbody(self):
        attrname,attrtype=[],[]
        self.eat(BEGIN)
        while self.tk.tp!=END:
            attrname.append(self.eat(ID).v)
            self.eat(COLON)
            attrtype.append(self.parse_type())
            self.eat(SEMICOLON)
        self.eat(END)
    ...
    def factor(self):
        if self.match_id("new"):
            self.eat()
            return self.fh(New(self.parse_type()))
            ...
            if self.tk.tp==LPAREN:
                ...
                if self.tk.tp!=RPAREN:
                    ...
                    while self.tk.tp==COMMA:
                        ...
                        args.append(self.xexpr())
                self.eat(RPAREN)
                return self.fh(CallMethod(v,aname,args))
            return self.fh(DotOp(v,aname))
        return v
#}
#类型作用域{
class TypeScope:
    ...
    def delete(self):
        ...
        self.sstack.pop()
        self.istack.pop()
    def findi(self,name:str):
        for i in reversed(self.istack):
            if name in i:
                return i[name]
        throw(CheckerError(f"Undefined identifier '{name}'"))
...
class Checker:
    ...
    visit_InterfaceInstance=lambda self,node:BasicType(node.iname)
    ...
    def visit_VarDecl(self,node:VarDecl):
        for i in range(len(node.types)):
            ...
            if node.types[i]==None:
                node.types[i]=v
            if type(v)==type(node.types[i])==BasicType and self.is_interface(node.types[i].typename) and node.types[i].typename in self.scope.findtype(v.typename).impls:
                continue
            if not type_eq(node.types[i],v):
                throw(CheckerError())
    ...
    def visit_Assign(self,node:Assign):
        ...
        while type(lvalue) in (ElementOp,DotOp):
            if type(lvalue)==ElementOp and not type_eq(self.visit(lvalue.r),BasicType("int")):
                throw(CheckerError())
            if type(lvalue)==DotOp and type(lvalue.r)!=str:
                throw(CheckerError())
            lvalue=lvalue.l
        if type(lvalue)!=Var:
            throw(CheckerError())
        if type(base)==BasicType and type(target)==BasicType:
            if not type_eq(base,target):
                if not self.is_interface(base.typename):
                    throw(CheckerError())
                if base.typename not in self.scope.findtype(target.typename).impls:
                    throw(CheckerError())
        return target
    ...
    def visit_MacroOrd(self,node:MacroOrd):
        ...
        if type(arg)!=BasicType or arg.typename!="string":
            throw(CheckerError())
        return BasicType("int")
    def visit_MacroArrayAdd(self,node:MacroArrayAdd):
        a,b=self.visit(node.a),self.visit(node.b)
        if type(a)!=ArrayType or type(b)!=ArrayType or not type_eq(a,b):
            throw(CheckerError())
        return a
    ...
    def visit_CallFunc(self,node:CallFunc):
        ...
        for i in range(len(args)):
            if not self.is_type_vaild(args[i],func.targs[i+1]):
                throw(CheckerError())
        return func.targs[0]
    def visit_Lambda(self,node:Lambda):
        ...
        self.rettype=self_rettype
        self.scope.delete()
        return TemplateType("function",[node.rettype]+node.paratype)
    def visit_StructDecl(self,node:StructDecl):
        self.scope.sstack[-1][node.name]=Struct(node.name,node.attrname,node.attrtype)
    visit_NoOp=lambda self,node:None
    visit_Break=visit_Continue=lambda self,node:None
    def visit_Return(self,node:Return):
        rv=self.visit(node.retvalue)
        node.rettype=self.rettype
        if type(rv)==type(self.rettype)==BasicType:
            if self.is_interface(self.rettype.typename) and not self.is_interface(rv.typename):
                if self.rettype.typename in self.scope.findtype(rv.typename).impls:
                    return
        if not type_eq(rv,self.rettype):
            throw(CheckerError())
    def visit_DotOp(self,node:DotOp):
        l=self.visit(node.l)
        r=node.r
        if type(l)!=BasicType or l.typename in "int bool string null_t float":
            throw(CheckerError())
        if r not in self.scope.findtype(l.typename).attrname:
            throw(CheckerError())
        return self.scope.findtype(l.typename).attrtype[self.scope.findtype(l.typename).attrname.index(r)]
    def visit_New(self,node:New):
        if type(node.tp)!=BasicType or node.tp.typename in "int float bool string null_t":
            throw(CheckerError())
        return node.tp
    def visit_MethodDecl(self,node:MethodDecl):
        self.scope.findtype(node.typename)
        self.scope.buildin({
            node.typename+"."+node.mname:TemplateType("function",[node.rettype]+node.paratype)
        })
        self.scope.new()
        self.scope.buildin(dict(zip(node.para,node.paratype)))
        self_rettype=self.rettype
        ...
        self.rettype=self_rettype
        self.scope.delete()
    def visit_CallMethod(self,node:CallMethod):
        obj=self.visit(node.obj)
        if type(obj)!=BasicType or obj.typename in "int,float,bool,null_t,string":
            throw(CheckerError())
        if self.is_interface(obj.typename):
            method:TemplateType=self.scope.findi(obj.typename).methods[node.mname]
        else:
            method:TemplateType=self.scope.find(obj.typename+"."+node.mname)
        paratype=method.targs[1:]
        if len(paratype)!=len(node.args)+1:
            throw(CheckerError())
        for i in range(len(paratype)-1):
            if not self.is_type_vaild(self.visit(node.args[i]),paratype[i+1]):
                throw(CheckerError())
        return method.targs[0]
    def visit_InterfaceDecl(self,node:InterfaceDecl):
        self.scope.istack[-1][node.name]=Interface(node.name,dict(zip(node.methods.keys(),[TemplateType("function",v) for k,v in node.methods.items()])))
    def visit_Impl(self,node:Impl):
        self.scope.findtype(node.typename).impls.append(node.iname)
        base=node.iname+" "+node.typename+" "
        for k,v in node.methods.items():
            mtype=TemplateType("function",v[0])
            self.scope.buildin({base+k:mtype})
            self.scope.new()
            self.scope.buildin(dict(zip(v[2],v[0][1:])))
            self_rettype=self.rettype
            self.rettype=v[0][0]
            self.visit(v[1])
            self.rettype=self_rettype
            self.scope.delete()
    visit_Object=lambda self,node:BasicType(node.typename)
    def is_mytype(self,typename):
        if typename not in "int float bool str null":
            return False
        for i in self.scope.sstack:
            if typename in i:
                return True
        for i in self.scope.istack:
            if typename in i:
                return True
        return False
    def is_interface(self,typename):
        for i in self.scope.istack:
            if typename in i:
                return True
        return False
    def is_struct(self,typename):
        for i in self.scope.sstack:
            if typename in i:
                return True
        return False
    def is_type_vaild(self,got,expect):
        if type_eq(expect,got):
            return True
        if type(expect)==type(got)==BasicType:
            if self.is_interface(expect.typename) and self.is_struct(got.typename):
                if expect.typename in self.scope.findtype(got.typename).impls:
                    return True
        return False
    def check(self):
...
class Interpreter:
    ...
    visit_int=visit_str=visit_bool=visit_float=visit_NoneType=visit_LiteralArray=visit_Function=visit_InterfaceInstance=lambda self,node:node
    def visit_VarDecl(self,node:VarDecl):
        v=[self.visit(i) for i in node.values]
        for i in range(len(v)):
            if type(typeof(v[i]))==type(node.types[i])==BasicType and self.is_interface(node.types[i].typename) and node.types[i].typename in self.scope.findtype(typeof(v[i]).typename).impls:
                v[i]=InterfaceInstance(node.types[i].typename,v[i])
        self.scope.buildin(dict(zip(node.names,v)))
    def visit_Block(self,node:Block):
        for i in node.children:
            v=self.visit(i)
    ...
    def visit_Assign(self,node:Assign):
        ...
        tp=typeof(self.visit(lvalue))
        if not type_eq(tp,target):
            target=InterfaceInstance(tp.typename,target)
    ...
    def visit_Lambda(self,node:Lambda):
        return Function(node.para,node.paratype,node.body,self.scope)
    def visit_CallFunc(self,node:CallFunc):
        func:Function=self.visit(node.func)
        args=[self.visit(i) for i in node.args]
        for i in range(len(args)):
            args[i]=self.arg_fill(func.paratype[i],args[i])
        ...
        return v.retvalue
    def visit_DotOp(self,node:DotOp):
        return self.visit(node.l).attr[node.r]
    def visit_New(self,node:New):
        _type:Struct=self.scope.findtype(node.tp.typename)
        return Object(node.tp.typename,dict(zip(_type.attrname,[new(i) for i in _type.attrtype])))
    def visit_MethodDecl(self,node:MethodDecl):
        self.scope.buildin({
            node.typename+"."+node.mname:Function(node.para,node.paratype,node.body,self.scope)
        })
    def visit_CallMethod(self,node:CallMethod):
        obj:Object|InterfaceInstance=self.visit(node.obj)
        if type(obj)==InterfaceInstance:
            func:Function=self.scope.find(obj.iname+" "+obj.obj.typename+" "+node.mname)
        else:
            func:Function=self.scope.find(obj.typename+"."+node.mname)
        args=[obj]+[self.visit(i) for i in node.args]
        for i in range(len(args)):
            args[i]=self.arg_fill(func.paratype[i+1],args[i])
        self_scope=self.scope
        self.scope=func.closure
        self.scope.new()
        self.scope.buildin(dict(zip(func.para,args)))
    ...
    def visit_MacroArrayAdd(self,node:MacroArrayAdd):
        a,b=self.visit(node.a),self.visit(node.b)
        return LiteralArray(a.l+b.l,a.tp)
    def visit_InterfaceDecl(self,node:InterfaceDecl):
        self.scope.istack[-1][node.name]=Interface(node.name,node.methods)
    def visit_Impl(self,node:Impl):
        self.scope.findtype(node.typename).impls.append(node.iname)
        base=node.iname+" "+node.typename+" "
        for k,v in node.methods.items():
            self.scope.buildin({base+k:Function(v[2],v[0],v[1],self.scope)})
    def is_mytype(self,typename:str):
        if typename not in "int float bool str null":
            return False
        for i in self.scope.sstack:
            if typename in i:
        ...
        for i in self.scope.istack:
            if typename in i:
                return True
        return False
    def is_interface(self,typename:str):
        for i in self.scope.istack:
            if typename in i:
                return True
        return False
    def is_struct(self,typename:str):
        for i in self.scope.sstack:
            if typename in i:
                return True
        return False
    ...
    def visit_Return(self,node:Return):
        v=self.visit(node.retvalue)
        if not type_eq(typeof(v),node.rettype):
            return Return(InterfaceInstance(node.rettype.typename,v))
        return Return(v)
    def arg_fill(self,expect,got):
        tp=typeof(got)
        if type_eq(expect,tp):
            return got
        else:
            return InterfaceInstance(expect.typename,got)
    visit_Object=lambda self,node:node
...

{
    ...
    var counter:function<int> =Counter(1);
    var i:int;
    while(i<10){
        print counter();
        i=i+1;
...

{
    ...
    method T printa(this:T):null_t{
        print[sep=""] "T object:a=",this.a;
        return null;
    }
    var a:T=new T;
    a.printa();
...
{
    struct Printer{}
    method Printer print(this:Printer,a:string):string{
        print a;
        return a;
    }
    var a:Printer=new Printer;
...

{
    ...
    method T changeA(this:T,to:int):int{
        this.a=to;
        return to;
    }
    var a:T=new T;
...
{
    var a=array:int{1,2,3},b=array:int{4,5,6};
    var c=array_add!(a,b);
    print c;
}
