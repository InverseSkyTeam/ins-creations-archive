...
def compile(prog):
    ...
    def compile_expr(expr):
        match expr:
            case "const", val:
                output.append((Runner.push, val))
            case "var", name:
                output.append((Runner.load, *find_var(name)))
            case "list", l:
                for i in l:
                    compile_expr(i)
                output.append((Runner._list, len(l)))
            case "dict", l:
                ks = []
                for k, v in l:
                    ks.append(k)
                    compile_expr(v)
                output.append((Runner._dict, ks))
            case "module", l:
                ks = []
                for k, v in l:
                    ks.append(k)
                    compile_expr(v)
                output.append((Runner.module, ks))
            case "unary", op, arg:
                compile_expr(arg)
                output.append((Runner.unary, op))
            case "expr", l:
                for i in l:
                    if isinstance(i, TokenType):
                        output.append((Runner.binary, i))
                    else:
                        compile_expr(i)
            case "attr", obj, attr:
                compile_expr(obj)
                output.append((Runner.attr, attr))
            case "index", obj, index:
                compile_expr(obj)
                compile_expr(index)
                output.append((Runner.index,))
            case "call", fn, args:
                for i in args:
                    compile_expr(i)
                compile_expr(fn)
                output.append((Runner.call, len(args)))
            case "func", args, body:
                fn_vars = args + hoist(body)
                output.append(
                    (Runner.func, len(output) + 1, "<lambda>", len(args), len(fn_vars))
                )
                output.append(None)
                pos = len(output) - 1
                scope.append({})
                for i, arg in enumerate(fn_vars):
                    scope[-1][arg] = i
                compile_block(body)
                output.append((Runner.push, None))
                output.append((Runner.ret,))
                output[pos] = Runner.jmp, len(output) - 1
                scope.pop()
            case "require", path:
                # 打包成函数
                try:
                ...
                vars = hoist(prog)
                output.append(
                    (Runner.func, len(output) + 1, f"<module {path}>", 0, len(vars))
                )
                output.append(None)
                pos = len(output) - 1
                scope.append({})
                ...
                output[pos] = Runner.jmp, len(output) - 1
                scope.pop()
                output.append((Runner.call, 0))
            case "class", parents, attrs:
                for i in parents:
                    compile_expr(i)
                for i in map(lambda a: a[1], attrs):
                    compile_expr(i)
                output.append(
                    (Runner._class, len(parents), list(map(lambda a: a[0], attrs)))
                )
            case _:
        ...
        match stmt:
            ...
            case "var", varlist:
                for name, val in varlist:
                    compile_expr(val)
                    output.append((Runner.store, *find_var(name)))
            case "func", name, args, body:
                fn_vars = args + hoist(body)
                output.append(
                    (Runner.func, len(output) + 2, name, len(args), len(fn_vars))
                )
                output.append((Runner.store, *find_var(name)))
                output.append(None)
                pos = len(output) - 1
            ...
            case "if", cases, else_case:
                ...
                for cond, body in cases:
                    ...
                    output[jnz] = Runner.jnz, len(output)
                    output.append(None)
                if else_case:
                    compile_block(else_case)
                for i in jmps:
                    output[i] = Runner.jmp, len(output) - 1
            case "while", cond, body:
                ...
                output.append(None)
                compile_block(body)
                output.append((Runner.jmp, while_beginposs[-1]))
                jmpends = while_jmpends.pop()
                output[jnz] = Runner.jnz, len(output) - 1
                for i in jmpends:
                    output[i] = Runner.jmp, len(output) - 1
            case "forto", iname, begin, end, step, body:
                ...
                compile_expr(begin)
                output.append((Runner.store, *varpos))
                while_jmpends.append([])
                while_beginposs.append(len(output) - 1)
                output.append((Runner.load, *varpos))
                compile_expr(end)
                output.append((Runner.binary, TokenType.LT))
                jnz = len(output)
                output.append(None)
                compile_block(body)
                compile_expr(step)
                output.append((Runner.load, *varpos))
                output.append((Runner.binary, TokenType.ADD))
                output.append((Runner.store, *varpos))
                output.append((Runner.jmp, while_beginposs[-1]))
                jmpends = while_jmpends.pop()
                output[jnz] = Runner.jnz, len(output) - 1
            ...
            case "assign", op, left, right:
                if op == TokenType.ASSIGN:
                    ...
                    compile_expr(left)
                    compile_expr(right)
                    output.append((Runner.binary, assignop2op[op]))
                compile_expr(left)
                if output[-1][0] not in (Runner.load, Runner.attr, Runner.index):
                    raise RainError(f"Invalid l-value: {left}")
                if output[-1][0] == Runner.load:
                    output[-1] = Runner.store, *output[-1][1:]
                elif output[-1][0] == Runner.attr:
                    output[-1] = Runner.setattr, *output[-1][1:]
                elif output[-1][0] == Runner.index:
                    output[-1] = Runner.setindex, *output[-1][1:]
            case _:
...
class Class:
    ...
    def __init__(self, parents: "list[Class]", attrs: dict):
        self.id = Class.id
        Class.id += 1
    ...
    def __eq__(self, other):
        return isinstance(other, Class) and self.id == other.id

    def __ne__(self, other):
        return not isinstance(other, Class) or self.id != other.id
...
class Runner:
    def __init__(self, prog: list, reserve_cnt: int, pc: int = 0, scope=None):
        self.stack = []
        self.pc_stack = []
        self.scope_stack = []
        self.pc = pc
        self.prog = prog
        self.reserve_cnt = reserve_cnt
        if scope is None:
            self.scope = [builtins[1]]
        else:
            self.scope = scope
        # print(self.scope[-1])
        while len(self.scope[-1]) < self.reserve_cnt:
            self.scope[-1].append(None)

    def binary(self, op):
        b = self.stack.pop()
        self.stack[-1] = binary_fntable[op.value](self.stack[-1], b)
    ...
    def load(self, n, i):
        self.stack.append(self.scope[-n][i])

    def store(self, n, i):
        self.scope[-n][i] = self.stack.pop()
    ...
    def call(self, n):
        ...
        if isinstance(func, Method):
            ...
            obj = func.recursive()
            if "$init" not in obj:
                raise RainNameError("$init unimplemented")
            args = [obj] + args
            func = obj["$init"]
        else:
            args = []
        ...
        if isinstance(func, Func):
            if len(args) != func.argcnt:
                # print(func)
                raise RainError(f"Invalid argument count for function {func.name}")
            self.scope_stack.append(self.scope)
            self.pc_stack.append(self.pc)
            self.pc = func.pc
            self.scope = func.closure.copy()
            self.scope.append(args)
            for _ in range(func.reserve_cnt - len(args)):
                self.scope[-1].append(None)
        elif callable(func):
            # print(f"call {args}")
    ...
    def setattr(self, attr):
        base, val = self.stack.pop(), self.stack.pop()
        base[attr] = val

    def setindex(self):
        index, base, val = self.stack.pop(), self.stack.pop(), self.stack.pop()
        base[index] = val
    ...
    def attr(self, attr):
        ...
        if isinstance(base, dict) and (isinstance(res, Func) or callable(res)):
            self.stack.append(Method(base, res))
        else:
    ...
    def _dict(self, ks):
        x = self.stack[-len(ks) :]
        del self.stack[-len(ks) :]
        self.stack.append(dict(zip(ks, x)))

    def func(self, p, name, argcnt, reserve_cnt):
        func = Func(argcnt, name, p, self.scope.copy(), reserve_cnt)
        self.stack.append(func)

    def module(self, ks):
        x = self.stack[-len(ks) :]
        del self.stack[-len(ks) :]
        self.stack.append(Module(dict(zip(ks, x))))

    def _class(self, p_cnt, attrs):
        attr_vs = self.stack[-len(attrs) :]
        del self.stack[-len(attrs) :]
        parents = self.stack[-p_cnt:]
        del self.stack[-p_cnt:]
        res = dict(zip(attrs, attr_vs))
        self.stack.append(Class(parents, res))

    def run(self):
        while self.prog[self.pc] is not None:
            # print(self.prog[self.pc])
            self.prog[self.pc][0](self, *self.prog[self.pc][1:])
            self.pc += 1


def print_bytecode(code):
    for i, op in enumerate(code[:-1]):
        print(i, op[0].__name__, *map(repr, op[1:]))
