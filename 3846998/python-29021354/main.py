...
def funcrun(code,arglist):
    ...
    while xxxxxx<len(code):
        ...
        if pd(ml,[2]) and ml[0]=="put":
            aaa=exp(ml[1])
            if getvar(ml[1])!=None:
                print(getvar(ml[1]),end="")
            elif aaa!=None:
                print(aaa,end="")
            elif xtype(ml[1]):
                print(xtype(ml[1])[1],end="")
            elif getarr(ml[1])!=None:
                print(getarr(ml[1]),end="")
            elif ml[1]=="enter":
                print()
            elif "[" in ml[1] and "]" in ml[1] and getarr(ml[1][0:ml[1].index("[")])!=None:
                if xint(ml[1][ml[1].index("[")+1:ml[1].index("]")])!=None:
                    print(getarr(ml[1][0:ml[1].index("[")])[xint(ml[1][ml[1].index("[")+1:ml[1].index("]")])],end="")
                elif getvar(ml[1][ml[1].index("[")+1:ml[1].index("]")])!=None:
                    print(getarr(ml[1][0:ml[1].index("[")])[getvar(ml[1][ml[1].index("[")+1:ml[1].index("]")])],end="")
                else:
                    print("列表元素的调用方法必须是列表名[数字或变量]！")
            else:
