...
class Runner:
    ...
    def run_tree(self,tree):
        if type(tree)==str:
            ...
            func=tree[0]
            if type(func)==str and func in ("fn","macro","if","begin","var","set!"):
                if func=="fn":
                    para=[]
                    for i in tree[1]:
                        if type(i)!=str:
                ...
                if func=="macro":
                    ...
                    for i in tree[1]:
                        ...
                        para.append(i)
                    body=tree[-1]
                    return Macro(para,body,self.scope)
                if func=="if":
                    if self.run_tree(tree[1]):
                        return self.run_tree(tree[2])
                    return self.run_tree(tree[3])
                if func=="begin":
                    if len(tree)==1:
                        return None
                    return [self.run_tree(i) for i in tree[1:]][-1]
                if func=="var":
                    self.scope.var[tree[1]]=self.run_tree(tree[2])
                if func=="set!":
                    self.scope.set(tree[1],self.run_tree(tree[2]))
            else:
                func=self.run_tree(func)
                if type(func)==Func:
                    args=[self.run_tree(i) for i in tree[1:]]
...
(begin
    ...
    (var GREET (macro()(list "println" "\\"Hello,world!\\"")))
    (var while (macro(cond body)(list "if" cond (list "begin" body (list "while" cond body)) "null")))
    (var repeat (macro(n body)(list "if" (list "!=" n "0") (list "begin" body (list "repeat" (list "-" n "1") body)) "null")))
    (GREET)
    (var i 0)
    (while (< i 10)(begin
        (println i)
