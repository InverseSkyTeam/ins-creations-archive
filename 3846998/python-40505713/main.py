...
Const,Id,Binary,Unary,Assign,While,If,Break,Continue,Return,List,Nth,Call,Fn,Let=[i for i in range(15)]
def parse(code):
    def lex():
        nonlocal code
        if code=="":
...
def run(tree,scope):
    ...
    if type(tree)==list:
        ...
        tp=tree[0]
        if tp==Const:
            return (tree[1],signal_default)
        if tp==Id:
            return (find(tree[1]),signal_default)
        if tp==Nth:
            return (run(tree[1],scope)[0][run(tree[2],scope)[0]],signal_default)
        if tp==Fn:
            return ((tree[1],tree[2],[],scope),signal_default)
        if tp==List:
            return ([run(i,scope)[0] for i in tree[1]],signal_default)
        if tp==Call:
            return (call(run(tree[1],scope)[0],[run(i,scope)[0] for i in tree[2]]),signal_default)
        if tp==Binary:
            return ({
                ADD:lambda a,b:a+b,
                SUB:lambda a,b:a-b,
                ...
                BOR:lambda a,b:a|b,
                XOR:lambda a,b:a^b,
            }[tree[1]](run(tree[2],scope)[0],run(tree[3],scope)[0]),signal_default)
        if tp==Unary:
            return ({
                ADD:lambda a:+a,
                SUB:lambda a:-a,
                NOT:lambda a:not a,
                INV:lambda a:~a,
            }[tree[1]](run(tree[2],scope)[0]),signal_default)
        if tp==Break:
