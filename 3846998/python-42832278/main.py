...
def edit(render):
    ...
    def do_cmds(cmd):
        ...
        for i in cmd:
            do_cmd(i)
    def do_cmd(ch):
        nonlocal mode,x,y,code,begin,beginy,history,size,sizey,selectx,selecty,nu,save
        if ord(ch)==224:
            ch=ord(getch())
            if ch==72:
                if x!=0:
            ...
            if ch in ikey:
                ...
                if ch=='\r':
                    if code[x] and code[x][y] in '([{':
                        code.insert(x+1,[" "]*(count_tab(x)+1)*2)
                        code.insert(x+2,[" "]*(count_tab(x))*2+code[x][y+1:])
                        code[x]=code[x][:y+1]
                        x+=1
                        y=count_tab(x-1)*2+1
                    elif y+1<len(code[x]) and code[x][y+1] in ')]}' or\
                    "".join(code[x][:3]) in ["for"] or\
                    "".join(code[x][:4]) in ["elif","else"] or\
                    "".join(code[x][:2]) in ["if"] or\
                    "".join(code[x][:5]) in ["while"] or\
                    "".join(code[x][:6]) in ["module","switch"] or\
