...
class Checker:
    ...
    def visit_If(self,t):
        self.visit(t[0])
        Checker(t[1],Scope(self.scope)).check()
    def visit_IfElse(self,t):
        self.visit(t[0])
        Checker(t[1],Scope(self.scope)).check()
        Checker(t[2],Scope(self.scope)).check()
    def visit_While(self,t):
        self.visit(t[0])
        Checker(t[1],Scope(self.scope)).check()
    def visit_For(self,t):
        i1=Checker(t[0],Scope(self.scope))
        i1.check()
        i1.visit(t[1])
        Checker(t[3],Scope(i1.scope)).check()
        i1.visit(t[2])
    ...
    visit_int=lambda self,a:None
    visit_float=lambda self,a:None
    visit_bool=lambda self,a:None
    visit_NoneType=lambda self,a:None
    visit_list=lambda self,a:None
    visit_str=lambda self,a:None
    visit_Var=lambda self,t:self.scope.find(t[0])
    def check(self):
        self.visit(self.t)
class Interpreter:
    def visit(self,node):
...
'''
Checker(Parser(Lexer(code)).stmt()).check()
Interpreter(Parser(Lexer(code)).stmt()).interprete()