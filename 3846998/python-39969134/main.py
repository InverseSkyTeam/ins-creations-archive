...
def run(code:str):
    ...
    def stmt(tab=0):
        ...
        if tk[0]==DEF:
            ...
            eat(RPAREN)
            return Tree("def",name,para,block(tab))
        if tk[0]==IF:
            eat()
            return Tree("if",expr(),block(tab))
        if tk[0]==WHILE:
            eat()
            return Tree("while",expr(),block(tab))
        if tk[0]==BREAK:
            eat()
            return Tree("break",)
        if tk[0]==CONTINUE:
        ...
        if tk[0]==RETURN:
            eat()
            return Tree("return",expr())
        else:
            e=expr()
            if tk[0]==ASSIGN:
                eat()
                return Tree("assign",e,expr())
            return e
    def expr():
        nonlocal tk
        res=expr0()
        while tk[0] in (AND,OR):
            res=Tree("binary",eat()[0],res,expr0())
        return res
    def expr0():
        nonlocal tk
        res=expr1()
        while tk[0]==IN:
            res=Tree("binary",eat()[0],res,expr1())
        return res
    def expr1():
        nonlocal tk
        res=expr2()
        while tk[0] in (BAND,BOR,XOR):
            res=Tree("binary",eat()[0],res,expr2())
        return res
    def expr2():
        nonlocal tk
        res=expr3()
        while tk[0] in (EQ,NE,GT,LT,GE,LE):
            res=Tree("binary",eat()[0],res,expr3())
        return res
    def expr3():
        nonlocal tk
        res=expr4()
        while tk[0] in (LSH,RSH):
            res=Tree("binary",eat()[0],res,expr4())
        return res
    def expr4():
        nonlocal tk
        res=term()
        while tk[0] in (ADD,SUB):
            res=Tree("binary",eat()[0],res,term())
        return res
    def term():
        nonlocal tk
        res=factor()
        while tk[0] in (MUL,DIV,MOD):
            res=Tree("binary",eat()[0],res,factor())
        return res
    def factor():
        nonlocal tk
        if tk[0]==CONST:
            return fh(eat()[1])
        if tk[0]==ID:
            return fh(Tree("var",eat()[1]))
        if tk[0] in (ADD,SUB,NOT,INV):
            return Tree("unary",eat()[0],fh(factor()))
        if tk[0]==LSB:
            eat()
            l=[]
            if tk[0]!=RSB:
    ...
    def runcode(tree):
        ...
        if cmd=="program":
            ...
            if runcode(tree[1]):
                for i in tree[2]:
                    if i[0] in ("return","break","continue"):
                        ...
                        return i
                    v=runcode(i)
                    if type(v)==Tree and v[0] in ("return","break","continue"):
                        return v
            return None
        elif cmd=="while":
            while runcode(tree[1]):
                ...
                for i in tree[2]:
                    ...
                    if i[0]=="return":
                        ...
                        return ("return",runcode(i[1]))
                    v=runcode(i)
                    if type(v)==Tree and v[0]=="break":
                        outscope()
                        return
                    if type(v)==Tree and v[0]=="return":
                        outscope()
                        return v
                    if type(v)==Tree and v[0]=="continue":
                        break
                outscope()
            return None
        elif cmd=="var":
            ...
            if lvalue[0]!="var":
                indexlist=[]
                while type(lvalue)==Tree and lvalue[0]=="nth":
