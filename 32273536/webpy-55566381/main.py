...
def parse(token_stream: TokenStream):
    ...
    def import_decl():
        ...
        name = eat(TokenType.STRING)[0][1]
        return ["import", ["str", escape(name)]]
...
def interpret(tree: Any) -> Any:
    ...
    env.append(
        {"println": print, "readln": input, "object": dict, "array": list, "len": len}
    )
    ...
    def eval_expr(t: Any) -> Any:
        match t:
            case ["unary", op, v]:
                ...
                if op == TokenType.SUB:
                    return operator.neg(eval_expr(v))
    ...
    def eval_stmt(t: Any) -> None:
        match t:
            ...
            case ["import", ["str", str(name)]]:
                ...
                eval_program(t)
                e = pop_env()
                peek_env()[name.removesuffix(".ks")] = e
            case _:
                raise NotImplementedError()
