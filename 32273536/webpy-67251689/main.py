...
def set_meta_table(f,t):
    setattr(f,"_meta",t)

def get_meta_table(f):
    try:
        return get_kattr(f,"_meta")
    except AttributeError:
        return {}
    
def noself(f):
    return lambda _,*args: f(*args)

def interpret(tree: Any, e: Optional[list[dict[str, Any]]] = None) -> Any:
    # interpreter varriables
    env: list[dict[str, Any]] = e if e else [{
            "println": noself(print),
            "readln": noself(input),
            "object": noself(dict),
            "array": noself(list),
            "len": noself(len),
            "apply": noself(apply),
            "set_meta_table": noself(set_meta_table),
            "get_meta_table": noself(get_meta_table)
        }]
    ...
    def build_function(params: list[str], body: Any) -> Any:
        ...
        def function_wrapper(self, *args: Any) -> Any:
            nonlocal env
            invalid_arg = False
            try:
            ...
            env = fn_env
            return_value = None
            if v := get_meta_table(self).get("this"):
                env[-1]["this"] = v
            try:
                eval_stmt(fn_body)
            ...
            env = cur_env
            return return_value
        set_meta_table(function_wrapper, {"this": function_wrapper})
...
if __name__ == "__main__":
    print(
        execute(
            r"""let Animal = func (name, sound) {
                    let e = {};
                    
                    // constructor body
                    println("Constructing a animal");
                    println("animal name: " + name);
                    e.name = name;
                    e.sound = sound;
                    e.get_name = func () { return this.name; };
                    
                    set_meta_table(e.get_name, {"this": e} );
                    return e;
                };
                let a = Animal("cat", "meow");
                let b = Animal("dog", "w");
                println(a.get_name());
                println(b.get_name());
                """
