...
try:
    with open(system_site+'/prove/the_prove.txt','r',encoding='utf-8') as f:
        if f.read() == 'New installed 0.41 Edition.':
            part = 'main'
        else:
            shutil.rmtree('../../小轩电脑')
            part = 'install'
...
class File(object):
    def __init__(self,name,ty,pos,img='../../小轩电脑/C/User/system/icon/未知格式文件.png'):
        ...
        if self.ty == '.txt':
            ...
            self.img = pygame.transform.scale(pygame.image.load('../../小轩电脑/C/User/system/icon/图片png文件.png'),(100,100))
        elif self.ty == '.jpg':
            self.img = pygame.transform.scale(pygame.image.load('../../小轩电脑/C/User/system/icon/图片jpg文件.png'),(100,100))
        elif self.ty == '.mp3':
            self.img = pygame.transform.scale(pygame.image.load('../../小轩电脑/C/User/system/icon/音乐文件.png'),(100,100))
        elif self.ty == '.exe':
            self.img = pygame.transform.scale(pygame.image.load('../../小轩电脑/C/User/system/icon/可执行文件.png'),(100,100))
        else:
...
while True:
    while part == 'install':
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    ...
                    copy_file('file/files', '../../小轩电脑/C/User/Admin/desktop')
                    with open(system_site+'/prove/the_prove.txt','w',encoding='utf-8') as f:
                        f.write('New installed 0.41 Edition.')
                    f.close()
                    with open('../../小轩电脑/C/User/Admin/desktop/my computer.txt','w',encoding='utf-8') as f:
                        f.write('小轩电脑0.4 Edition 专业版 10MB 下一个版本：小轩电脑0.5')
                    f.close()
                    with open('../../小轩电脑/C/User/Admin/desktop/abc.txt','w',encoding='utf-8') as f:
                        f.write('这里能打字！开心每一天！')
                    f.close()
                    update_all()
                    part = 'install_ok'
        ...
        show_text('安装完成后，您将得到小轩电脑0.4Edition专业版',pos=(11,41))
    ...
    while part == 'install_ok':
        ...
        show_text('版本：小轩电脑0.4Edition专业版',pos=(11,41))
        pygame.display.update()
        try:root.update()
        except:sys.exit()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == ord('s'):
                    part = '关机'
        screen.fill((65,255,255))
        show_text('小轩电脑0.4 | '+str(round(file_size,2))+'MB/10MB',pos=(11,11))
        show_text('='*80,pos=(0,41))
        for i in filelist:
            i.draw()
        show_text('按下DELETE键删除系统,U键刷新桌面,S键关机(都是虚拟)',color=(0,0,0),pos=(11,750))
        if file_size > 10:
            part = 'BLUESCREEN'
        pygame.display.update()
        try:root.update()
    ...
    while part == 'BLUESCREEN':
        ...
        show_text('错误问题：file_size > 10MB',color=(255,255,255),pos=(11,41))
    ...
    while part == '回退':
        ...
        show_text('按下7：搞错了，我还是重返小轩电脑0.4专业版吧',color=(255,255,255),pos=(11,390))
...
- 显示 - 磁盘与驱动器(1):
 ...
 磁盘名称:( C: )
  磁盘命名: 本地磁盘(C:/)
''')
                    if input('此处键入以上您拥有的磁盘名称,如A: | B: | C:,即可进入磁盘图形界面，键入其他或者回车并且关闭cmd引擎则自动关闭引擎') == 'C:':
                        print('准备进入')
                        import pygame
                        print('访问成功')
                        pygame.init()
                        ...
                        print('加载素材、模块成功')
                        sleep(0.3)
                        print('加载界面与包出现问题，请关闭cmd或者程序，重新运行')
                    else:
                        sys.exit()
        screen.fill((255,255,255))
        show_text('你保存的编辑与更改已经转入存储，请放心，它们很安全',pos=(11,11))
        show_text('按下空格键关机(关闭“小轩电脑0.4专业版”)',pos=(11,41))
        pygame.display.update()
        try:root.update()
        except:sys.exit()