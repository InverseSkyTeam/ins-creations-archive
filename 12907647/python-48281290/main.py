...
class Enemy:
    def __init__(self,name,level=level):
        ...
        self.pos = self.rect.center = data.enemy_out[level][0]
    ...
    def move(self):
        ...
        if (abs(x_-x) <= self.speed / 2) and (abs(y_-y) <= self.speed / 2):
...
tower_list = []
enemy_list = [Enemy('N1') for i in range(10)]
