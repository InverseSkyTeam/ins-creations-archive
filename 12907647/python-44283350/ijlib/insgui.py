...
class INSwindow(tk.Toplevel):
    def __init__(self,w,h,winx=0,winy=0,radius=25,title='INS window',bg='#313131',titlebg='#111111',icon=None):
        ...
        # 内部窗口--------------------------------------------
        self.winframe = tk.Frame(self,width=self.w,height=self.h,bd=0,bg=self.bgc)
        ...
        self.closebtn.bind("<Enter>",lambda event:self.hover(event,self.closebtn))
        self.closebtn.bind("<Leave>",lambda event:self.hover(event,self.closebtn))
        self.fullscbtn.bind("<Enter>",lambda event:self.hover(event,self.fullscbtn))
        self.fullscbtn.bind("<Leave>",lambda event:self.hover(event,self.fullscbtn))
        self.icbtn.bind("<Enter>",lambda event:self.hover(event,self.icbtn))
        self.icbtn.bind("<Leave>",lambda event:self.hover(event,self.icbtn))
    
    def hover(self,event,ctrl):
        tp = str(event.type)
        if tp == 'Enter':
            ctrl['bg'] = '#637f71'
        else:
    ...
    def circlewin(self,to='auto'):   # 1方角转圆角 0圆角转方角
        if to == 'auto':
            to = self.savemode
        if to:
            self.savemode = 0
    ...
    def to_resize(self,event):
        ...
        if self.edown2:
            ...
            if self.win_minsize[1] > newh-self.radius-self.outsideborder:
    ...
    def zoomed(self):
        ...
        if self.state() == 'zoomed':
            ...
            self.circlewin(not self.savemode)
        else:
