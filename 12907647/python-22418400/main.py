...
colorList = ['red','blue','black','white','orange','green','yellow','lightyellow','purple','skyblue','pink','lightskyblue']
addflist = 'py编译器.jhx\n中文.py\ngameeasy.py\nsave_file.py\n'
...
def addf():
    global addflist
    filenames = tkinter.filedialog.askopenfilenames(initialdir='D:/system_jhxc/python_file',title='上传文件')
    if len(filenames) != 0:
        filename =""
        for i in range(0,len(filenames)):
            filename += str(filenames[i])
    try:
        addflist += os.path.basename(filename)+'\n'
    except:
        pass
    try:
        with open(filename,'rb') as f:
            a = r"~/学而思直播/code/cache/asset/22418400/"
            me = os.path.expanduser(a+os.path.basename(filename))
            with open(me,'wb') as f2:
                f2.write(f.read())
                f2.close()
            f.close()
    except:
...
def delf():
    filenames = tk.filedialog.askopenfilenames(initialdir='D:/system_jhxc/python_file',title='选择删除文件',filetypes=[("Text files", "*.txt")])
    if len(filenames) != 0:
        filename =""
        for i in range(0,len(filenames)):
            filename += str(filenames[i])
...
def alladdf():
    global addflist
    messagebox.showinfo('所有文件',addflist)
def alladdf2(event):
    alladdf()
...
# 滚动条
sbar_y = tk.Scrollbar(root)
sbar_y.pack(side=tk.RIGHT,fill=tk.Y)
sbar_x = tk.Scrollbar(root,orient = tk.HORIZONTAL)
sbar_x.pack(side=tk.TOP,fill=tk.X)

textbox = tk.Text(height=31,width=100,font=('黑体',14),undo=True,yscrollcommand=sbar_y.set,xscrollcommand=sbar_x.set,wrap = 'none')
...
fgcolorbutton = tk.Button(bg='lightyellow',fg='green',text=' '*81+'字体换色'+' '*81,command=fgcolorf)
runStart = tk.Button(bg='orange',fg='white',text=' '*80+'▶运行(F5)'+' '*80,command=run_main)
pygame_exit_window = tk.Button(bg='lightskyblue',text=' '*75+'关闭pygame窗口'+' '*75,command=pygame_exit)
svtext = tk.StringVar()
...
fileMenu.add_command(label="上传文件",command=addf,accelerator='F3')
...
fileMenu.add_command(label="所有文件",command=alladdf,accelerator='Ctrl+Q')
