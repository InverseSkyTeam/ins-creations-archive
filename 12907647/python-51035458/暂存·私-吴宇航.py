...
class Class:
    def __init__(self,closure,attrs):
        self.closure, self.attrs = closure, attrs
    def __call__(self,runner,*args):
        obj = Dict(self.attrs)
        runner(Call(obj.get('__init__'),[obj,*args]),self.closure)
        return obj
    ...
    def run(self,tree,scope):
        ...
        if tp == Let:
            ...
            if type(left) == Index:
                ...
                return value
            if type(left) == Dot:
                dic = self.run(left[0],scope)
                dic.set(left[1],right)
                sys.exit()
        ...
        if tp == Defclass:
            ...
            scope[-1][name] = Class(scope+[class_scope],class_scope)
            return
        if tp == If:
        ...
        if tp == Continue:
            return tree
        if tp == Call:
            if type(tree[0]) not in (Var,Dot):    # 类方法来的
                func = tree[0]
                return func(self.run,*tree[1])
