...
try:
    with open(system_site+'/prove/the_prove.txt','r',encoding='utf-8') as f:
        if f.read() == 'New installed AS|REAL Edition.':
            part = 'main'
            f.close()
        else:
...
def rename(name,Ff3=False):
    ...
    def check_button_ok():
        ...
        try:
            ...
            print('\ / : * ? " < > |')
            print('这些符号都是非法命名，为系统运行正常，请不要再取这样(包含已上符号)的名字')
        update_all()
...
while 1:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    ...
                    with open(system_site+'/prove/the_prove.txt','w',encoding='utf-8') as f:
                        f.write('New installed AS|REAL Edition.')
                    f.close()
                    with open(desktop_site+'/info.txt','w',encoding='utf-8') as f:
                        f.write('小轩电脑1.0Edition(AS)正式版 32MB\n开心每一天^_^')
                    f.close()
                    update_all()
                    part = 'install_ok'
        screen.fill((255,255,255))
        show_text('感谢您选择小轩电脑!',pos=(11,11))
        show_text('小轩电脑0.13.5-0.18.9.1Edition均未发布',pos=(11,41))
        show_text('您将直接升级体验 小轩电脑 AS Edition!',pos=(11,71))
        show_text('将会在C盘学而思安装',pos=(11,101))
        show_text('请注意，要确保您的C盘至少拥有35MB的空间',color=(255,30,111),pos=(11,141))
        show_text('更快，更好，更实用，更美观，更方便',pos=(11,181))
        show_text('小轩电脑第一个1.0大版本系统,Alpha Start!(AS)',pos=(11,211))
        show_text('如确保无误，按下空格安装',pos=(11,241))
    ...
    while part == 'install_ok':
        ...
        show_text('安装完成！空格键下一页',pos=(11,11))
        show_text('版本：小轩电脑1.0Edition',pos=(11,41))
        pygame.display.update()
        try:root.update()
        except:sys.exit()
    ...
    while part == 'main':
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    for i in filelist:
                        ...
                        if i_types and ((not rightlist.show) and rightlist.type=='type-normal'):
                            if i_types[0] == '.txt':
                                txt_w(i_types[1])
                            elif i_types[0] == '.Ff3':
        ...
        try:
            ...
            pass
        show_text('小轩电脑AS | 存储:'+str(round(file_size,2))+'MB/32MB',pos=(11,11))
    ...
    while part == 'BLUESCREEN':
        ...
        show_text('错误问题：file_size > 32MB',color=(255,255,255),pos=(11,41))
    ...
    while part == '删除系统':
        ...
        show_text('小轩电脑1.0:请不我删除我好吗？(O_O)呜呜呜(T_T)',color=(255,255,255),pos=(11,41))
