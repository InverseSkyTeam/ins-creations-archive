...
model_old = ModelOld('data/cube/picture.obj', 'data/cube/logo.png', div_num=(5, 3.5), pos=(30, 26, 81))
...
class Window:
    def __init__(self, screen):
        ...
        # 处理世界的模型实例。
        self.model = Model()
        self.model.initialize(452692)  # 生成世界
    ...
    def render_3d(self, width, height, screen1):
        ...
        if not res1:
            render_cube(self.model.model_drop, height, width, self.screen, z_buffer, angle1, angle2,
                        tx, ty, tz, PLAYER_FOV + self.fov_offset)
        model_old.texture_array = pygame.surfarray.pixels3d(screen1).copy()[-1::-1, -1::-1, :]
