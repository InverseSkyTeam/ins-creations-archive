...
def yinchang_000():
    ...
    BOX_POS = [230, 230]
    BOX_SIZE = [170, 160]
    sans.say("今 天 是 多 么……美 好 的 一 天 啊……")
...
def yinchang_1():
    ...
    if DEBUG:
        ...
        pass
    # 测试区结束
    sans.say("准备好了？不满392血不要哭哦~（doge）咩~")
...
def first_round4():
    ...
    sans.say("只是一个开头杀而已，何必用尽全力？        emmm...        \n下一个回合吧")
...
players_turn("* You are really brave, stick to here?!")
...
players_turn("* Xiao Xuan and toby are laughing you!")
...
players_turn("* az,you're quite nb!(Blood volume care)")
...
players_turn("* This determination disturbs sans from drink coke or ketchup")
...
players_turn("* you will die...")

@add_attack
def first_round3_222():
    set_turn_time(450)
    player.type = RED_SOUL
    for _ in range(0, 300, 2):
        bones.append(
...
def mercy1():
    ...
    sans.say("huff,puff.好了，我也累了，不如我们休息一下？")
...
@add_attack
def mercy3():
    sans.say("赶紧按下饶恕，不要不给面子，小轩还想让我讲冷笑话呢")

@add_attack
def mercy4():
    sans.headtype = SANS_NO_EYES
    sans.say("这也是个改过自新的机会，你绝对不想见到下个回合")
    ...
    while True:
        ...
        if is_players_turn: # 玩家回合
            ...
            if page == MERCY_SANS_SPARE: # 你都饶恕了，想必也不想继续玩了（）
                print('你都饶恕了，想必也不想继续玩了。\nsans对你冷笑一声，就向你砍了一万刀')
                exit()

            if page == MERCY_SANS_FLEE: # 你都逃跑了，想必也不想继续玩了（）
                print('你都逃跑了，想必也不想继续玩了。\nsans对你冷笑一声，给你一个大大的嘲笑')
                exit()
