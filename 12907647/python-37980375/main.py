...
tab = '    '
spc = ' '
Version = 'V0.6+ build 546 [insjhx] 逆天小轩 all rights reversed.'
...
class LineRunner(CodesRunner):
    def __init__(self,command):
        ...
        self.helptext = '0.6+的帮助懒得写，自己点击改编查看；out VersionNews输出更新日志'
        if self.command.replace(' ','').replace('\n','').replace('\t','') == '':
            ...
            if len(self.commandlist) < 2:
                if self.commandlist[0][count*4:] in ['else','break','conti']:
                    self.commandlist = [self.commandlist[0],'/']
                    self.command += ' /'
                else:
    ...
    def run(self) -> str:   # 运行单行
        global codetype,codepool,codecmdline,indent_pool,indentkeyword,breakpointflag
        if codetype in ['insjhx','default'] or ('$!' in self.command and '!$' in self.command):
            ...
            if not sp_assign[0]:
                sp_assign = (self.commandlist[0]+' '+self.commandlist[1]).split('=')
            ...
            if parser.must_indent() and (self.commandlist[0] != 'space' or is_assign):
                if self.command[:4] == '    ':
                    ...
                    if indentkeyword == 'if':
                        if self.commandlist[0] not in ['elif','else']:
                            ...
                            try:
                                if eval(indent_pool[-1][1]):
                                    ...
                                    self.blockrunner.quickruncode2()
                                print('if-block ok.',breakpointflag)
                            except:
                    ...
                    elif indentkeyword == 'else':
                        indentkeyword = ''
                        if indent_pool[-2][0] == 'if':
                            if len(indent_pool[-2]) == 2:
                                del indent_pool[-2]
                                return '[Error|IndentError] the inside indent is bad.'
                            if len(indent_pool[-1]) == 2:
                                del indent_pool[-1]
                                return '[Error|IndentError] the inside indent is bad.'
                            try:
                                if eval(indent_pool[-2][1]):
                                    self.blockrunner = BlockRunner(indent_pool[-2][2])
                                else:
                                    self.blockrunner = BlockRunner(indent_pool[-1][2])
                                self.blockrunner.quickruncode2()
                                print('if-else-block ok.')
                            except:
                                print('[Error|IfError] the code in if-else-block is bad.')
                            del indent_pool[-1]; del indent_pool[-1]
                        else:
                            if len(indent_pool[-1]) == 2:
                                del indent_pool[-1]
                                return '[Error|IndentError] the inside indent is bad.'
                            ...
                            while True:
                                index -= 1
                                if indent_pool[index][0] == 'if':
                                    break
                                if len(indent_pool[index]) == 2:
                                    del indent_pool[index]
                                    return '[Error|IndentError] the inside indent is bad.'
                            if len(indent_pool[index]) == 2:
                                del indent_pool[index]
                                return '[Error|IndentError] the inside indent is bad.'
                            try:
                                s = 0
                                for condition in range(index,-1):
                                    if eval(indent_pool[condition][1]):
                                        self.blockrunner = BlockRunner(indent_pool[index][2])
                                        s = 1
                                        break
                                if not s:
                                    self.blockrunner = BlockRunner(indent_pool[index][2])
                                self.blockrunner.quickruncode2()
                                print('if-elif-else-block ok.')
                            except:
                                print('[Error|IfError] the code in if-elif-else-block is bad.')
                            while indent_pool[-1][0] != 'if':
                                del indent_pool[-1]
                            del indent_pool[-1]
                        indentkeyword = ''
                    
                    elif indentkeyword == 'elif':
                        if self.commandlist[0] not in ['elif','else']:
                            indentkeyword = ''
                            index = 0
                            while True:
                            ...
                            if len(indent_pool[index]) == 2:
                                ...
                                return '[Error|IndentError] the inside indent is bad.'
                            try:
                                for condition in range(index,0):
                                    if eval(indent_pool[condition][1]):
                                        self.blockrunner = BlockRunner(indent_pool[index][2])
                                        break
                                self.blockrunner.quickruncode2()
                                print('if-elif-block ok.')
                            except:
                                print('[Error|IfError] the code in if-elif-block is bad.')
                            while indent_pool[-1][0] != 'if':
                                del indent_pool[-1]
                            del indent_pool[-1]
                            indentkeyword = ''
                    
                    elif indentkeyword == 'while':
                        indentkeyword = ''
                        if len(indent_pool[-1]) == 2:
                            del indent_pool[-1]
                            return '[Error|IndentError] the inside indent is bad.'
                        ...
                        try:
                            while eval(indent_pool[-1][1]) and (not breakpointflag):
                                self.blockrunner = BlockRunner(indent_pool[-1][2])
                                self.blockrunner.quickruncode2()
                            breakpointflag = False
                            print('while-block ok.')
                        except Exception as e:
                            print('[Error|WhileError] the code in while-block is bad.',str(e))
            ...
            try:
                ...
                elif self.commandlist[0] == 'range':
                    ans = self.cmd_range(eval(self.commandlist[1]))
                ...
                elif self.commandlist[0] == 'elif':
                    ans = self.cmd_i_elif(self.commandlist[1])
                elif self.commandlist[0] == 'else':
                    ans = self.cmd_i_else()
                elif self.commandlist[0] == 'while':
                    ans = self.cmd_i_while(self.commandlist[1])
                elif self.commandlist[0] == 'for':
                    ans = self.cmd_i_for(self.commandlist[1])
                elif self.commandlist[0] == 'break':
                    ans = self.cmd_i_break()
                elif self.commandlist[0] == 'conti':
                    ans = self.cmd_i_conti()
                elif self.commandlist[0] == 'read':
                    ans = self.cmd_readin(self.commandlist[1])
    ...
    def cmd_i_elif(self,cmd):
        global indent_pool,indentkeyword
        if indentkeyword in ['if','elif']:
            indent_pool.append(['elif',cmd])
            indentkeyword = 'elif'
            return '--is in elif'
        return '[Error|IfError] there is no if or elif and you make a new elif.'
    
    def cmd_i_else(self):
        global indent_pool,indentkeyword
        if indentkeyword in ['if','elif']:
            indent_pool.append(['else','/'])
            indentkeyword = 'else'
            return '--is in else'
        return '[Error|IfError] there is no if or elif and you make an else.'
    
    def cmd_i_while(self,cmd):
        global indent_pool,indentkeyword
        indent_pool.append(['while',cmd])
        indentkeyword = 'while'
        return '--is in while'
    
    def cmd_i_for(self,cmd):
        global indent_pool,indentkeyword
        indent_pool.append(['for',cmd])
        indentkeyword = 'for'
        return '--is in for'
    
    def cmd_i_break(self):
        return (True,'break','_call')
    
    def cmd_i_conti(self):
        return (True,'continue','_call')
    
    def cmd_readin(self,cmd):
        print('start read.')
        with open(cmd+'.py','r',encoding='utf-8') as f:
            codeblockpackage = f.read()
            f.close()
        print('end read.')
        blockrunner = BlockRunner(codeblockpackage)
    ...
    def quickruncode2(self):
        ...
        for self.codeline in self.codelist:
            ...
            try:
                ...
                ansin2 = ''
            if ansin == (True,'continue','_call') or ansin2 == (True,'continue','_call'):
                return
            if ansin == (True,'break','_call') or ansin2 == (True,'break','_call'):
                breakpointflag = True
                return
