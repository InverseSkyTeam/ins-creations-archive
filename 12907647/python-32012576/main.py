...
def configscale():
    ...
    def setscale():
        global picscale
        picscale = round(float(configscaleentry.get()),1)
        print('比例尺(1:'+str(picscale)+')设置成功！精确度:'+str(1/picscale))
        configscalewindow.destroy()
    configscalelabel = tk.Label(configscalewindow,text='在下面输入x的值，将会按1:x的比例尺放大图像\n0.2≤x≤5 且 x越小越精确\nx最多保留小数点后1位')
    configscaleentry = tk.Entry(configscalewindow)
    configscalebutton = tk.Button(configscalewindow,text='设置此比例尺',command=setscale)
