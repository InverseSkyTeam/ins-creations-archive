...
class Life:
    def __init__(self,size,hp,ab,speed,hlimit,intel):
        self.size = round(size)
        self.tothp = self.hp = round(hp,1)
        self.intellect = round(intel)
        self.live_time = 0
        self.to_new_time = 0
    ...
    def get_power(self):
        return round(self.hp * 0.4 + self.hunt_limit * 0.9 + self.size * 7 + self.speed * 15 + self.get_attack(),2)
    def get_attack(self):
    ...
    def able_to_attack(self,target):
        ...
        hunt_radius = (self.speed - target.speed) * self.hunt_limit * 1.3 + 1
        return (self.get_power() - target.get_power() > 50) and (hunt_distance < hunt_radius)
    ...
    def act(self,_range):
        ...
        self.keep(_range)
    def show(self):
        pygame.draw.rect(screen,(int(self.hp*255/self.tothp),int(self.hp*255/self.tothp),int(self.speed*255/15)),self.rect,0)
    ...
    def move(self):
        ...
        if self.target:
            if abs(self.position[0] - self.target.position[0]) < HALFWIDTH:
                if self.position[0] < self.target.position[0] - self.target.size / 2: self.position[0] += self.speed
                elif self.position[0] > self.target.position[0] + self.target.size / 2: self.position[0] -= self.speed
            else:
                if self.position[0] < self.target.position[0] - self.target.size / 2: self.position[0] -= self.speed
                elif self.position[0] > self.target.position[0] + self.target.size / 2: self.position[0] += self.speed
            if abs(self.position[1] - self.target.position[1]) < HALFHEIGHT:
                if self.position[1] < self.target.position[1] - self.target.size / 2: self.position[1] += self.speed
                elif self.position[1] > self.target.position[1] + self.target.size / 2: self.position[1] -= self.speed
            else:
                if self.position[1] < self.target.position[1] - self.target.size / 2: self.position[1] -= self.speed
                elif self.position[1] > self.target.position[1] + self.target.size / 2: self.position[1] += self.speed
    ...
    def create_new(self,new_range):
        ...
        if new_attr <= 1500:
            new_range.append(Life(self.size,self.tothp,self.attack_basic,self.speed,self.hunt_limit,self.intellect,))
        elif new_attr <= 8500:
            new_range.append(Life(
                ...
                self.speed,
                max(min(self.hunt_limit + random.randint(-5,5), 600), 1),
                self.intellect,
            ))
        elif new_attr <= 9500:
            new_range.append(Life(
                ...
                max(min(self.hunt_limit * random.randint(8,12) / 10, 600), 1),
                self.intellect,
            ))
        elif new_attr <= 9900:
            new_range.append(Life(
                ...
                max(min(self.hunt_limit * random.randint(8,11) / 10, 400), 20),
                self.intellect,
            ))
        else:
            new_range.append(Life(
                ...
                max(min(self.hunt_limit * random.randint(3,13) / 10, 200), 10),
                self.intellect,
            ))
        self.hp -= 3

l = Life(30,100,300,3,80,15)
lives = [Life(10,100,10,1,100,10) for i in range(80)]
lives.append(l)
