import requests,json,webbrowser as wbs
def help_():
...
作品访问器.收藏(网址)
作品访问器.评论数(网址)
作品访问器.api(网址) 直接跳转到api网页，不再输出
作品访问器.作品说明(网址) 如果没有值，表示无作品说明
作品访问器2.代码多少(网址) 这个是第二个作品访问器
作品访问器2.运行源代码(网址) 这个研究项目前不成熟，无法加载素材
作品访问器3.pt问答() print询问作品作品链接
作品访问器3.tk问答() tk询问作品作品链接
作品访问器3.展示(网址) 展示作品综合信息
作品访问器4.热度(网址)
作品访问器4.手动变量(网址) 手动变量是一种来手动调整热度的变量，详情见https://code.xueersi.com/home/project/detail?lang=code&pid=32654247&version=offline&form=python&langType=python
help_() 输出此帮助（你既然都知道了，看这条还有啥用呢）
''')
def 版权():
    print('小轩&&宇宙工作室')
class 作品访问器(object):
...
class 作品访问器2(object):
    ...
    def 代码多少(cls,url):
        ...
        if lenth <= 1000:
            return_text = '代码极少，可能非常氵，有'+str(lenth)+'个字符，大约有1~50行'
        elif lenth <= 10000:
...
class 作品访问器3(object):
    ...
    def 展示(cls,url):
        ...
        tk_helper.show_data('作品全面剖析器-|宇宙工作室|小轩',url)
    @classmethod
    def tk问答(cls):
        from codeBreaker.tk_helper import tk
        def submit():
            try:
                作品访问器3.展示(ask.get())
            except:
                label['text']='请输入正确的作品链接!'
                label['fg']='red'
        root = tk.Tk()
        label = tk.Label(root,text='输入作品链接',bg='yellow')
        ask = tk.Entry(root,bg='yellowgreen')
        button = tk.Button(root,text='                提交(submit)                ',command=submit,bg='orange')
        label.grid(row=0,column=0)
        ask.grid(row=0,column=1)
        button.grid(row=2,column=0,columnspan=9999)
        root.mainloop()
    @classmethod
    def pt问答(cls):
        作品访问器3.展示(input('输入你要找的作品的网址（复制，右键粘贴）'))
...
class 作品访问器4(object):
    ...
    def 热度(cls,url):
        ...
        a=requests.get(url=pid, headers=headers)
        sdf=json.loads(a.text)
        return sdf['data']['popular_score']
    @classmethod
    def 手动变量(cls,url):   # 手动变量模块的分析特别鸣谢邢瑞林
        pid=str(url.split("id=")[1].split("&")[0])
        pid = 'https://code.xueersi.com/api/compilers/'+pid+'?id='+pid
        headers = {'Content-Type':'application/json'}
        a=requests.get(url=pid, headers=headers)
        sdf=json.loads(a.text)
        return sdf['data']['manual_weight']