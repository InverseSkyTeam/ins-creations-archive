...
def get_pos_undergroundwater(args):
    return (random.randint(0,width-1),random.randint(height,whole_height-1))
...
def make_undergroundwater(x,y,*mores):
    ...
    while water > 0:
        ...
        while ((thisx+nextxadd, thisy+nextyadd) in waterlist) or (distance > distance_most) or over:
            nextxadd = random.randint(-2,2)
            nextyadd = random.randint(-2,2)
            over = not (0 <= thisx+nextxadd < width and height < thisy+nextyadd < whole_height)
            distance = abs(thisx+nextxadd-x) + abs(thisy+nextyadd-y)
...
print('本次生成达到预期效果。\n彩蛋:第3版开始用装饰器，开始写成屎山但是可以运行。打开改编，它代码就在“彩蛋.py”里。含义是“我竟然写了一个带参装饰器，它的参数是一个函数，这个函数的功能是返回一个可变函数，而这个装饰器又是装饰另一个函数的”，所以看懂有些难度。现在改进了，提高了可读性。另外，速度很快，我感觉很爽。')
