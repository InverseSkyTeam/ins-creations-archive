# TODO: Drop/Bag/Box有[ TODO ]标记
# TODO: 物品放置
# TODO: 区分bag和inventory
# TODO: 背包物品交互
# TODO: 物品、工具使用
# TODO: 特殊方块交互（如工作台，熔炉，箱子，它们也有bag）
# TODO: 新增生物
# TODO: 修饰：游戏时间、天体运行、背景颜色
...
class Map:
    ...
    def put(self, row, col):
        if row >= 0 and col >= 0 and (not self.data[row][col]):
            bid = self.visitor.bag.items[-1][inventory_choose_index].card.push()
            if bid // 1000 == 2:
                self.data[row][col] = binfo[bid][1]
    @property
    def x(self):
...
class Box:
    def __init__(self, pos, tp='normal'):
        self.text = '<box>'
        self.card = Card(0, 0)
        self.pos = pos
        self.states = {
...
class PlayerBag(Bag):
    ...
    def include(self, card):
        ...
        if inventory_record[0] == 1:
            self.items[-1][inventory_record[1]].card.add()
            return True
        ...
        for l in range(self.row - 1):
            ...
            if bag_record[0] == 1:
                self.items[l][bag_record[1]].card.add()
                return True
            if first_empty == -1:
        ...
        if vx < 0:
            ...
            for row in range(top, bottom + 1):
                if right >= MPW - 1 or data[row][right+1]:
                    ...
                    break
        self.v(x = vx)             # 不先移动x的后果是，遇到右上角向右下这类情况，直接卡墙！
...
class Drop(Thing):
    ...
    def draw(self, gamemap):
        ...
        # TODO: 下面判断应该连drop本身的宽度也考虑进去，否则左边边界加载不完全，右边却加载了不必要的多余部分
...
class Player(AutoLiving):
    def __init__(self, w=40, h=120):
        ...
        self.page.include(Box((self.page.rect.left + 80 + BOX_SIZE * 2, self.page.rect.top + 100 + BOX_SIZE // 2), '<box-craft>'))
    def control(self):
        if self.floating:
            self.speedup(addsy=g)
...
# def find_selected_box(bag, pos):
#     for line in bag.items:
#         for b in line:
#             if collision(b.pos[0], b.pos[1], BOX_SIZE, BOX_SIZE, pos[0], pos[1], 1, 1):
#                 return b

# def find_selected_box_in_page(page, pos):
#     for b in page.items:
#         if '<box' in b.text and collision(b.pos[0], b.pos[1], BOX_SIZE, BOX_SIZE, pos[0], pos[1], 1, 1):
#             return b
