...
class INSwindow(tk.Tk):
    def __init__(self,w,h,winx=0,winy=0,radius=25,title='INS window',bg='#313131',titlebg='#111111',icon=None):
        ...
        self.headislandMenu.add_command(label='最大化/正常化',command=self.zoomed)
        self.headislandMenu.add_command(label='最小化/正常化',command=self.iconic)
    ...
    def get_evt_pos(self,event):
        self.evt_pos = (event.x_root,event.y_root,self.winfo_x(),self.winfo_y())
...
root = INSwindow(400,300,400,100,title='INSwindow(0.7 dev)')
