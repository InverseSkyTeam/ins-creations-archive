        ...
        for item in state.items:
            if item.pos < len(item.product.body):
                for char in terminal:
                    if (state.status, char) not in goto_table:
                        continue
                    goto_state = goto_table[(state.status, char)]
                    if (state.status, char) not in action_table:
                        action_table[(state.status, char)] = ("SHIFT", goto_state)
                    elif (state.status, char) in conflicts:
                        conflicts[(state.status, char)].append(("SHIFT", goto_state, item))
                    else:
                        conflicts[(state.status, char)] = [action_table[(state.status, char)], ("SHIFT", goto_state, item)]
            elif item.product.name != "S'":
                if (state.status, item.follow) not in action_table:
                    action_table[(state.status, item.follow)] = ("REDUCE", item.product)
                elif (state.status, item.follow) in conflicts:
                    conflicts[(state.status, item.follow)].append(("REDUCE", item.product))
                else:
                    conflicts[(state.status, item.follow)] = [action_table[(state.status, item.follow)], ("REDUCE", item.product)]
            elif item.follow == "$":
                action_table[(state.status, "$")] = ("ACCEPT", None)
    for k, v in conflicts.items():
        if len(v) == 2 and (v[0][0] == "SHIFT" and v[1][0] == "REDUCE" or v[0][0] == "REDUCE" and v[1][0] == "SHIFT"):
            if v[0][0] == "SHIFT" and v[1][0] == "REDUCE":
                shift, reduce_ = v
            else:
                shift, reduce_ = v
            shift_item, reduce_item = shift[2], reduce_[1]
            ...
            for term in reduce_item.product.body[::-1]:
                if term in terminal:
                    ...
                    break
            if shift_priority is not None and reduce_priority is not None:
                if shift_priority < reduce_priority:
                    action_table[k] = shift[:2]
                elif shift_priority > reduce_priority:
                    action_table[k] = reduce_
                else:
                    if shift_associativity == "LEFT" and reduce_associativity == "LEFT":
                        ...
                        action_table[k] = ("CONFLICT", {"SHIFT": shift[:2], "REDUCE": reduce_})
            else:
                action_table[k] = ("CONFLICT", {"SHIFT": shift[:2], "REDUCE": reduce_})
        else:
            info = {"SHIFT": [], "REDUCE": []}
            for x in v:
                if x[0] == "SHIFT":
                    info["SHIFT"].append(x[:2])
                else:
                    info["REDUCE"].append(x)
            action_table[k] = ("CONFLICT", info)
    return action_table
...
class ProductNode:
    def __init__(self, product, children = None):
        self.product = product
        self.children = children if children else []
class StateNode:
    def __init__(self, state, depth, point = None):
        self.state = state
        self.depth = depth
        self.point = point if point else []
    def __eq__(self, other):
    ...
    def parse(self):
        ...
        while True:
            ...
            for i in active:
                node = active[i]
                state = node.state
                if (state, tp) in action_table:
                    action, arg = action_table[(state, tp)]
                ...
                if node.depth >= length:
                    ...
                    for arg in args:
                        ...
                        for _ in range(length):
                            children.append(node.point[0])
                            node = node.point[1]
                        product = ProductNode(arg, children[::-1])
                        nonterm = NontermNode(arg.name, [product])
                        goto_state = goto_table[(state, arg.name)]
                        if goto_state not in new_active:
                            new_active[goto_state] = StateNode(goto_state, node.depth + 1, [(nonterm, node)])
                        else:
                            new_active[goto_state].point.append((nonterm, node))
                else:
                    for arg in args:
                        childrens = {}
                        stack = [node]
                        ...
                        while stack:
                            if len(stack) == length + 1:
                                if stack[-1].state in childrens:
                                    childrens[stack[-1]].append(copy.deepcopy(children)[:-1])
                                else:
                                    childrens[stack[-1]] = [copy.deepcopy(children)[:-1]]
                                stack.pop()
                                children.pop()
                                index += 1
                                continue
                            node = stack[-1]
                            if index < len(node.point):
                                stack.append(node.point[index][1])
                                children.append(node.point[index][0])
                            else:
                                stack.pop()
                                children.pop()
                        for begin, ways in childrens.items():
                            nonterm = NontermNode(arg.name)
                            for way in ways:
                                product = ProductNode(arg, way[::-1])
                                nonterm.children.append(product)
                            goto_state = goto_table[(begin.state, arg.name)]
                            if goto_state not in new_active:
                                new_active[goto_state] = StateNode(goto_state, begin.depth + 1, [(nonterm, begin)])
                            else:
                                new_active[goto_state].point.append((nonterm, begin))
