    ...
    def move(self, map: Map):
        ...
        if self.vx < 0:
            for row in range(self.t_bl, self.b_bl + 1):
                if self.l_bl <= 0 or map.data[row][self.l_bl - 1]:
                    self.vx = -min(-self.vx, self.l_space)
                    break
        elif self.vx > 0:
            for row in range(self.t_bl, self.b_bl + 1):
                if self.r_bl >= MPW_bl - 1 or map.data[row][self.r_bl + 1]:
        ...
        if self.vy < 0:
            for col in range(self.l_bl, self.r_bl + 1):
                if self.t_bl <= 0 or map.data[self.t_bl - 1][col]:
                    self.vy = -min(-self.vy, self.t_space)
                    break
        elif self.vy > 0:
            for col in range(self.l_bl, self.r_bl + 1):
                if self.b_bl >= MPH_bl - 1 or map.data[self.b_bl + 1][col]:
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if slot_select_mode and event.button == 4:
                for page in showing_pages:
                    for slot in page.items:
                ...
                if slot_select_mode:
                    if event.key == ord('a'):         # 平均分配物品
                        ...
                        for page in showing_pages:
                            for slot in page.items:
                                if slot.selected:
                                    if (not mouse_slot.bid) or (slot.bid and slot.bid != mouse_slot.bid):
                                        break
                                    count += 1
                        if not count:
                            break
                        adds = mouse_slot.count // count
                        overflow = mouse_slot.count % count
                        for page in showing_pages:
                            for slot in page.items:
                                if slot.selected:
                                    slot.count += adds
                                    if (not slot.bid) and slot.count:
                                        slot.bid = mouse_slot.bid
                                    if slot.count > 64:
