...
class MainApp(object):
    ...
    def open_file_window(self):
        ...
        self.file_upload_btn = tk.Button(self.file_window,text='上传本地文件',command=file_upload)
        self.file_upload_lbl = tk.Label(self.file_window,text='文件路径：未上传')
        self.file_upload_ok_btn = tk.Button(self.file_window,text='发送上传的文件(发送后稍等)',command=file_send)
    ...
    def send_file(self,name):
        name1 = name.split('/')[-1]
        name2 = time.strftime('[%Y-%m-%d %H:%M:%S ')+name1+']'
        content = time.strftime('[%Y-%m-%d %H:%M:%S]')+self.username+':\n'
        self.chattextbox['state'] = 'normal'
        self.chattextbox.insert('end',content)
        self.chattextbox.insert('end',name2+'\n')
        self.chattextbox.insert('end','-'*70)
        self.save('.'+name1.split('.')[-1])
        self.chattextbox.see('end')
        self.chattextbox['state'] = 'disable'
        url = clib.upfile(name)
        dic = eval(clib.read_from_cloud('逆天聊-文件海洋-小轩端#01',12907647))
        dic[name2] = url
        clib.save_to_cloud('逆天聊-文件海洋-小轩端#01',str(dic),12907647)
    
    def read_file(self,name):
        url = eval(clib.read_from_cloud('逆天聊-文件海洋-小轩端#01',12907647))[name]
    ...
    def load(self):
        ...
        while self.running:
            ...
            if contentlist != newcontentlist or self.running == 'starting':
                ...
                # 文件转换按钮
                while 1:
                    indexstart = self.chattextbox.search('{[', "1.0")
                    indexend = self.chattextbox.search(']}', "1.0")
                    if (not indexstart) or (not indexend):
                        break
                    text2 = self.chattextbox.get(indexstart+'+2c',indexend)
                    self.chattextbox.delete(indexstart,indexend+'+2c')
                    def commandbtn(name='['+text2+']'):
                        Thread(target=self.read_file,args=[name]).start()
                    btn = tk.Button(self.chattextbox,text=text2,relief='flat',command=commandbtn)
                    self.chattextbox.window_create(indexstart, window=btn)
